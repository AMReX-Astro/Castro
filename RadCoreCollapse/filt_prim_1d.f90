subroutine ca_filt_prim(lo, hi, &
  ...
  integer :: i
  double precision :: rhotmpinv, p, e, X(nspec+1)

  double precision, parameter :: rhothreshold = 1.d-50

  if (filt_T .eq. 1) then

     do i=lo(1), hi(1)
        if (Snew(i,URHO) < rhothreshold .and.  &
             any(mask(i-1:i+1) .ne. 0.d0) ) then
  ...
  do i=lo(1), hi(1)
     ...
     X(nspec+1) = Stmp(i,UFX) * rhotmpinv
     ...
     Snew(i,UFX) = X(nspec+1) * Snew(i,URHO)
  end do
