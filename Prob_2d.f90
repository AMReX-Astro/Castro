...
subroutine ca_initdata(level,time,lo,hi,nscal, &
  ...
  do j = lo(2), hi(2)
     do i = lo(1), hi(1)
        call eos_given_RTX(state(i,j,UEINT),temppres(i,j),state(i,j,URHO), &
     ...
     TOL = 1.d-8
      ...
      do n = 1, NVAR
         ...
         if ( bc(1,1,n).eq.EXT_DIR .and. adv_l1.lt.domlo(1)) then
            ...
            print *, "ERROR: invalid BC in Prob_2d.f90"
            stop
               ...
               do i=adv_l1,adv_h1
                  ...
                  if (n .eq. URHO) then
                     ...
                     if (interp_BC) then
                        ...
                        call eos_given_RTX(eint,pres_above, &
                                           adv(i,j+1,URHO),adv(i,j+1,UTEMP), &
                                           adv(i,j+1,UFS:UFS-1+nspec)/adv(i,j+1,URHO))
                        ...
                        do iter = 1, MAX_ITER
                           ...
                           ! pressure from EOS
                           call eos_dpdr_given_RTX(eint,pres_zone,dens_zone,temp_zone, &
                                                   X_zone, dpdr)
                        ...
                        if (.not. converged_hse) then
                           print *, 'ERROR: failure to converge in -Y BC'
                           stop
                        endif
                     ...
                     call eos_given_RTX(eint,pres_zone,dens_zone,temp_zone,X_zone)
...
!     XLO
      if ( bc(1,1,1).eq.EXT_DIR .and. adv_l1.lt.domlo(1)) then
         print *, "We shoundn't be here (xlo denfill)"
         stop
      end if
