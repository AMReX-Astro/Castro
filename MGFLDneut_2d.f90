...
subroutine ca_compute_dedx(  &
  ...
  !$OMP PARALLEL DO PRIVATE(i,j,rho,xn,dT,dYe,T1,T2,Ye1,Ye2,e1,e2,dummy_pres)
  do j=dedT_l2, dedT_h2
  do i=dedT_l1, dedT_h1

     rho = S(i,j,URHO)
     xn(1:nspec) = S(i,j,UFS:UFS+nspec-1) / rho
     xn(nspec+1:nspec+naux)  = ye(i,j)
     ...
     call eos_given_RTX(e1, dummy_pres, rho, T(i,j), xn)
     xn(nspec+1:nspec+naux)  = Ye2
     call eos_given_RTX(e2, dummy_pres, rho, T(i,j), xn)
