...
We can now initialize a  simulation using data from a  plotfile. This should not be thought of as a restart mode, but rather
a new simulation with a special initialization. In order to use this
feature, you must make sure the  plotfile has the proper
...
consistent with the  grid structure.

MAESTRO Plotfile Requirements
=============================

The  plotfile needs to have the following variables:
...
-  Optional species (such as “X(C12)”) - there is an option to
   not read any species from the  plotfile. In this case, you
...
-  | **castro.MAESTRO_nspec** = 3
   | If you do not specify this, it
     will default to the number of species in the  network,
     “NumSpec”. We have this here because sometimes  and  will use different networks with different number of species.
...
-  | **castro.MAESTRO_cutoff_density** = 1.e6
   | The code will use
     ...
     **castro.MAESTRO_cutoff_density** to a tiny value, such that rho0
     ...
     INITDATA_OVERWRITE, we overwrite newly initialized  data in
...
-  | **castro.MAESTRO_init_type** = 2
   |  will read in data
     from the  plotfile, and then call the EOS to make sure that
...
a standard spherical  initialization.

#. | INITDATA_MAESTRO
   | This fills in the  state by taking
     the  data, calling the EOS, and making the proper variables
...
#. | INITDATA_OVERWRITE
   | This overwrites the initialized  data using the new 1D initial model for all cells that map into
     radial coordinates greater than r_model_start.

Additional Notes
================

Note that for both single-level and multilevel  to  initialization, the  base grid structure does not have to match
the  base grid structure, as long as the problem domain is the
same. For example, if the coarsest level in a  plotfile
contains :math:`64^3` cells divided into 8-\ :math:`32^3` grids, it is ok to use a
 base grid structure with 1-\ :math:`64^3` grid, 64-\ :math:`16^3` grids, or
anything else you can imagine - the grids don’t even have to be the
same size. As is normally the case, the  base grid structure is
created based on the parameters in the  inputs file, such as
**amr.max_grid_size**, **amr.blocking_factor**, etc.

Multilevel Restart
------------------

When initialing from a multilevel  plotfile, there are some
extra steps. First, you need to create a -compatible grids file
from the  plotfile. This can be done with the
...
tee gr0.maestro”, to generate the -compatible grids file. Note
that the base grid structure is still controlled by
**amr.max_grid_size**, **amr.blocking_factor**, etc., since in  BoxLib, the grids file only indicates the refined grid structure,
whereas in Fortran BoxLib the grids file contains the base grid and
refined grid structures.

Now, when you initialize the  simulation, you need to specify
the grid file using **amr.regrid_file = "gr0_3d.128_2levels"**,
for example. You can happily run this now, but note that the
regridding algorithm will never be called (since  thinks it’s
...
changed, you must do a traditional  restart from the
-generated checkpoint file (you can still use the same
...
Sometimes a  plotfile will only have 1 or 2 total levels, but
you ultimately want to run a  simulation with many more levels
of refinement. My recommended strategy is the following:

#. Initialize a  simulation from the  plotfile
   while preserving the exact same grid structure and run for 10 time
   steps.

#. Do a traditional  restart from chk00010, but do not
   ...
   before settle in using the   regridding algorithm.

#. Do a traditional  restart from chk00020, but increase
