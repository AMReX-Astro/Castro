...
\item {\tt castro.spherical\_star}: (0 or 1; default: 0)
...
\item {\tt castro.ppm\_reference} uses the integral under the parabola for
  ...
  The original \castro\ paper~\ref{castro_I} used the cell-centered
  value ({\tt ppm\_reference = 0}), but the method in the PPM paper~\ref{ppm}
...
\item {\tt castro.use\_colglaz} uses the Colella \& Glaz Riemann solver~\cite{colglaz} to
  ...
  \castro\ paper~\ref{castro_I}.

  The Colella \& Glaz solver is iterative, and two runtime parameters are used
  to control its behavior:
  \begin{itemize}
  \item {\tt castro.cg\_maxiter}: number of iterations for C/G algorithm
    (Integer; default: 12)
...
\begin{itemize}
\item You must have USE\_POINTMASS = TRUE in the GNUmakefile for {\bf
