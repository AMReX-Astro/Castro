       ...
       do i = lo(1), hi(1)
          ...
          if (rot_source_type == 1) then
            ...
            ! If rot_source_type == 1, then calculate SrEcorr before updating the velocities.
             ...
             unew(i,j,idir1) = unew(i,j,idir1) + HALF * dt * omega(rot_axis)**2 * r(idir1) * (rhon + rhoo) &
                             + dt * omega(rot_axis) * uold(i,j,idir2)
             unew(i,j,idir2) = unew(i,j,idir2) + HALF * dt * omega(rot_axis)**2 * r(idir2) * (rhon + rhoo) & 
                             - dt * omega(rot_axis) * uold(i,j,idir1)
             ...
             unew(i,j,idir1) = (mom1 + dt * omega(rot_axis) * mom2) / (ONE + (dt * omega(rot_axis))**2)
             unew(i,j,idir2) = (mom2 - dt * omega(rot_axis) * mom1) / (ONE + (dt * omega(rot_axis))**2)
