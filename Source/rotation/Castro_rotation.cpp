...
void Castro::construct_old_rotation_source(MultiFab& source, MultiFab& state_in,
                                           Real time, Real dt) {
...
#pragma gpu box(bx)
        ca_rsrc(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                ...
                BL_TO_FORTRAN_ANYD(volume[mfi]), AMREX_REAL_ANYD(dx), dt, time);
    }
    ...
    if (verbose > 1) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Castro::construct_old_rotation_source() time = "
                          << run_time << "\n"
                          << "\n";
#ifdef BL_LAZY
        });
...
void Castro::construct_new_rotation_source(MultiFab& source,
                                           MultiFab& state_old,
...
#pragma gpu box(bx)
            ca_corrrsrc(
                ...
                BL_TO_FORTRAN_ANYD((*mass_fluxes[2])[mfi]), AMREX_REAL_ANYD(dx),
                dt, time, BL_TO_FORTRAN_ANYD(volume[mfi]));
        }
    }
    ...
    if (verbose > 1) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Castro::construct_new_rotation_source() time = "
                          << run_time << "\n"
                          << "\n";
#ifdef BL_LAZY
        });
...
void Castro::fill_rotation_field(MultiFab& phi, MultiFab& rot,
                                 MultiFab& state_in, Real time) {
...
#pragma gpu box(bx)
        ca_fill_rotational_potential(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(phi[mfi]), AMREX_REAL_ANYD(dx), time);
    }
...
#pragma omp parallel
    ...
    for (MFIter mfi(state_in, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        const Box& bx = mfi.growntilebox(ng);
#pragma gpu box(bx)
        ca_fill_rotational_acceleration(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(rot[mfi]), BL_TO_FORTRAN_ANYD(state_in[mfi]),
            AMREX_REAL_ANYD(dx), time);
    }
}

AMREX_GPU_HOST_DEVICE
void Castro::inertial_to_rotational_velocity_c(
    const int i, const int j, const int k, const GeometryData& geomdata,
    const Real* center, const Real* omega, const Real time, Real* v) {
