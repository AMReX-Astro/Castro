...
Castro::strang_react_first_half(Real time, Real dt)
{
	...
	// Note that we want to use the "old" data here; we've already done a swap on the new data
	// for the old data, so this is really the new-time burn from the last timestep.
	...
	// Copy data from the state. Note that this is a parallel copy from FabArray, and the
	// parallel copy assumes that the data on the ghost zones in state is valid and consistent
	// with the data on the interior zones, since either the ghost or valid zones may end up
	// filling a given destination zone.

	state_temp->copy(state, 0, 0, state.nComp(), state.nGrow(), state.nGrow());

	// Create the mask. We cannot use the inrior_mask from above, generated by
