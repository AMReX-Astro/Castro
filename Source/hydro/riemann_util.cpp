...
void Castro::compute_flux_q(const Box& bx, Array4<Real const> const& qint,
                            Array4<Real> const& F,
#ifdef RADIATION
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) {
        ...
        if (fspace_t == 1) {
            for (int g = 0; g < NGROUPS; g++) {
                ...
                rF(i, j, k, g) =
                    (1.0_rt + f1) * qint(i, j, k, QRAD + g) * u_adv;
            }
        } else {
            // type 2
            for (int g = 0; g < NGROUPS; g++) {
        ...
        compute_hybrid_flux(qgdnv_zone, geomdata, center, idir, i, j, k,
                            F_zone);
        for (int n = 0; n < NUM_STATE; n++) {
            F(i, j, k, n) = F_zone[n];
        }
...
void Castro::store_godunov_state(const Box& bx, Array4<Real const> const& qint,
#ifdef RADIATION
                                 Array4<Real const> const& lambda,
#endif
