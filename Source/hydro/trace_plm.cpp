...
void Castro::trace_plm(const Box& bx, const int idir,
                       Array4<Real const> const& q_arr,
                       Array4<Real const> const& qaux_arr,
                       Array4<Real const> const& dq, Array4<Real> const& qm,
                       Array4<Real> const& qp,
#if AMREX_SPACEDIM < 3
                       Array4<Real const> const& dloga,
#endif
                       Array4<Real const> const& srcQ, const Box& vbx,
                       const Real dt) {
    ...
    if (ppm_type != 0) {
        std::cout << "Oops -- shouldnt be in tracexy with ppm_type != 0"
                  << std::endl;
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        Real trace_fac0 =
            0.0_rt;  //  FOURTH*dtdx*(e(1) - e(1))*(1.0_rt - sign(1.0_rt, e(1)))
        Real trace_fac1 = 0.25_rt * dtdx * (e[0] - e[1]) *
                          (1.0_rt - std::copysign(1.0_rt, e[1]));
        Real trace_fac2 = 0.25_rt * dtdx * (e[0] - e[2]) *
                          (1.0_rt - std::copysign(1.0_rt, e[2]));
        ...
        if ((idir == 0 && i >= vlo[0]) || (idir == 1 && j >= vlo[1]) ||
            ...
            qp(i, j, k, QRHO) =
                amrex::max(lsmall_dens, rho_ref + apright + amright + azrright);
            qp(i, j, k, QUN) = un_ref + (apright - amright) * cc / rho;
            qp(i, j, k, QUT) = ut_ref + azut1rght;
            qp(i, j, k, QUTT) = utt_ref + azutt1rght;
            qp(i, j, k, QPRES) =
                amrex::max(lsmall_pres, p_ref + (apright + amright) * csq);
            qp(i, j, k, QREINT) =
                rhoe_ref + (apright + amright) * enth * csq + azeright;
        ...
        trace_fac0 = 0.25_rt * dtdx * (e[2] - e[0]) *
                     (1.0_rt + std::copysign(1.0_rt, e[0]));
        trace_fac1 = 0.25_rt * dtdx * (e[2] - e[1]) *
                     (1.0_rt + std::copysign(1.0_rt, e[1]));
        trace_fac2 =
            0.0_rt;  //  FOURTH*dtdx*(e(3) - e(3))*(1.0_rt + sign(1.0_rt, e(3)))
        ...
        if (idir == 0 && i <= vhi[0]) {
            qm(i + 1, j, k, QRHO) =
                amrex::max(lsmall_dens, rho_ref + apleft + amleft + azrleft);
            qm(i + 1, j, k, QUN) = un_ref + (apleft - amleft) * cc / rho;
            qm(i + 1, j, k, QUT) = ut_ref + azut1left;
            qm(i + 1, j, k, QUTT) = utt_ref + azutt1left;
            qm(i + 1, j, k, QPRES) =
                amrex::max(lsmall_pres, p_ref + (apleft + amleft) * csq);
            qm(i + 1, j, k, QREINT) =
                rhoe_ref + (apleft + amleft) * enth * csq + azeleft;

            // add the source terms
            qm(i + 1, j, k, QRHO) =
                amrex::max(lsmall_dens, qm(i + 1, j, k, QRHO) +
            ...
            qm(i, j + 1, k, QPRES) =
                max(lsmall_pres, p_ref + (apleft + amleft) * csq);
            qm(i, j + 1, k, QREINT) =
                rhoe_ref + (apleft + amleft) * enth * csq + azeleft;

            // add the source terms
            qm(i, j + 1, k, QRHO) =
                amrex::max(lsmall_dens, qm(i, j + 1, k, QRHO) +
                                            0.5_rt * dt * srcQ(i, j, k, QRHO));
            ...
            // add the source terms
            qm(i, j, k + 1, QRHO) =
                amrex::max(lsmall_dens, qm(i, j, k + 1, QRHO) +
                                            0.5_rt * dt * srcQ(i, j, k, QRHO));
        ...
        for (int ipassive = 0; ipassive < npassive; ipassive++) {
            ...
            if ((idir == 0 && i >= vlo[0]) || (idir == 1 && j >= vlo[1]) ||
                ...
                qp(i, j, k, n) = q_arr(i, j, k, n) +
                                 0.5_rt * (-1.0_rt - spzero) * dq(i, j, k, n);
