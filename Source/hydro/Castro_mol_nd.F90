             ...
             do k = lo(3), hi(3)
                do j = lo(2), hi(2)
                   do i = lo(1), hi(1)
                      ...
                      qm(i+1,j,k,n) = q(i,j,k,n) + HALF*dq(i,j,k,n) + &
                           HALF*dx(1)*q(i,j,k,QRHO)*const_grav

                      ! right state at i-1/2 interface
                      qp(i,j,k,n) = q(i,j,k,n) - HALF*dq(i,j,k,n) - &
                              HALF*dx(1)*q(i,j,k,QRHO)*const_grav
                   end do
                end do
             end do
             ...
             do k = lo(3), hi(3)
                do j = lo(2), hi(2)
                   do i = lo(1), hi(1)
                      ...
                      ! left state at i+1/2 interface
                      qm(i,j+1,k,n) = q(i,j,k,n) + HALF*dq(i,j,k,n) + &
             ...
             do k = lo(3), hi(3)
                do j = lo(2), hi(2)
                   do i = lo(1), hi(1)
                      ...
                      ! left state at i+1/2 interface
                      qm(i,j,k+1,n) = q(i,j,k,n) + HALF*dq(i,j,k,n) + &
       ...
       if (lo(1) == domlo(1) .and. physbc_lo(1) == Symmetry) then
          ...
          do k = lo(3)-dg(3), hi(3)+dg(3)
             do j = lo(2)-dg(2), hi(2)+dg(2)
                ...
                qm(domlo(1),j,k,:) = qp(domlo(1),j,k,:)
                qm(domlo(1),j,k,QU) = -qp(domlo(1),j,k,QU)
       ...
       if (hi(1)+1 == domhi(1)+1 .and. physbc_hi(1) == Symmetry) then
          ...
          do k = lo(3)-dg(3), hi(3)+dg(3)
             do j = lo(2)-dg(2), hi(2)+dg(2)
                ...
                qp(domhi(1)+1,j,k,:) = qm(domhi(1)+1,j,k,:)
                qp(domhi(1)+1,j,k,QU) = -qm(domhi(1)+1,j,k,QU)
       ...
       if (lo(2) == domlo(2) .and. physbc_lo(2) == Symmetry) then
          ...
          do k = lo(3)-dg(3), hi(3)+dg(3)
             do i = lo(1)-1, hi(1)+1
                ...
                qm(i,domlo(2),k,:) = qp(i,domlo(2),k,:)
                qm(i,domlo(2),k,QV) = -qp(i,domlo(2),k,QV)
       ...
       if (hi(2)+1 == domhi(2)+1 .and. physbc_hi(2) == Symmetry) then
          ...
          do k = lo(3)-dg(3), hi(3)+dg(3)
             do i = lo(1)-1, hi(1)+1
                ...
                qp(i,domhi(2)+1,k,:) = qm(i,domhi(2)+1,k,:)
                qp(i,domhi(2)+1,k,QV) = -qm(i,domhi(2)+1,k,QV)
       ...
       if (lo(3) == domlo(3) .and. physbc_lo(3) == Symmetry) then
          ...
          do j = lo(2)-1, hi(2)+1
             do i = lo(1)-1, hi(1)+1
                ...
                qm(i,j,domlo(3),:) = qp(i,j,domlo(3),:)
                qm(i,j,domlo(3),QW) = -qp(i,j,domlo(3),QW)
       ...
       if (hi(3)+1 == domhi(3)+1 .and. physbc_hi(3) == Symmetry) then
          ...
          do j = lo(2)-1, hi(2)+1
             do i = lo(1)-1, hi(1)+1
                ...
                qp(i,j,domhi(3)+1,:) = qm(i,j,domhi(3)+1,:)
                qp(i,j,domhi(3)+1,QW) = -qm(i,j,domhi(3)+1,QW)
