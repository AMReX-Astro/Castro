...
void Castro::mol_plm_reconstruct(const Box& bx, const int idir,
                                 Array4<Real const> const& q_arr,
                                 Array4<Real const> const& flatn_arr,
                                 Array4<Real const> const& src_q_arr,
                                 Array4<Real> const& dq, Array4<Real> const& qm,
    ...
    if (use_pslope == 1) {
        pslope(bx, idir, q_arr, flatn_arr, dq, src_q_arr);
    }

    amrex::ParallelFor(
        bx, NQ, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k, int n) noexcept {
            ...
            if (idir == 0) {
                ...
                qm(i + 1, j, k, n) =
                    q_arr(i, j, k, n) + 0.5_rt * dq(i, j, k, n);
...
void Castro::mol_ppm_reconstruct(const Box& bx, const int idir,
                                 Array4<Real const> const& q_arr,
                                 Array4<Real const> const& flatn_arr,
