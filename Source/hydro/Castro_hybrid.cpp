...
Castro::construct_old_hybrid_source(Real time, Real dt)
{
    ...
    fill_hybrid_hydro_source(*old_sources[0], Sborder, ng, mult_factor);
}
...
Castro::construct_new_hybrid_source(Real time, Real dt)
{
    ...
    fill_hybrid_hydro_source(*new_sources[0], S_old, ng, mult_factor);

    // Time center with the new data.

    mult_factor = 0.5;

    fill_hybrid_hydro_source(*new_sources[0], S_new, ng, mult_factor);
