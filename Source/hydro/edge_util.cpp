...
void Castro::reset_edge_state_thermo(const Box& bx, Array4<Real> const& qedge) {
    ...
    amrex::ParallelFor(
        bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
            ...
            qedge(i, j, k, QPTOT) =
                qedge(i, j, k, QPTOT) + (qedge(i, j, k, QPRES) - old_p_state);
#endif
        });
}

void Castro::edge_state_temp_to_pres(const Box& bx, Array4<Real> const& qm,
