...
///
void Castro::construct_mol_hydro_source(Real time, Real dt,
                                        MultiFab& A_update) {

#ifdef RADIATION
    amrex::Abort(
        "Error: radiation not supported for the MOL hydro source term");
#else
    ...
    if (verbose && ParallelDescriptor::IOProcessor()) {
        std::cout << "... construct advection term, SDC iteration: "
                  << sdc_iteration << "; current node: " << current_sdc_node
                  << std::endl;
    }
...
#pragma omp parallel
#endif
    {
        ...
        for (MFIter mfi(S_new,
                        (sdc_order == 4) ? no_tile_size : hydro_tile_size);
             mfi.isValid(); ++mfi) {
            ...
            if (sdc_order == 4) {
                ...
                ibx[0] = amrex::grow(amrex::surroundingNodes(bx, 0),
                                     IntVect(AMREX_D_DECL(0, 1, 1)));
#if AMREX_SPACEDIM >= 2
                ibx[1] = amrex::grow(amrex::surroundingNodes(bx, 1),
                                     IntVect(AMREX_D_DECL(1, 0, 1)));
#endif
#if AMREX_SPACEDIM == 3
                ibx[2] = amrex::grow(amrex::surroundingNodes(bx, 2),
                ...
                for (int idir = 0; idir < AMREX_SPACEDIM; ++idir) {
                    ...
                    for (int n = 0; n < NQ; n++) {
                        ...
                        // operate on nbx1
                        fourth_interfaces(nbx1, idir, n, q_arr, q_int_arr);

                        // compute the limited interface states
                        // operate on obx -- this loop is over cell-centers
                        states(obx, idir, n, q_arr, q_int_arr, flatn_arr,
                               qm_arr, qp_arr);
                    }
                    ...
                    // operate on ibx[idir]
                    riemann_state(ibx[idir], qm_arr, qp_arr, q_avg_arr,
                                  qaux_arr, idir, 0);
                    ...
                    if (diffuse_temp == 1) {
                        bool is_avg = true;
                        fourth_add_diffusive_flux(ibx[idir], q_arr, QTEMP,
                                                  q_avg_arr, f_avg_arr, idir,
                                                  is_avg);
                    }
#endif
                    ...
                    amrex::ParallelFor(
                        ...
                        [=] AMREX_GPU_DEVICE(int i, int j, int k,
                            ...
                            Real lap = trans_laplacian(
                                i, j, k, n, idir, q_avg_arr, lo_periodic,
                                hi_periodic, domain_lo, domain_hi);

                            q_fc_arr(i, j, k, n) = q_avg_arr(i, j, k, n) -
                                                   (1.0_rt / 24.0_rt) * lap;
                        });
                    ...
                    if (diffuse_temp == 1) {
                        ...
                        bool is_avg = false;
                        fourth_add_diffusive_flux(nbx, T_cc.array(mfi), 0,
                    ...
                    amrex::ParallelFor(
                        ...
                        [=] AMREX_GPU_DEVICE(int i, int j, int k,
                                             int n) noexcept {
                            Real lap = trans_laplacian(
                                i, j, k, n, idir, f_avg_arr, lo_periodic,
                                hi_periodic, domain_lo, domain_hi);
                    ...
                    if (do_hydro == 1) {
                        ...
                        fourth_avisc(nbx, q_bar_arr, qaux_bar_arr, avis_arr,
                                     idir);
                ...
                for (int idir = 0; idir < AMREX_SPACEDIM; ++idir) {
                    ...
                    if (do_hydro) {
                        ...
                        if (ppm_type == 0) {
                            ...
                            Array4<Real> const src_arr =
                                sources_for_hydro.array(mfi);

                            src_to_prim(qbx, q_arr, src_arr, src_q_arr);

                            mol_plm_reconstruct(obx, idir, q_arr, flatn_arr,
                                                src_q_arr, dq_arr, qm_arr,
                                                qp_arr);

                        } else {

                            mol_ppm_reconstruct(obx, idir, q_arr, flatn_arr,
                                                qm_arr, qp_arr);
                        }

                        // ppm_temp_fix = 1
                        if (ppm_temp_fix == 1) {
                            edge_state_temp_to_pres(obx, qm.array(),
                                                    qp.array());
                        }
                        ...
                        cmpflx_plus_godunov(nbx, qm_arr, qp_arr, flux_arr,
                                            q_int_arr, qe_arr, qaux_arr,
                                            shk_arr, idir);
                    ...
                    ca_mol_diffusive_flux(
                        AMREX_ARLIM_ANYD(nbx.loVect()),
                        AMREX_ARLIM_ANYD(nbx.hiVect()), idir_f,
                        BL_TO_FORTRAN_ANYD(Sborder[mfi]),
                        BL_TO_FORTRAN_ANYD(cond),
                        BL_TO_FORTRAN_ANYD(flux[idir]), AMREX_ZFILL(dx));
...
#pragma gpu box(bx)
            ca_mol_consup(
                ...
                BL_TO_FORTRAN_ANYD(shk), BL_TO_FORTRAN_ANYD(statein),
                BL_TO_FORTRAN_ANYD(source_in), BL_TO_FORTRAN_ANYD(source_out),
                AMREX_REAL_ANYD(dx), dt, BL_TO_FORTRAN_ANYD(flux[0]),
#if AMREX_SPACEDIM >= 2
            ...
            if (time_integration_method == SpectralDeferredCorrections &&
                (current_sdc_node == 0 ||
                 sdc_iteration == sdc_order + sdc_extra - 1)) {
                ...
                if (!Geom().IsCartesian()) {
                    ...
                    AMREX_HOST_DEVICE_FOR_4D(
                        mfi.nodaltilebox(0), 1, i, j, k, n, {
                            P_radial_fab(i, j, k, 0) +=
                                scale * pradial_fab(i, j, k, 0);
                        });
                }
#endif
            }
    ...
    if (verbose > 0) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Castro::construct_mol_hydro_source() time = "
                          << run_time << "\n"
                          << "\n";
#ifdef BL_LAZY
        });
