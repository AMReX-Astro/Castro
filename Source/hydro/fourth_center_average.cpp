...
void Castro::make_cell_center(const Box& bx, Array4<Real const> const& U,
                              Array4<Real> const& U_cc,
                              GpuArray<int, 3> const& domlo,
                              GpuArray<int, 3> const& domhi) {
    ...
    amrex::ParallelFor(
        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
            Real lap = compute_laplacian(i, j, k, n, U, lo_periodic,
                                         hi_periodic, domlo, domhi);

            U_cc(i, j, k, n) = U(i, j, k, n) - (1.0_rt / 24.0_rt) * lap;
        });
}

void Castro::make_cell_center_in_place(const Box& bx, Array4<Real> const& U,
    ...
    for (int n = 0; n < U.nComp(); n++) {
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                tmp(i, j, k) = compute_laplacian(i, j, k, n, U, lo_periodic,
                                                 hi_periodic, domlo, domhi);
            });
...
void Castro::compute_lap_term(const Box& bx, Array4<Real const> const& U,
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
        lap(i, j, k) = (1.0_rt / 24.0_rt) *
                       compute_laplacian(i, j, k, ncomp, U, lo_periodic,
                                         hi_periodic, domlo, domhi);
    });
}

void Castro::make_fourth_average(const Box& bx, Array4<Real> const& q,
    ...
    amrex::ParallelFor(
        bx, q.nComp(),
        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
            Real lap = compute_laplacian(i, j, k, n, q_bar, lo_periodic,
                                         hi_periodic, domlo, domhi);
...
void Castro::make_fourth_in_place(const Box& bx, Array4<Real> const& q,
                                  Array4<Real> const& tmp,
                                  GpuArray<int, 3> const& domlo,
                                  GpuArray<int, 3> const& domhi) {
...
void Castro::make_fourth_in_place_n(const Box& bx, Array4<Real> const& q,
                                    const int ncomp, Array4<Real> const& tmp,
                                    GpuArray<int, 3> const& domlo,
                                    GpuArray<int, 3> const& domhi) {
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
        tmp(i, j, k) = compute_laplacian(i, j, k, ncomp, q, lo_periodic,
                                         hi_periodic, domlo, domhi);
    });
