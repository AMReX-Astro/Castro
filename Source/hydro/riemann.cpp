...
void Castro::cmpflx_plus_godunov(
    const Box& bx, Array4<Real> const& qm, Array4<Real> const& qp,
    Array4<Real> const& flx, Array4<Real> const& qint,
#ifdef RADIATION
    Array4<Real> const& rflx, Array4<Real> const& lambda_int,
#endif
    Array4<Real> const& qgdnv, Array4<Real const> const& qaux_arr,
    Array4<Real const> const& shk, const int idir) {
    ...
    if (riemann_solver == 0 || riemann_solver == 1) {
        ...
        riemann_state(bx, qm, qp, qint,
#ifdef RADIATION
                      lambda_int,
#endif
                      qaux_arr, idir, 0);

        compute_flux_q(bx, qint, flx,
#ifdef RADIATION
                       lambda_int, rflx,
#endif
                       idir, 0);
        ...
        // HLLC
        HLLC(bx, qm, qp, qaux_arr, flx, qint, idir);
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
                ...
                if (is_shock >= 1) {
                    ...
                    HLL(ql_zone, qr_zone, cl, cr, idir, coord, flx_zone);
...
void Castro::riemann_state(const Box& bx, Array4<Real> const& qm,
                           Array4<Real> const& qp, Array4<Real> const& qint,
#ifdef RADIATION
                           Array4<Real> const& lambda_int,
#endif
                           Array4<Real const> const& qaux_arr, const int idir,
    ...
    if (riemann_solver > 0) {
        amrex::Error(
            "ERROR: only the CGF Riemann solver is supported for radiation");
    }
#endif
#endif
    ...
    if (riemann_solver == 0) {
        ...
        riemannus(bx, qm, qp, qaux_arr, qint,
#ifdef RADIATION
                  lambda_int,
#endif
                  idir, compute_gammas);
...
#ifndef RADIATION
        riemanncg(bx, qm, qp, qaux_arr, qint, idir);
#endif
