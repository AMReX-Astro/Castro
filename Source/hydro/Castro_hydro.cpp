...
void Castro::cons_to_prim(const Real time) {

    BL_PROFILE("Castro::cons_to_prim()");

#ifdef RADIATION
    AmrLevel::FillPatch(*this, Erborder, NUM_GROW, time, Rad_Type, 0,
                        Radiation::nGroups);
...
#pragma omp parallel
    ...
    for (MFIter mfi(S_new, hydro_tile_size); mfi.isValid(); ++mfi) {
        ...
        ctoprim(qbx, time, Sborder_arr,
#ifdef RADIATION
                Erborder_arr, lamborder_arr,
#endif
                q_arr, qaux_arr);
    }
}
...
void Castro::cons_to_prim(MultiFab& u, MultiFab& q_in, MultiFab& qaux_in,
                          Real time) {
...
#pragma omp parallel
    ...
    for (MFIter mfi(u, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        ctoprim(bx, time, u_arr,
#ifdef RADIATION
                Erborder_arr, lamborder_arr,
#endif
                q_in_arr, qaux_in_arr);
    }
}

#ifdef TRUE_SDC
void Castro::cons_to_prim_fourth(const Real time) {
...
#pragma omp parallel
    ...
    for (MFIter mfi(S_new, hydro_tile_size); mfi.isValid(); ++mfi) {
        ...
        make_cell_center(qbxm1, Sborder.array(mfi), U_cc_arr, domain_lo,
                         domain_hi);

        // enforce the minimum density on the new cell-centered state
        ca_enforce_minimum_density(AMREX_ARLIM_ANYD(qbxm1.loVect()),
                                   AMREX_ARLIM_ANYD(qbxm1.hiVect()),
                                   BL_TO_FORTRAN_ANYD(U_cc), verbose);
        ...
        ctoprim(qbx, time, Sborder_arr, q_bar_arr, qaux_bar_arr);
        ...
        ctoprim(qbxm1, time, U_cc_arr, q_arr, qaux_arr);
    }
...
#pragma omp parallel
    ...
    for (MFIter mfi(S_new, hydro_tile_size); mfi.isValid(); ++mfi) {
        ...
        make_fourth_average(qbxm1, q.array(mfi), q_bar.array(mfi), domain_lo,
                            domain_hi);

        // not sure if we need to convert qaux this way, or if we can
        // just evaluate it (we may not need qaux at all actually)

        make_fourth_average(qbxm1, qaux.array(mfi), qaux_bar.array(mfi),
                            domain_lo, domain_hi);
    }
...
void Castro::check_for_cfl_violation(const Real dt) {
...
#pragma omp parallel
    ...
    for (MFIter mfi(S_new, hydro_tile_size); mfi.isValid(); ++mfi) {
        ...
        reduce_op.eval(
            ...
            [=] AMREX_GPU_HOST_DEVICE(int i, int j,
                ...
                Real courx =
                    (qaux_arr(i, j, k, QC) + std::abs(q_arr(i, j, k, QU))) *
                    dtdx;
                Real coury =
                    (qaux_arr(i, j, k, QC) + std::abs(q_arr(i, j, k, QV))) *
                    dtdy;
                Real courz =
                    (qaux_arr(i, j, k, QC) + std::abs(q_arr(i, j, k, QW))) *
                    dtdz;
                ...
                if (ltime_integration_method == 0) {
                    ...
                    if (verbose == 1) {
                        ...
                        if (courx > 1.0_rt) {
                            std::cout << std::endl;
                            std::cout << "Warning:: CFL violation in "
                                         "check_for_cfl_violation"
                                      << std::endl;
                            std::cout << ">>> ... (u+c) * dt / dx > 1 " << courx
                                      << std::endl;
                            std::cout << ">>> ... at cell i = " << i
                                      << " j = " << j << " k = " << k
                                      << std::endl;
                            std::cout << ">>> ... u = " << q_arr(i, j, k, QU)
                                      << " c = " << qaux_arr(i, j, k, QC)
                                      << std::endl;
                            std::cout
                                << ">>> ... density = " << q_arr(i, j, k, QRHO)
                                << std::endl;
                        }
                        ...
                        if (coury > 1.0_rt) {
                            ...
                            std::cout << "Warning:: CFL violation in "
                                         "check_for_cfl_violation"
                                      << std::endl;
                            std::cout << ">>> ... (v+c) * dt / dx > 1 " << coury
                                      << std::endl;
                            std::cout << ">>> ... at cell i = " << i
                                      << " j = " << j << " k = " << k
                                      << std::endl;
                            std::cout << ">>> ... v = " << q_arr(i, j, k, QV)
                                      << " c = " << qaux_arr(i, j, k, QC)
                                      << std::endl;
                            std::cout
                                << ">>> ... density = " << q_arr(i, j, k, QRHO)
                        ...
                        if (courz > 1.0_rt) {
                            ...
                            std::cout << "Warning:: CFL violation in "
                                         "check_for_cfl_violation"
                                      << std::endl;
                            std::cout << ">>> ... (w+c) * dt / dx > 1 " << courz
                                      << std::endl;
                            std::cout << ">>> ... at cell i = " << i
                                      << " j = " << j << " k = " << k
                                      << std::endl;
                            std::cout << ">>> ... w = " << q_arr(i, j, k, QW)
                                      << " c = " << qaux_arr(i, j, k, QC)
                                      << std::endl;
                            std::cout
                                << ">>> ... density = " << q_arr(i, j, k, QRHO)
    ...
    if (courno > 1.0) {
        amrex::Print() << "WARNING -- EFFECTIVE CFL AT LEVEL " << level
                       << " IS " << courno << std::endl
                       << std::endl;
