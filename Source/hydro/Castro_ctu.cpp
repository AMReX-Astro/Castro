...
void Castro::consup_hydro(const Box& bx, Array4<Real const> const& shk,
                          Array4<Real> const& update, Array4<Real> const& flux0,
...
#endif
                          Array4<Real const> const& vol, const Real dt) {
    ...
    amrex::ParallelFor(
        ...
        [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k, int n) noexcept {
            ...
            update(i, j, k, n) =
                update(i, j, k, n) + (flux0(i, j, k, n) * area0(i, j, k) -
                                      flux0(i + 1, j, k, n) * area0(i + 1, j, k)
#if AMREX_SPACEDIM >= 2
            ...
            if (n == UEINT) {
                ...
                Real pdu = (qx(i + 1, j, k, GDPRES) + qx(i, j, k, GDPRES)) *
                           (qx(i + 1, j, k, GDU) * area0(i + 1, j, k) -
                            qx(i, j, k, GDU) * area0(i, j, k));

#if AMREX_SPACEDIM >= 2
                pdu += (qy(i, j + 1, k, GDPRES) + qy(i, j, k, GDPRES)) *
                       (qy(i, j + 1, k, GDV) * area1(i, j + 1, k) -
                        qy(i, j, k, GDV) * area1(i, j, k));
#endif

#if AMREX_SPACEDIM == 3
                pdu += (qz(i, j, k + 1, GDPRES) + qz(i, j, k, GDPRES)) *
                       (qz(i, j, k + 1, GDW) * area2(i, j, k + 1) -
                        qz(i, j, k, GDW) * area2(i, j, k));
#endif
                ...
                if (!mom_flux_has_p(0, 0, coord)) {
                    update(i, j, k, UMX) +=
                        -(qx(i + 1, j, k, GDPRES) - qx(i, j, k, GDPRES)) /
                        dx[0];
                }
#endif
            }
...
void Castro::ctu_ppm_states(const Box& bx, const Box& vbx,
...
#endif
#if AMREX_SPACEDIM == 3
                            Array4<Real> const& qzm, Array4<Real> const& qzp,
#endif
#if AMREX_SPACEDIM < 3
    ...
    for (int idir = 0; idir < AMREX_SPACEDIM; idir++) {
        ...
        if (idir == 0) {
            trace_ppm(bx, idir, q_arr, qaux_arr, srcQ, flatn, qxm, qxp,
#if AMREX_SPACEDIM <= 2
                      dloga,
#endif
                      vbx, dt);
            ...
            trace_ppm(bx, idir, q_arr, qaux_arr, srcQ, flatn, qym, qyp,
#if AMREX_SPACEDIM <= 2
                      dloga,
#endif
                      vbx, dt);
            ...
            trace_ppm(bx, idir, q_arr, qaux_arr, srcQ, flatn, qzm, qzp, vbx,
                      dt);
...
#ifdef RADIATION
void Castro::ctu_ppm_rad_states(
    const Box& bx, const Box& vbx, Array4<Real const> const& q_arr,
    Array4<Real const> const& flatn, Array4<Real const> const& qaux_arr,
    Array4<Real const> const& srcQ, Array4<Real> const& qxm,
    ...
    for (int idir = 0; idir < AMREX_SPACEDIM; idir++) {
        ...
        if (idir == 0) {
            ...
            trace_ppm_rad(bx, idir, q_arr, qaux_arr, srcQ, flatn, qxm, qxp,
#if AMREX_SPACEDIM <= 2
                          dloga,
#endif
                          vbx, dt);
            ...
            trace_ppm_rad(bx, idir, q_arr, qaux_arr, srcQ, flatn, qym, qyp,
#if AMREX_SPACEDIM <= 2
                          dloga,
#endif
                          vbx, dt);
            ...
            trace_ppm_rad(bx, idir, q_arr, qaux_arr, srcQ, flatn, qzm, qzp, vbx,
                          dt);
...
void Castro::ctu_plm_states(const Box& bx, const Box& vbx,
                            Array4<Real const> const& q_arr,
                            Array4<Real const> const& flatn_arr,
                            Array4<Real const> const& qaux_arr,
                            Array4<Real const> const& srcQ,
                            Array4<Real> const& dq, Array4<Real> const& qxm,
                            Array4<Real> const& qxp,
#if AMREX_SPACEDIM >= 2
                            Array4<Real> const& qym, Array4<Real> const& qyp,
#endif
    ...
    for (int idir = 0; idir < AMREX_SPACEDIM; idir++) {
        ...
        for (int n = 0; n < NQ; n++) {
            ...
            uslope(bx, idir, q_arr, n, flatn_arr, dq);
        }

        if (use_pslope == 1) {
            pslope(bx, idir, q_arr, flatn_arr, dq, srcQ);
        }

        // compute the interface states

        if (idir == 0) {
            trace_plm(bx, 0, q_arr, qaux_arr, dq, qxm, qxp,
#if AMREX_SPACEDIM < 3
                      dloga,
#endif
                      srcQ, vbx, dt);
            ...
            trace_plm(bx, 1, q_arr, qaux_arr, dq, qym, qyp,
#if AMREX_SPACEDIM < 3
                      dloga,
#endif
                      srcQ, vbx, dt);
            ...
            trace_plm(bx, 2, q_arr, qaux_arr, dq, qzm, qzp, srcQ, vbx, dt);
#endif
        }
        ...
        if (idir == 0) {
            if (lo_bc_test) {
                ...
                amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(
...
void Castro::src_to_prim(const Box& bx, Array4<Real const> const& q_arr,
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        srcQ(i, j, k, QU) =
            (src(i, j, k, UMX) - q_arr(i, j, k, QU) * srcQ(i, j, k, QRHO)) *
            rhoinv;
        srcQ(i, j, k, QV) =
            (src(i, j, k, UMY) - q_arr(i, j, k, QV) * srcQ(i, j, k, QRHO)) *
            rhoinv;
        srcQ(i, j, k, QW) =
            (src(i, j, k, UMZ) - q_arr(i, j, k, QW) * srcQ(i, j, k, QRHO)) *
            rhoinv;
        srcQ(i, j, k, QREINT) = src(i, j, k, UEINT);
        srcQ(i, j, k, QPRES) =
            eos_state.dpde *
                (srcQ(i, j, k, QREINT) -
                 q_arr(i, j, k, QREINT) * srcQ(i, j, k, QRHO) * rhoinv) *
                rhoinv +
            eos_state.dpdr_e * srcQ(i, j, k, QRHO);
        ...
        for (int ipassive = 0; ipassive < npassive; ++ipassive) {
            ...
            srcQ(i, j, k, iq) =
                (src(i, j, k, n) - q_arr(i, j, k, iq) * srcQ(i, j, k, QRHO)) /
                q_arr(i, j, k, QRHO);
        }
#endif
    });
