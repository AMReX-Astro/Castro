...
void Castro::uslope(const Box& bx, const int idir,
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
                if (lplm_limiter == 1) {
                    ...
                    dq(i, j, k, n) = flatn_arr(i, j, k) * dsgn *
                                     amrex::min(dlim, std::abs(dcen));
                    ...
                    Real dq1 = (4.0_rt / 3.0_rt) * dcen -
                               (1.0_rt / 6.0_rt) * (dfp1 + dfm1);
                    dq(i, j, k, n) = flatn_arr(i, j, k) * dsgn *
                                     amrex::min(dlim, std::abs(dq1));
                }
            });
    }
}

void Castro::pslope(const Box& bx, const int idir,
                    Array4<Real const> const& q_arr,
                    Array4<Real const> const& flatn_arr, Array4<Real> const& dq,
    ...
    if (plm_iorder == 1) {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
            ...
            Real dsgn = std::copysign(1.0_rt, dcen);
            Real dlim =
                dlftp1 * drgtp1 >= 0.0_rt
                    ? 2.0_rt * amrex::min(std::abs(dlftp1), std::abs(drgtp1))
                    : 0.0_rt;
            Real dfp1 = dsgn * amrex::min(dlim, std::abs(dcen));
            ...
            dsgn = std::copysign(1.0_rt, dcen);
            dlim = dlftm1 * drgtm1 >= 0.0_rt
                       ? 2.0_rt * amrex::min(std::abs(dlftm1), std::abs(drgtm1))
                       : 0.0_rt;
            ...
            dsgn = std::copysign(1.0_rt, dcen);
            dlim = dlft * drgt >= 0.0_rt
                       ? 2.0_rt * amrex::min(std::abs(dlft), std::abs(drgt))
                       : 0.0_rt;

            Real dp1 =
                (4.0_rt / 3.0_rt) * dcen - (1.0_rt / 6.0_rt) * (dfp1 + dfm1);
            dq(i, j, k, QPRES) =
                flatn_arr(i, j, k) * dsgn * amrex::min(dlim, std::abs(dp1));

            if (idir == 0) {
                dq(i, j, k, QPRES) +=
                    q_arr(i, j, k, QRHO) * src(i, j, k, QU) * dx[0];
            } else if (idir == 1) {
                dq(i, j, k, QPRES) +=
                    q_arr(i, j, k, QRHO) * src(i, j, k, QV) * dx[1];
            } else {
                dq(i, j, k, QPRES) +=
                    q_arr(i, j, k, QRHO) * src(i, j, k, QW) * dx[2];
            }
        });
    }
}
