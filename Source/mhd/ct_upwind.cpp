...
void Castro::corner_couple(const Box& bx, Array4<Real> const& qr_out,
                           ...
                           Array4<Real const> const& Ed3, const int d1,
                           const int d2, const int d3, const Real dt) {
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        utmp[UMAGD1] =
            ur(i, j, k, UMAGD1) -
            sgn * cdtdx * (Ed3(i + b[0], j + b[1], k + b[2]) - Ed3(i, j, k));
        ...
        utmp[UMAGD3] =
            ur(i, j, k, UMAGD3) +
            sgn * 0.5_rt * cdtdx *
                ((Ed1(i + err[0], j + err[1], k + err[2]) -
                  Ed1(i + elr[0], j + elr[1], k + elr[2])) +
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        utmp[UMAGD3] = ul(i, j, k, UMAGD3) +
                       sgn * 0.5_rt * cdtdx *
                           ((Ed1(i + err[0], j + err[1], k + err[2]) -
                             Ed1(i + elr[0], j + elr[1], k + elr[2])) +
                            (Ed1(i + erl[0], j + erl[1], k + erl[2]) -
                             Ed1(i + ell[0], j + ell[1], k + ell[2])));
...
void Castro::half_step(const Box& bx, Array4<Real> const& qr_out,
                       Array4<Real> const& ql_out, Array4<Real const> const& ur,
                       Array4<Real const> const& ul,
                       Array4<Real const> const& flxd1,
                       Array4<Real const> const& flxd2,
                       Array4<Real const> const& Ed,
                       Array4<Real const> const& Ed1,
                       Array4<Real const> const& Ed2, const int d, const int d1,
                       const int d2, const Real dt) {
    ...
    c1r[d1] =
        1;  // add +1 to the d1 direction in the first flxd1 term of the subtraction
    c2r[d2] =
        1;  // add +1 to the d2 direction in the first flxd2 term of the subtraction
    ...
    amrex::ParallelFor(
        bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
            ...
            utmp[UMAGD1] = ur(i, j, k, UMAGD1) +
                           sgn * hdtdx *
                               ((Ed(i + err[0], j + err[1], k + err[2]) -
                                 Ed(i + erl[0], j + erl[1], k + erl[2])) +
                                (Ed(i + elr[0], j + elr[1], k + elr[2]) -
                                 Ed(i + ell[0], j + ell[1], k + ell[2])) -
                                (Ed2(i + e2rr[0], j + e2rr[1], k + e2rr[2]) -
                                 Ed2(i + e2lr[0], j + e2lr[1], k + e2lr[2])) -
                                (Ed2(i + e2rl[0], j + e2rl[1], k + e2rl[2]) -
                                 Ed2(i + ell[0], j + ell[1], k + ell[2])));
            ...
            utmp[UMAGD2] = ur(i, j, k, UMAGD2) -
                           sgn * hdtdx *
                               ((Ed(i + err[0], j + err[1], k + err[2]) -
                                ...
                                (Ed(i + erl[0], j + erl[1], k + erl[2]) -
                                 Ed(i + ell[0], j + ell[1], k + ell[2])) -
                                (Ed1(i + e1rr[0], j + e1rr[1], k + e1rr[2]) -
                                 Ed1(i + e1lr[0], j + e1lr[1], k + e1lr[2])) -
                                (Ed1(i + e1rl[0], j + e1rl[1], k + e1rl[2]) -
                                 Ed1(i + ell[0], j + ell[1], k + ell[2])));
