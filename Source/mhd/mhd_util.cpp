...
void Castro::check_for_mhd_cfl_violation(const Box& bx, const Real dt,
            ...
            if (verbose == 1) {
                ...
                if (courx > 1.0_rt) {
                    ...
                    std::cout << "Warning:: CFL violation in "
                                 "check_for_mhd_cfl_violation"
                              << std::endl;
                    std::cout << ">>> ... (u+c) * dt / dx > 1 " << courx
                              << std::endl;
                    std::cout << ">>> ... at cell i = " << i << " j = " << j
                              << " k = " << k << std::endl;
                    std::cout << ">>> ... u = " << q_arr(i, j, k, QU)
                              << " c = " << cx << std::endl;
                    std::cout << ">>> ... B = " << q_arr(i, j, k, QMAGX) << " "
                              << q_arr(i, j, k, QMAGY) << " "
                              << q_arr(i, j, k, QMAGZ) << std::endl;
                    std::cout << ">>> ... density = " << q_arr(i, j, k, QRHO)
                              << std::endl;
                    std::cout
                        << ">>> ... internal e = " << q_arr(i, j, k, QREINT)
                        << std::endl;
                    std::cout << ">>> ... pressure = " << q_arr(i, j, k, QPRES)
                              << std::endl;
                }
...
void Castro::consup_mhd(const Box& bx, Array4<Real> const& update,
...
void Castro::PrimToCons(const Box& bx, Array4<Real const> const& q_arr,
    ...
    amrex::ParallelFor(
        bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
            ...
            u_arr(i, j, k, UEDEN) =
                eos_state.rho * eos_state.e +
                +0.5_rt * q_arr(i, j, k, QRHO) *
            ...
            for (int n = 0; n < NumSpec; n++) {
                u_arr(i, j, k, UFS + n) =
                    q_arr(i, j, k, QRHO) * q_arr(i, j, k, QFS + n);
            }
        });
}

void Castro::prim_half(const Box& bx, Array4<Real> const& q2D,
                       Array4<Real const> const& q_arr,
                       Array4<Real const> const& flxx,
                       Array4<Real const> const& flxy,
                       Array4<Real const> const& flxz, const Real dt) {
