...
void Castro::plm(const Box& bx, const int idir, Array4<Real const> const& s,
                 Array4<Real const> const& qaux,
                 Array4<Real const> const& flatn, Array4<Real const> const& Bx,
                 Array4<Real const> const& By, Array4<Real const> const& Bz,
                 Array4<Real> const& qleft, Array4<Real> const& qright,
                 Array4<Real const> const& srcQ, const Real dt) {
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        smhd[IEIGN_P] = q_zone(QMAGX) * q_zone(QU) +
                        q_zone(QMAGY) * q_zone(QV) + q_zone(QMAGZ) * q_zone(QW);
        ...
        if (idir == 0) {
            qleft(i + 1, j, k, QRHO) = amrex::max(
                small_dens, q_zone(QRHO) + 0.5_rt * summ_p[IEIGN_RHO] +
                                0.5_rt * dt * smhd[IEIGN_RHO]);
            qleft(i + 1, j, k, QU) = q_zone(QU) + 0.5_rt * summ_p[IEIGN_U] +
                                     0.5_rt * dt * smhd[IEIGN_U];
            qleft(i + 1, j, k, QV) = q_zone(QV) + 0.5_rt * summ_p[IEIGN_V] +
                                     0.5_rt * dt * smhd[IEIGN_V];
            qleft(i + 1, j, k, QW) = q_zone(QW) + 0.5_rt * summ_p[IEIGN_W] +
                                     0.5_rt * dt * smhd[IEIGN_W];
            qleft(i + 1, j, k, QPRES) = amrex::max(
                small_pres, q_zone(QPRES) + 0.5_rt * summ_p[IEIGN_P] +
                                0.5_rt * dt * smhd[IEIGN_P]);

            qleft(i + 1, j, k, QMAGX) = Bx(i + 1, j, k);  // Bx stuff
            qleft(i + 1, j, k, QMAGY) = q_zone(QMAGY) +
                                        0.5_rt * summ_p[IEIGN_BT] +
                                        0.5_rt * dt * smhd[IEIGN_BT];
            qleft(i + 1, j, k, QMAGZ) = q_zone(QMAGZ) +
                                        0.5_rt * summ_p[IEIGN_BTT] +
                                        0.5_rt * dt * smhd[IEIGN_BTT];

        } else if (idir == 1) {
            qleft(i, j + 1, k, QRHO) = amrex::max(
                small_dens, q_zone(QRHO) + 0.5_rt * summ_p[IEIGN_RHO] +
                                0.5_rt * dt * smhd[IEIGN_RHO]);
            qleft(i, j + 1, k, QU) = q_zone(QU) + 0.5_rt * summ_p[IEIGN_U] +
                                     0.5_rt * dt * smhd[IEIGN_U];
            qleft(i, j + 1, k, QV) = q_zone(QV) + 0.5_rt * summ_p[IEIGN_V] +
                                     0.5_rt * dt * smhd[IEIGN_V];
            qleft(i, j + 1, k, QW) = q_zone(QW) + 0.5_rt * summ_p[IEIGN_W] +
                                     0.5_rt * dt * smhd[IEIGN_W];
            qleft(i, j + 1, k, QPRES) = amrex::max(
                small_pres, q_zone(QPRES) + 0.5_rt * summ_p[IEIGN_P] +
                                0.5_rt * dt * smhd[IEIGN_P]);

            qleft(i, j + 1, k, QMAGX) = q_zone(QMAGX) +
                                        0.5_rt * summ_p[IEIGN_BT] +
                                        0.5_rt * dt * smhd[IEIGN_BT];
            qleft(i, j + 1, k, QMAGY) = By(i, j + 1, k);  // By stuff
            qleft(i, j + 1, k, QMAGZ) = q_zone(QMAGZ) +
                                        0.5_rt * summ_p[IEIGN_BTT] +
                                        0.5_rt * dt * smhd[IEIGN_BTT];

        } else {
            qleft(i, j, k + 1, QRHO) = amrex::max(
                small_dens, q_zone(QRHO) + 0.5_rt * summ_p[IEIGN_RHO] +
                                0.5_rt * dt * smhd[IEIGN_RHO]);
            qleft(i, j, k + 1, QU) = q_zone(QU) + 0.5_rt * summ_p[IEIGN_U] +
                                     0.5_rt * dt * smhd[IEIGN_U];
            qleft(i, j, k + 1, QV) = q_zone(QV) + 0.5_rt * summ_p[IEIGN_V] +
                                     0.5_rt * dt * smhd[IEIGN_V];
            qleft(i, j, k + 1, QW) = q_zone(QW) + 0.5_rt * summ_p[IEIGN_W] +
                                     0.5_rt * dt * smhd[IEIGN_W];
            qleft(i, j, k + 1, QPRES) = amrex::max(
                small_pres, q_zone(QPRES) + 0.5_rt * summ_p[IEIGN_P] +
                                0.5_rt * dt * smhd[IEIGN_P]);

            qleft(i, j, k + 1, QMAGX) = q_zone(QMAGX) +
                                        0.5_rt * summ_p[IEIGN_BT] +
                                        0.5_rt * dt * smhd[IEIGN_BT];
            qleft(i, j, k + 1, QMAGY) = q_zone(QMAGY) +
                                        0.5_rt * summ_p[IEIGN_BTT] +
                                        0.5_rt * dt * smhd[IEIGN_BTT];
        ...
        qright(i, j, k, QRHO) =
            amrex::max(small_dens, q_zone(QRHO) + 0.5_rt * summ_m[IEIGN_RHO] +
                                       0.5_rt * dt * smhd[IEIGN_RHO]);
        qright(i, j, k, QU) =
            q_zone(QU) + 0.5_rt * summ_m[IEIGN_U] + 0.5_rt * dt * smhd[IEIGN_U];
        qright(i, j, k, QV) =
            q_zone(QV) + 0.5_rt * summ_m[IEIGN_V] + 0.5_rt * dt * smhd[IEIGN_V];
        qright(i, j, k, QW) =
            q_zone(QW) + 0.5_rt * summ_m[IEIGN_W] + 0.5_rt * dt * smhd[IEIGN_W];
        qright(i, j, k, QPRES) =
            amrex::max(small_pres, q_zone(QPRES) + 0.5_rt * summ_m[IEIGN_P] +
                                       0.5_rt * dt * smhd[IEIGN_P]);

        if (idir == 0) {
            qright(i, j, k, QMAGX) = Bx(i, j, k);  // Bx stuff
            qright(i, j, k, QMAGY) = q_zone(QMAGY) + 0.5_rt * summ_m[IEIGN_BT] +
                                     0.5_rt * dt * smhd[IEIGN_BT];
            qright(i, j, k, QMAGZ) = q_zone(QMAGZ) +
                                     0.5_rt * summ_m[IEIGN_BTT] +
                                     0.5_rt * dt * smhd[IEIGN_BTT];

        } else if (idir == 1) {
            qright(i, j, k, QMAGX) = q_zone(QMAGX) + 0.5_rt * summ_m[IEIGN_BT] +
                                     0.5_rt * dt * smhd[IEIGN_BT];
            ...
            qright(i, j, k, QMAGX) = q_zone(QMAGX) + 0.5_rt * summ_m[IEIGN_BT] +
                                     0.5_rt * dt * smhd[IEIGN_BT];
            qright(i, j, k, QMAGY) = q_zone(QMAGY) +
                                     0.5_rt * summ_m[IEIGN_BTT] +
                                     0.5_rt * dt * smhd[IEIGN_BTT];
        ...
        for (int n = 0; n < NumSpec; n++) {
            ...
            if (idir == 0) {
                qleft(i + 1, j, k, QFS + n) =
                    q_zone(QFS + n) + 0.5_rt * (1.0_rt - dtdx * un) * dW;
            } else if (idir == 1) {
                qleft(i, j + 1, k, QFS + n) =
                    q_zone(QFS + n) + 0.5_rt * (1.0_rt - dtdx * un) * dW;
            } else {
        ...
        if (idir == 0) {
            qleft(i + 1, j, k, QRHO) =
                amrex::max(small_dens, qleft(i + 1, j, k, QRHO) +
                                           0.5_rt * dt * srcQ(i, j, k, QRHO));
            qleft(i + 1, j, k, QU) =
                qleft(i + 1, j, k, QU) + 0.5_rt * dt * srcQ(i, j, k, QU);
            qleft(i + 1, j, k, QV) =
                qleft(i + 1, j, k, QV) + 0.5_rt * dt * srcQ(i, j, k, QV);
            qleft(i + 1, j, k, QW) =
                qleft(i + 1, j, k, QW) + 0.5_rt * dt * srcQ(i, j, k, QW);
            qleft(i + 1, j, k, QPRES) =
                qleft(i + 1, j, k, QPRES) + 0.5_rt * dt * srcQ(i, j, k, QPRES);
            qleft(i + 1, j, k, QREINT) = qleft(i + 1, j, k, QREINT) +
                                         0.5_rt * dt * srcQ(i, j, k, QREINT);

        } else if (idir == 1) {
            qleft(i, j + 1, k, QRHO) =
                amrex::max(small_dens, qleft(i, j + 1, k, QRHO) +
                                           0.5_rt * dt * srcQ(i, j, k, QRHO));
            qleft(i, j + 1, k, QU) =
                qleft(i, j + 1, k, QU) + 0.5_rt * dt * srcQ(i, j, k, QU);
            qleft(i, j + 1, k, QV) =
                qleft(i, j + 1, k, QV) + 0.5_rt * dt * srcQ(i, j, k, QV);
            qleft(i, j + 1, k, QW) =
                qleft(i, j + 1, k, QW) + 0.5_rt * dt * srcQ(i, j, k, QW);
            qleft(i, j + 1, k, QPRES) =
                qleft(i, j + 1, k, QPRES) + 0.5_rt * dt * srcQ(i, j, k, QPRES);
            qleft(i, j + 1, k, QREINT) = qleft(i, j + 1, k, QREINT) +
                                         0.5_rt * dt * srcQ(i, j, k, QREINT);

        } else {
            qleft(i, j, k + 1, QRHO) =
                max(small_dens, qleft(i, j, k + 1, QRHO) +
                                    0.5_rt * dt * srcQ(i, j, k, QRHO));
            qleft(i, j, k + 1, QU) =
                qleft(i, j, k + 1, QU) + 0.5_rt * dt * srcQ(i, j, k, QU);
            qleft(i, j, k + 1, QV) =
                qleft(i, j, k + 1, QV) + 0.5_rt * dt * srcQ(i, j, k, QV);
            qleft(i, j, k + 1, QW) =
                qleft(i, j, k + 1, QW) + 0.5_rt * dt * srcQ(i, j, k, QW);
            qleft(i, j, k + 1, QPRES) =
                qleft(i, j, k + 1, QPRES) + 0.5_rt * dt * srcQ(i, j, k, QPRES);
            qleft(i, j, k + 1, QREINT) = qleft(i, j, k + 1, QREINT) +
                                         0.5_rt * dt * srcQ(i, j, k, QREINT);
        }

        qright(i, j, k, QRHO) =
            max(small_dens,
                qright(i, j, k, QRHO) + 0.5_rt * dt * srcQ(i, j, k, QRHO));
        qright(i, j, k, QU) =
            qright(i, j, k, QU) + 0.5_rt * dt * srcQ(i, j, k, QU);
        qright(i, j, k, QV) =
            qright(i, j, k, QV) + 0.5_rt * dt * srcQ(i, j, k, QV);
        qright(i, j, k, QW) =
            qright(i, j, k, QW) + 0.5_rt * dt * srcQ(i, j, k, QW);
        qright(i, j, k, QPRES) =
            qright(i, j, k, QPRES) + 0.5_rt * dt * srcQ(i, j, k, QPRES);
        qright(i, j, k, QREINT) =
            qright(i, j, k, QREINT) + 0.5_rt * dt * srcQ(i, j, k, QREINT);
    });
}
