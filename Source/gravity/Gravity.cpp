...
#pragma gpu
		ca_test_residual(AMREX_INT_ANYD(bx.loVect()),  AMREX_INT_ANYD(bx.hiVect()),
		                 ...
		                 BL_TO_FORTRAN_ANYD((*grad_phi_prev[level][0])[mfi]),
#if AMREX_SPACEDIM >=2
...
#pragma gpu
		ca_test_residual(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
		                 BL_TO_FORTRAN_ANYD(Rhs[mfi]),
		                 BL_TO_FORTRAN_ANYD((*grad_phi_curr[level][0])[mfi]),
#if AMREX_SPACEDIM >=2
...
#pragma omp parallel
#endif
	{
	    ...
	    {
	        const Box& bx = mfi.tilebox();
            // don't offload due to dataPtrs to FArrayBoxes, potential race condition with
            // these dataPtrs as fortran function is summing over them
            // Maybe it would work with AMREX_MFITER_REDUCE_SUM?
