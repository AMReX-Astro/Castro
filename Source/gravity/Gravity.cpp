...
#include "gravity_queries.H"
        ...
        if ((gravity::gravity_type != "ConstantGrav") &&
            ...
            amrex::Abort(
                "Options are ConstantGrav, PoissonGrav, MonopoleGrav, or "
                "PrescribedGrav");
        }
        ...
        if (gravity::gravity_type == "ConstantGrav") {
            if (dgeom.IsSPHERICAL())
                amrex::Abort(
                    "Can't use constant direction gravity with non-Cartesian "
                    "coordinates");
        }
        ...
        if (gravity::gravity_type == "PoissonGrav") {
            amrex::Abort(
                " gravity::gravity_type = PoissonGrav doesn't work well in 1-d "
                "-- please set gravity::gravity_type = MonopoleGrav");
        } else if (gravity::gravity_type == "MonopoleGrav" &&
                   !(dgeom.IsSPHERICAL())) {
            amrex::Abort("Only use MonopoleGrav in 1D spherical coordinates");
        } else if (gravity::gravity_type == "ConstantGrav" &&
                   dgeom.IsSPHERICAL()) {
            amrex::Abort(
                "Can't use constant gravity in 1D spherical coordinates");
        }

#elif (BL_SPACEDIM == 2)
        if (gravity::gravity_type == "MonopoleGrav" && dgeom.IsCartesian()) {
            amrex::Abort(
                " gravity::gravity_type = MonopoleGrav doesn't make sense in "
        ...
        if (pp.contains("get_g_from_phi") && !gravity::get_g_from_phi &&
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Warning: gravity::gravity_type = PoissonGrav "
                             "assumes get_g_from_phi is true"
                          << std::endl;
        ...
        if (n_abs_tol <= 1) {
            ...
            for (int lev = 1; lev < nlevs; ++lev)
                abs_tol[lev] = abs_tol[lev - 1] *
                               std::pow(parent->refRatio(lev - 1)[0], 2);
            ...
            amrex::Abort(
                "If you are providing multiple values for abs_tol, you must "
                "provide at least one value for every level up to "
                "amr.max_level.");
        }
        ...
        if (pp.contains("ml_tol")) {
            ...
            amrex::Warning(
                "The gravity parameter ml_tol has been renamed rel_tol. ml_tol "
                "is now deprecated.");

            if (!pp.contains("rel_tol")) rel_tol_name = "ml_tol";
        }
        ...
        if (n_rel_tol <= 1) {
            ...
            for (int lev = 0; lev < MAX_LEV; ++lev) rel_tol[lev] = tol;
            ...
            amrex::Abort(
                "If you are providing multiple values for rel_tol, you must "
                "provide at least one value for every level up to "
                "amr.max_level.");
        }
        ...
        if (pp.contains("delta_tol"))
            amrex::Warning(
                "The gravity parameter delta_tol is no longer used.");
        ...
        if (gravity::verbose > 1 && ParallelDescriptor::IOProcessor()) {
            std::cout << "Getting Gconst from constants: " << C::Gconst
                      << std::endl;
...
#endif
        if (gravity::verbose > 1 && ParallelDescriptor::IOProcessor())
            std::cout << "Maximum radius for which the mass is contained in "
                         "the domain: "
...
void Gravity::solve_for_phi(int level, MultiFab& phi,
    ...
    if (gravity::verbose) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Gravity::solve_for_phi() time = " << end
                          << std::endl
                          << std::endl;
#ifdef BL_LAZY
        });
...
void Gravity::gravity_sync(int crse_level, int fine_level,
    ...
    for (int lev = crse_level; lev <= fine_level; ++lev) {
        rhs[lev - crse_level].reset(
            new MultiFab(LevelData[lev]->boxArray(),
                         LevelData[lev]->DistributionMap(), 1, 0));
        MultiFab::Copy(*rhs[lev - crse_level], *dphi[lev - crse_level], 0, 0, 1,
                       0);
        rhs[lev - crse_level]->mult(1.0 / Ggravity);
        MultiFab::Add(*rhs[lev - crse_level], *drho[lev - crse_level], 0, 0, 1,
                      0);
    }
    ...
    if (crse_level == 0 && !crse_geom.isAllPeriodic()) {
        ...
        if (gravity::direct_sum_bcs)
            fill_direct_sum_BCs(crse_level, fine_level,
                                amrex::GetVecOfPtrs(rhs),
                                *delta_phi[crse_level]);
        else {
            if (gravity::lnum >= 0) {
                fill_multipole_BCs(crse_level, fine_level,
                                   amrex::GetVecOfPtrs(rhs),
                                   *delta_phi[crse_level]);
            } else {
                int fill_interior = 0;
                make_radial_phi(crse_level, *rhs[0], *delta_phi[crse_level],
                                fill_interior);
            }
        }
#elif (BL_SPACEDIM == 2)
    ...
    if (crse_geom.isAllPeriodic() &&
        ...
        Real local_correction =
            delta_phi[0]->sum() / grids[crse_level].numPts();
    ...
    for (int lev = crse_level; lev <= fine_level; lev++) {
        ...
        for (int n = 0; n < BL_SPACEDIM; n++)
            grad_phi_curr[lev][n]->plus(*ec_gdPhi[lev - crse_level][n], 0, 1,
                                        0);

        get_new_grav_vector(
            lev, LevelData[lev]->get_new_data(Gravity_Type),
...
void Gravity::GetCrsePhi(int level, MultiFab& phi_crse, Real time) {
    ...
    MultiFab::LinComb(phi_crse, alpha, phi_new, 0, omalpha, phi_old, 0, 0, 1,
                      1);
...
void Gravity::actual_multilevel_solve(
    ...
    for (int ilev = 0; ilev < nlevels; ilev++) {
        ...
        if (!use_previous_phi_as_guess) phi_p[ilev]->setVal(0.);
    }

    const auto& rhs = get_rhs(crse_level, nlevels, is_new);

    if (!use_previous_phi_as_guess && crse_level == 0 &&
        !(parent->Geom(0).isAllPeriodic())) {
    ...
    if (fine_level >= crse_level) {
        ...
        Vector<MultiFab*> res_null;
        solve_phi_with_mlmg(crse_level, fine_level, phi_p,
                            amrex::GetVecOfPtrs(rhs), grad_phi_p, res_null,
                            time);
        ...
        for (int amr_lev = fine_level; amr_lev > crse_level; amr_lev--) {
            ...
            if (is_new == 1) {
                amrex::average_down(
                    ...
                    LevelData[amr_lev - 1]->get_new_data(PhiGrav_Type), 0, 1,
                    ratio);
            } else if (is_new == 0) {
                amrex::average_down(
                    LevelData[amr_lev]->get_old_data(PhiGrav_Type),
                    LevelData[amr_lev - 1]->get_old_data(PhiGrav_Type), 0, 1,
                    ratio);
            }
        }
    ...
    for (int amr_lev = gravity::max_solve_level + 1; amr_lev <= finest_level_in;
        ...
        if (is_new == 1) {
            ...
            LevelData[amr_lev]->FillCoarsePatch(phi, 0, time, PhiGrav_Type, 0,
                                                1, 1);
        ...
        for (int n = 0; n < BL_SPACEDIM; ++n) {
            amrex::InterpFromCoarseLevel(
                *grad_phi[amr_lev][n], time, *grad_phi[amr_lev - 1][n], 0, 0, 1,
                ...
                gp_phys_bc, 0, parent->refRatio(amr_lev - 1), gp_interp, gp_bcs,
                0);
        }
    }
}
...
void Gravity::get_old_grav_vector(int level, MultiFab& grav_vector, Real time) {
    ...
    if (level > gravity::max_solve_level) {
        ...
        LevelData[level]->FillCoarsePatch(grav_vector, 0, time, Gravity_Type, 0,
                                          3, ng);
    ...
    if (gravity::gravity_type == "ConstantGrav") {
        ...
        const Real prev_time =
            LevelData[level]->get_state_data(State_Type).prevTime();
...
void Gravity::get_new_grav_vector(int level, MultiFab& grav_vector, Real time) {
    ...
    if (gravity::gravity_type == "ConstantGrav") {
        ...
        const Real cur_time =
            LevelData[level]->get_state_data(State_Type).curTime();
...
void Gravity::test_level_grad_phi_prev(int level) {
    ...
    if (geom.isAllPeriodic()) {
        if (gravity::verbose > 1 && ParallelDescriptor::IOProcessor() &&
            mass_offset != 0.0)
            std::cout
                << " ... subtracting average density from RHS at level ... "
    ...
    if (gravity::verbose > 1) {
        ...
        if (ParallelDescriptor::IOProcessor()) {
            std::cout << "... test_level_grad_phi_prev at level " << level
                      << std::endl;
            std::cout << "       norm of RHS             " << rhsnorm
                      << std::endl;
        }
    }
...
#pragma omp parallel
    ...
    for (MFIter mfi(Rhs, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        ca_test_residual(bx.loVect(), bx.hiVect(), BL_TO_FORTRAN(Rhs[mfi]),
...
#endif
        if (ParallelDescriptor::IOProcessor())
            std::cout << "       norm of residual        " << resnorm
                      << std::endl;
...
void Gravity::create_comp_minus_level_grad_phi(
    ...
    if (gravity::verbose > 1 && ParallelDescriptor::IOProcessor()) {
        std::cout << "\n";
        std::cout << "... compute difference between level and composite "
                     "solves at level "
                  << level << "\n";
        std::cout << "\n";
    }
    ...
    comp_minus_level_phi.minus(
        parent->getLevel(level).get_old_data(PhiGrav_Type), 0, 1, 0);
...
void Gravity::average_fine_ec_onto_crse_ec(int level, int is_new) {
    ...
    for (int i = 0; i < crse_gphi_fine_BA.size(); ++i)
        crse_gphi_fine_BA.set(i,
                              amrex::coarsen(grids[level + 1][i], fine_ratio));
...
void Gravity::test_composite_phi(int crse_level) {
    ...
    for (int ilev = 0; ilev < nlevels; ++ilev) {
        ...
        MultiFab::Copy(*phi[ilev],
                       LevelData[amr_lev]->get_new_data(PhiGrav_Type), 0, 0, 1,
                       1);
    ...
    solve_phi_with_mlmg(crse_level, finest_level_local,
                        amrex::GetVecOfPtrs(phi), amrex::GetVecOfPtrs(rhs),
                        grad_phi_null, amrex::GetVecOfPtrs(res), time);
...
void Gravity::make_prescribed_grav(int level, Real time, MultiFab& grav_vector,
                                   MultiFab& phi) {
...
void Gravity::interpolate_monopole_grav(int level, RealVector& radial_grav,
...
#pragma omp parallel
    ...
    for (MFIter mfi(grav_vector, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        ca_put_radial_grav(bx.loVect(), bx.hiVect(), dx, &dr,
                           ...
                           radial_grav.dataPtr(), geom.ProbLo(), &n1d, &level);
    }
}

void Gravity::make_radial_phi(int level, const MultiFab& Rhs, MultiFab& phi,
                              int fill_interior) {
...
#pragma gpu box(bx)
            ca_compute_radial_mass(
                AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                AMREX_REAL_ANYD(dx), dr, BL_TO_FORTRAN_ANYD(Rhs[mfi]),
#ifdef _OPENMP
                priv_radial_mass[tid].dataPtr(), priv_radial_vol[tid].dataPtr(),
#else
                radial_mass.dataPtr(), radial_vol.dataPtr(),
#endif
                AMREX_REAL_ANYD(geom.ProbLo()), n1d, gravity::drdxfac, level);
        }
...
#pragma omp for
    ...
    ca_integrate_grav(radial_mass.dataPtr(), radial_den.dataPtr(),
                      radial_grav.dataPtr(), &max_radius_all_in_domain, &dr,
                      &n1d);
...
#pragma gpu box(bx)
        ca_put_radial_phi(
            ...
            radial_phi.dataPtr(), AMREX_REAL_ANYD(geom.ProbLo()), n1d,
            fill_interior);
    }
    ...
    if (gravity::verbose) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            ParallelDescriptor::ReduceRealMax(end, IOProc);
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Gravity::make_radial_phi() time = " << end
                          << std::endl
                          << std::endl;
#ifdef BL_LAZY
        });
...
void Gravity::fill_multipole_BCs(int crse_level, int fine_level,
    ...
    amrex::ParallelFor(boxq0, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
        qL0_arr(i, j, k) = 0.0;
        qU0_arr(i, j, k) = 0.0;
    });
    ...
    amrex::ParallelFor(boxqC, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
        qLC_arr(i, j, k) = 0.0;
        qUC_arr(i, j, k) = 0.0;
    });
    ...
    amrex::ParallelFor(boxqS, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
        qLS_arr(i, j, k) = 0.0;
        qUS_arr(i, j, k) = 0.0;
    });
...
#pragma gpu box(bx)
                ca_compute_multipole_moments(
                    AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                    AMREX_INT_ANYD(domain.loVect()),
                    AMREX_INT_ANYD(domain.hiVect()), AMREX_REAL_ANYD(dx),
                    BL_TO_FORTRAN_ANYD(source[mfi]),
                    BL_TO_FORTRAN_ANYD((*volume[lev])[mfi]), gravity::lnum,
#ifdef _OPENMP
                    priv_qL0[tid]->dataPtr(), priv_qLC[tid]->dataPtr(),
                    priv_qLS[tid]->dataPtr(), priv_qU0[tid]->dataPtr(),
                    priv_qUC[tid]->dataPtr(), priv_qUS[tid]->dataPtr(),
#else
                    qL0.dataPtr(), qLC.dataPtr(), qLS.dataPtr(), qU0.dataPtr(),
                    qUC.dataPtr(), qUS.dataPtr(),
#endif
                    npts, boundary_only);
            }
...
#pragma gpu box(bx)
        ca_put_multipole_phi(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            AMREX_INT_ANYD(domain.loVect()), AMREX_INT_ANYD(domain.hiVect()),
            AMREX_REAL_ANYD(dx), BL_TO_FORTRAN_ANYD(phi[mfi]), gravity::lnum,
            qL0.dataPtr(), qLC.dataPtr(), qLS.dataPtr(), qU0.dataPtr(),
            qUC.dataPtr(), qUS.dataPtr(), npts, boundary_only);
    }
    ...
    if (gravity::verbose) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            ParallelDescriptor::ReduceRealMax(end, IOProc);
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Gravity::fill_multipole_BCs() time = " << end
                          << std::endl
                          << std::endl;
#ifdef BL_LAZY
        });
...
#if (BL_SPACEDIM == 3)
void Gravity::fill_direct_sum_BCs(int crse_level, int fine_level,
                                  const Vector<MultiFab*>& Rhs, MultiFab& phi) {
    ...
    amrex::ParallelFor(boxXY, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
    ...
    amrex::ParallelFor(boxXZ, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
    ...
    amrex::ParallelFor(boxYZ, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
...
#pragma gpu box(bx)
                ca_compute_direct_sum_bc(
                    AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                    AMREX_REAL_ANYD(dx), symmetry_type,
                    AMREX_INT_ANYD(physbc_lo), AMREX_INT_ANYD(physbc_hi),
                    BL_TO_FORTRAN_ANYD(r), BL_TO_FORTRAN_ANYD(v),
                    AMREX_REAL_ANYD(crse_geom.ProbLo()),
                    AMREX_REAL_ANYD(crse_geom.ProbHi()),
#ifdef _OPENMP
...
#else
                    bcXYLo.dataPtr(), bcXYHi.dataPtr(), bcXZLo.dataPtr(),
                    bcXZHi.dataPtr(), bcYZLo.dataPtr(), bcYZHi.dataPtr(),
#endif
                    AMREX_INT_ANYD(bc_lo), AMREX_INT_ANYD(bc_hi),
                    AMREX_REAL_ANYD(bc_dx));
            }
...
#pragma gpu box(bx)
        ca_put_direct_sum_bc(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(p), bcXYLo.dataPtr(), bcXYHi.dataPtr(),
            bcXZLo.dataPtr(), bcXZHi.dataPtr(), bcYZLo.dataPtr(),
            bcYZHi.dataPtr(), AMREX_INT_ANYD(bc_lo), AMREX_INT_ANYD(bc_hi));
    }
    ...
    if (gravity::verbose) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            ParallelDescriptor::ReduceRealMax(end, IOProc);
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Gravity::fill_direct_sum_BCs() time = " << end
                          << std::endl
                          << std::endl;
#ifdef BL_LAZY
        });
...
#if (BL_SPACEDIM < 3)
void Gravity::applyMetricTerms(int level, MultiFab& Rhs,
                               const Vector<MultiFab*>& coeffs) {
...
#pragma omp parallel
    ...
    for (MFIter mfi(Rhs, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        apply_metric(bx, Rhs.array(mfi), (Rhs[mfi]).box(),
...
void Gravity::unweight_cc(int level, MultiFab& cc) {
...
void Gravity::unweight_edges(int level, const Vector<MultiFab*>& edges) {
...
#pragma omp parallel
    ...
    for (int idir = 0; idir < BL_SPACEDIM; ++idir) {
        for (MFIter mfi(*edges[idir], TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            do_unweight_edges(bx, (*edges[idir]).array(mfi), idir, dx,
                              coord_type);
        }
    }
}
        ...
        if (diff > eps && old_mass_offset > 0) {
            if (ParallelDescriptor::IOProcessor()) {
                std::cout << " ... new vs old mass_offset " << mass_offset
                          ...
                          << std::endl;
                std::cout << " ... Gravity::set_mass_offset -- total mass has "
                             "changed!"
                          << std::endl;
                ;
            }
...
void Gravity::add_pointmass_to_gravity(int level, MultiFab& phi,
                                       MultiFab& grav_vector, Real point_mass) {
...
void Gravity::make_radial_gravity(int level, Real time,
                                  RealVector& radial_grav) {
    ...
    for (int lev = 0; lev <= level; lev++) {
        const Real t_old =
            LevelData[lev]->get_state_data(State_Type).prevTime();
        ...
        if (eps == 0.0) {
            ...
            S_new.copy(LevelData[lev]->get_new_data(State_Type), 0, 0,
                       NUM_STATE);
            S_new.mult(alpha);

            S.plus(S_new, 0, NUM_STATE, 0);
        } else {
            std::cout << " Level / Time in make_radial_gravity is: " << lev
                      << " " << time << std::endl;
            std::cout << " but old / new time      are: " << t_old << " "
                      << t_new << std::endl;
...
#pragma gpu box(bx)
                ca_compute_radial_mass(
                    AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                    AMREX_REAL_ANYD(dx), dr, BL_TO_FORTRAN_ANYD(fab),
#ifdef _OPENMP
                    priv_radial_mass[tid].dataPtr(),
                    priv_radial_vol[tid].dataPtr(),
#else
                    radial_mass[lev].dataPtr(), radial_vol[lev].dataPtr(),
#endif
                    AMREX_REAL_ANYD(geom.ProbLo()), n1d, gravity::drdxfac, lev);

#ifdef GR_GRAV
                ca_compute_avgpres(
                    bx.loVect(), bx.hiVect(), dx, &dr, BL_TO_FORTRAN(fab),
#ifdef _OPENMP
...
#pragma omp for
    ...
    if (do_diag > 0 && ParallelDescriptor::IOProcessor())
        std::cout
            << "Gravity::make_radial_gravity: Sum of mass over all levels "
            << sum_over_levels << std::endl;
        ...
        for (int lev = level - 1; lev >= 0; lev--) {
            ...
            for (int i = 0; i < n1d / ratio; i++) {
                for (int n = 0; n < ratio; n++) {
                    radial_mass_summed[ratio * i + n] +=
                        1. / double(ratio) * radial_mass[lev][i];
                }
            }
        }
    }
    ...
    if (do_diag > 0 && ParallelDescriptor::IOProcessor()) {
        ...
        std::cout << "Gravity::make_radial_gravity: Sum of combined mass "
                  << sum_added << std::endl;
    }
    ...
    // First add the contribution from this level
    for (int i = 0; i < n1d; i++) radial_vol_summed[i] = radial_vol[level][i];
    ...
    if (level > 0) {
        ...
        for (int lev = level - 1; lev >= 0; lev--) {
            ...
            for (int i = 0; i < n1d / ratio; i++) {
                for (int n = 0; n < ratio; n++) {
                    radial_vol_summed[ratio * i + n] +=
                        1. / double(ratio) * radial_vol[lev][i];
                }
            }
        }
    }
    ...
    for (int i = 0; i < n1d; i++) {
        ...
        if (radial_vol_summed[i] > 0.)
            radial_den_summed[i] /= radial_vol_summed[i];
    }
    ...
    // First add the contribution from this level
    for (int i = 0; i < n1d; i++) radial_pres_summed[i] = radial_pres[level][i];
    ...
    if (level > 0) {
        ...
        for (int lev = level - 1; lev >= 0; lev--) {
            ...
            for (int i = 0; i < n1d / ratio; i++)
                for (int n = 0; n < ratio; n++)
                    radial_pres_summed[ratio * i + n] +=
                        1. / double(ratio) * radial_pres[lev][i];
        }
    }

    for (int i = 0; i < n1d; i++)
        if (radial_vol_summed[i] > 0.)
            radial_pres_summed[i] /= radial_vol_summed[i];

    // Integrate radially outward to define the gravity -- here we add the post-Newtonian correction
    ca_integrate_gr_grav(
    ...
    if (gravity::verbose) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            ParallelDescriptor::ReduceRealMax(end, IOProc);
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Gravity::make_radial_gravity() time = " << end
                          << std::endl
                          << std::endl;
#ifdef BL_LAZY
        });
    ...
    if (level > 0 && !geom.isAllPeriodic()) {
        ...
        for (int dir = 0; dir < BL_SPACEDIM; dir++) {
            if (!geom.isPeriodic(dir)) {
                if (phys_bc->lo(dir) != Symmetry) shrunk_domain.growLo(dir, -1);
                if (phys_bc->hi(dir) != Symmetry) shrunk_domain.growHi(dir, -1);
            }
        }
        if (!shrunk_domain.contains(grids[level].minimalBox()))
            amrex::Error(
                "Oops -- don't know how to set boundary conditions for grids "
                "at this level that touch the domain boundary!");
    }
}
...
void Gravity::update_max_rhs() {
    ...
    if (geom0.IsSPHERICAL() || geom0.IsRZ()) {
        ...
        for (int lev = 0; lev < nlevs; ++lev) {
            ...
            for (int i = 0; i < BL_SPACEDIM; i++) {
                coeffs[lev][i].reset(new MultiFab(
                    amrex::convert(grids[lev], IntVect::TheDimensionVector(i)),
                    dmap[lev], 1, 0));
