...
//
static int scalar_bc[] = {INT_DIR,      EXT_DIR,      FOEXTRAP,
                          REFLECT_EVEN, REFLECT_EVEN, REFLECT_EVEN};

static int norm_vel_bc[] = {INT_DIR,     EXT_DIR,     FOEXTRAP,
                            REFLECT_ODD, REFLECT_ODD, REFLECT_ODD};

static int tang_vel_bc[] = {INT_DIR,      EXT_DIR,      FOEXTRAP,
                            REFLECT_EVEN, REFLECT_EVEN, REFLECT_EVEN};

static int mag_field_bc[] = {INT_DIR,      EXT_DIR,  FOEXTRAP,
                             REFLECT_EVEN, FOEXTRAP, HOEXTRAP};
...
#ifdef MHD
static void set_mag_field_bc(BCRec& bc, const BCRec& phys_bc) {
    const int* lo_bc = phys_bc.lo();
    const int* hi_bc = phys_bc.hi();
    for (int i = 0; i < BL_SPACEDIM; i++) {
...
static void replace_inflow_bc(BCRec& bc) {
...
void Castro::variableSetUp() {
    ...
    if (use_retry) {
        amrex::Warning(
            "use_retry = 1 is not supported with true SDC.  Disabling");
        use_retry = 0;
    }
    if (!abort_on_failure) {
        amrex::Warning(
            "abort_on_failure = F not supported with true SDC.  Resetting");
        abort_on_failure = 1;
    ...
    if (!use_retry && !abort_on_failure) {
        amrex::Error(
            "use_retry = 0 and abort_on_failure = F is dangerous and not "
            "supported");
    }
#endif
#endif
    ...
    ParallelDescriptor::ReduceRealMax(run_stop,
    ...
    ca_set_problem_params(dm, phys_bc.lo(), phys_bc.hi(), Interior, Inflow,
    ...
    desc_lst.addDescriptor(Mag_Type_x, xface, StateDescriptor::Point, 0, 1,
                           interp, state_data_extrap, store_in_checkpoint);
    IndexType yface(IntVect{AMREX_D_DECL(0, 1, 0)});
    desc_lst.addDescriptor(Mag_Type_y, yface, StateDescriptor::Point, 0, 1,
                           interp, state_data_extrap, store_in_checkpoint);
    IndexType zface(IntVect{AMREX_D_DECL(0, 0, 1)});
    desc_lst.addDescriptor(Mag_Type_z, zface, StateDescriptor::Point, 0, 1,
                           interp, state_data_extrap, store_in_checkpoint);
#endif
    ...
    desc_lst.addDescriptor(
        Gravity_Type, IndexType::TheCellType(), StateDescriptor::Point,
        NUM_GROW, 3, &cell_cons_interp, state_data_extrap, store_in_checkpoint);
#endif
    ...
    store_in_checkpoint = false;
    desc_lst.addDescriptor(
        Rotation_Type, IndexType::TheCellType(), StateDescriptor::Point,
        NUM_GROW, 3, &cell_cons_interp, state_data_extrap, store_in_checkpoint);
#endif
    ...
    if (time_integration_method == SimplifiedSpectralDeferredCorrections) {
        ...
        store_in_checkpoint = true;
        desc_lst.addDescriptor(Simplified_SDC_React_Type,
                               IndexType::TheCellType(), StateDescriptor::Point,
                               NUM_GROW, NQSRC, &cell_cons_interp,
                               state_data_extrap, store_in_checkpoint);
    }
#endif
#endif
    ...
    desc_lst.setComponent(State_Type, URHO, name, bcs, stateBndryFunc);
    ...
    desc_lst.setComponent(Source_Type, URHO, state_type_source_names,
                          source_bcs, genericBndryFunc);
    ...
    desc_lst.setComponent(Reactions_Type, NumSpec + 1, "rho_enuc", bc,
                          genericBndryFunc);
    desc_lst.setComponent(Reactions_Type, NumSpec + 2, "weights", bc,
                          genericBndryFunc);
#endif
    ...
    if (time_integration_method == SimplifiedSpectralDeferredCorrections) {
        for (int i = 0; i < NQSRC; ++i) {
            ...
            desc_lst.setComponent(Simplified_SDC_React_Type, i,
                                  std::string(buf), bc, genericBndryFunc);
        }
    }
#endif
#endif
    ...
    desc_lst.addDescriptor(Rad_Type, IndexType::TheCellType(),
                           StateDescriptor::Point, ngrow, ncomp, interp);
    set_scalar_bc(bc, phys_bc);
    replace_inflow_bc(bc);
    ...
    if (!Radiation::do_multigroup) {
        desc_lst.setComponent(Rad_Type, Rad, "rad", bc, genericBndryFunc);
    } else {
        for (int i = 0; i < Radiation::nGroups; i++) {
            sprintf(rad_name, "rad%d", i);
            desc_lst.setComponent(Rad_Type, i, rad_name, bc, genericBndryFunc);
        }
    }
#endif
    ...
    if (time_integration_method == SpectralDeferredCorrections &&
        ...
        desc_lst.addDescriptor(SDC_Source_Type, IndexType::TheCellType(),
                               StateDescriptor::Point, 2, NUM_STATE, interp,
                               state_data_extrap, store_in_checkpoint);
        ...
        desc_lst.setComponent(SDC_Source_Type, URHO, sdc_source_names,
                              sdc_source_bcs, genericBndryFunc);
    }
#endif
    ...
    // Pressure
    //
    derive_lst.add("pressure", IndexType::TheCellType(), 1, ca_derpres,
                   the_same_box);
    ...
    // Kinetic energy
    //
    derive_lst.add("kineng", IndexType::TheCellType(), 1, ca_derkineng,
                   the_same_box);
    ...
    // Sound speed (c)
    //
    derive_lst.add("soundspeed", IndexType::TheCellType(), 1, ca_dersoundspeed,
                   the_same_box);
    derive_lst.addComponent("soundspeed", desc_lst, State_Type, URHO,
                            NUM_STATE);

    //
    // Gamma_1
    //
    derive_lst.add("Gamma_1", IndexType::TheCellType(), 1, ca_dergamma1,
                   the_same_box);
    ...
    // Mach number(M)
    //
    derive_lst.add("MachNumber", IndexType::TheCellType(), 1, ca_dermachnumber,
                   the_same_box);
    derive_lst.addComponent("MachNumber", desc_lst, State_Type, URHO,
                            NUM_STATE);
    ...
    // Wave speed u+c
    //
    derive_lst.add("uplusc", IndexType::TheCellType(), 1, ca_deruplusc,
                   the_same_box);
    ...
    // Wave speed u-c
    //
    derive_lst.add("uminusc", IndexType::TheCellType(), 1, ca_deruminusc,
                   the_same_box);
    ...
    // Entropy (S)
    //
    derive_lst.add("entropy", IndexType::TheCellType(), 1, ca_derentropy,
                   the_same_box);
    ...
    if (diffuse_temp) {
        ...
        //
        derive_lst.add("thermal_cond", IndexType::TheCellType(), 1, ca_dercond,
                       the_same_box);
        derive_lst.addComponent("thermal_cond", desc_lst, State_Type, URHO,
                                NUM_STATE);

        //
        // thermal diffusivity (k_th/(rho c_v))
        //
        derive_lst.add("diff_coeff", IndexType::TheCellType(), 1,
                       ca_derdiffcoeff, the_same_box);
        derive_lst.addComponent("diff_coeff", desc_lst, State_Type, URHO,
                                NUM_STATE);

        //
        // diffusion term (the divergence of thermal flux)
        //
        derive_lst.add("diff_term", IndexType::TheCellType(), 1, ca_derdiffterm,
                       grow_box_by_one);
        derive_lst.addComponent("diff_term", desc_lst, State_Type, URHO,
                                NUM_STATE);
    }
#endif
    ...
    // Vorticity
    //
    derive_lst.add("magvort", IndexType::TheCellType(), 1, ca_dermagvort,
                   grow_box_by_one);
    ...
    // Div(u)
    //
    derive_lst.add("divu", IndexType::TheCellType(), 1, ca_derdivu,
                   grow_box_by_one);
    ...
    //
    derive_lst.add("eint_E", IndexType::TheCellType(), 1, ca_dereint1,
                   the_same_box);
    ...
    // Internal energy as derived from rho*e, part of the state
    //
    derive_lst.add("eint_e", IndexType::TheCellType(), 1, ca_dereint2,
                   the_same_box);
    derive_lst.addComponent("eint_e", desc_lst, State_Type, URHO, NUM_STATE);
    ...
    // Log(density)
    //
    derive_lst.add("logden", IndexType::TheCellType(), 1, ca_derlogden,
                   the_same_box);
    derive_lst.addComponent("logden", desc_lst, State_Type, URHO, NUM_STATE);

    derive_lst.add("StateErr", IndexType::TheCellType(), 3, ca_derstate,
                   grow_box_by_one);
    ...
    for (int i = 0; i < NumSpec; i++) {
        ...
        derive_lst.add(spec_string, IndexType::TheCellType(), 1, ca_derspec,
                       the_same_box);
    ...
    // Abar
    //
    derive_lst.add("abar", IndexType::TheCellType(), 1, ca_derabar,
                   the_same_box);
    ...
    // Velocities
    //
    derive_lst.add("x_velocity", IndexType::TheCellType(), 1, ca_dervel,
                   the_same_box);
    ...
    derive_lst.add("y_velocity", IndexType::TheCellType(), 1, ca_dervel,
                   the_same_box);
    ...
    derive_lst.add("z_velocity", IndexType::TheCellType(), 1, ca_dervel,
                   the_same_box);
    ...
    //
    derive_lst.add("t_sound_t_enuc", IndexType::TheCellType(), 1,
                   ca_derenuctimescale, the_same_box);
    derive_lst.addComponent("t_sound_t_enuc", desc_lst, State_Type, URHO,
                            NUM_STATE);
    derive_lst.addComponent("t_sound_t_enuc", desc_lst, Reactions_Type, NumSpec,
                            1);
#endif

    derive_lst.add("magvel", IndexType::TheCellType(), 1, ca_dermagvel,
                   the_same_box);
    derive_lst.addComponent("magvel", desc_lst, State_Type, URHO, 1);
    derive_lst.addComponent("magvel", desc_lst, State_Type, UMX, 3);

    derive_lst.add("radvel", IndexType::TheCellType(), 1, ca_derradialvel,
                   the_same_box);
    derive_lst.addComponent("radvel", desc_lst, State_Type, URHO, 1);
    derive_lst.addComponent("radvel", desc_lst, State_Type, UMX, 3);

    derive_lst.add("circvel", IndexType::TheCellType(), 1, ca_dercircvel,
                   the_same_box);
    derive_lst.addComponent("circvel", desc_lst, State_Type, URHO, 1);
    derive_lst.addComponent("circvel", desc_lst, State_Type, UMX, 3);

    derive_lst.add("magmom", IndexType::TheCellType(), 1, ca_dermagmom,
                   the_same_box);
    derive_lst.addComponent("magmom", desc_lst, State_Type, UMX, 3);

    derive_lst.add("angular_momentum_x", IndexType::TheCellType(), 1,
                   ca_derangmomx, the_same_box);
    derive_lst.addComponent("angular_momentum_x", desc_lst, State_Type, URHO,
                            1);
    ...
    derive_lst.add("angular_momentum_y", IndexType::TheCellType(), 1,
                   ca_derangmomy, the_same_box);
    derive_lst.addComponent("angular_momentum_y", desc_lst, State_Type, URHO,
                            1);
    ...
    derive_lst.add("angular_momentum_z", IndexType::TheCellType(), 1,
                   ca_derangmomz, the_same_box);
    derive_lst.addComponent("angular_momentum_z", desc_lst, State_Type, URHO,
                            1);
    derive_lst.addComponent("angular_momentum_z", desc_lst, State_Type, UMX, 3);

#ifdef GRAVITY
    derive_lst.add("maggrav", IndexType::TheCellType(), 1, ca_dermaggrav,
                   the_same_box);
    ...
    //
    derive_lst.add("particle_count", IndexType::TheCellType(), 1, ca_dernull,
                   the_same_box);
    ...
    derive_lst.add("total_particle_count", IndexType::TheCellType(), 1,
                   ca_dernull, the_same_box);
    derive_lst.addComponent("total_particle_count", desc_lst, State_Type, URHO,
                            1);
#endif

#ifdef RADIATION
    if (Radiation::do_multigroup) {
        derive_lst.add("Ertot", IndexType::TheCellType(), 1, ca_derertot,
                       the_same_box);
        derive_lst.addComponent("Ertot", desc_lst, Rad_Type, 0,
                                Radiation::nGroups);
    }
#endif
    ...
    //x component
    derive_lst.add("B_x", IndexType::TheCellType(), 1, ca_dermagcenx,
                   the_same_box);
    derive_lst.addComponent("B_x", desc_lst, Mag_Type_x, 0, 1);
    //y component
    derive_lst.add("B_y", IndexType::TheCellType(), 1, ca_dermagceny,
                   the_same_box);
    derive_lst.addComponent("B_y", desc_lst, Mag_Type_y, 0, 1);
    //z component
    derive_lst.add("B_z", IndexType::TheCellType(), 1, ca_dermagcenz,
                   the_same_box);
    ...
    derive_lst.addComponent("E_x", desc_lst, State_Type, Density,
                            1);  //For velocities
    ...
    derive_lst.addComponent("E_y", desc_lst, State_Type, Density,
                            1);  //For velocities
    ...
    derive_lst.addComponent("E_z", desc_lst, State_Type, Density,
                            1);  //For velocities
    derive_lst.addComponent("E_z", desc_lst, State_Type, Xmom, 1);
    derive_lst.addComponent("E_z", desc_lst, State_Type, Ymom, 1);

    //Divergence of B
    derive_lst.add("Div_B", IndexType::TheCellType(), 1, ca_derdivb,
                   the_same_box);
    ...
    for (int i = 0; i < NumAux; i++) {
        derive_lst.add(aux_names[i], IndexType::TheCellType(), 1, ca_derspec,
                       the_same_box);
...
#include <Problem_Derives.H>
    ...
    for (int n = 0; n < num_src; ++n) source_names[n] = "";
    ...
    if (sdc_quadrature == 0) {
        ...
        if (sdc_order == 2) {
            ...
            dt_sdc.resize(SDC_NODES);
            dt_sdc = {0.0, (4.0 - std::sqrt(6.0)) / 10.0,
                      (4.0 + std::sqrt(6.0)) / 10.0, 1.0};

            node_weights.resize(SDC_NODES);
            node_weights = {0.0, (16.0 - std::sqrt(6.0)) / 36.0,
                            (16.0 + std::sqrt(6.0)) / 36.0, 1.0 / 9.0};
