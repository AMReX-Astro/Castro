...
void ca_dersoundspeed(const Box& bx, FArrayBox& derfab, int dcomp,
...
void ca_dermachnumber(const Box& bx, FArrayBox& derfab, int dcomp,
                      int /*ncomp*/, const FArrayBox& datfab,
                      const Geometry& geomdata, Real /*time*/,
                      const int* /*bcrec*/, int /*level*/) {
...
void ca_derdiffterm(const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
    ...
    amrex::ParallelFor(
        bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
            // (k grad T)_{i+1/2}
            Real kgradT_xhi =
                0.5_rt * (coeff_arr(i + 1, j, k) + coeff_arr(i, j, k)) *
                (dat(i + 1, j, k, UTEMP) - dat(i, j, k, UTEMP)) / dx[0];

            // (k grad T)_{i-1/2}
            Real kgradT_xlo =
                0.5_rt * (coeff_arr(i, j, k) + coeff_arr(i - 1, j, k)) *
                (dat(i, j, k, UTEMP) - dat(i - 1, j, k, UTEMP)) / dx[0];

#if AMREX_SPACEDIM >= 2
            // (k grad T)_{j+1/2}
            Real kgradT_yhi =
                0.5_rt * (coeff_arr(i, j + 1, k) + coeff_arr(i, j, k)) *
                (dat(i, j + 1, k, UTEMP) - dat(i, j, k, UTEMP)) / dx[1];

            // (k grad T)_{j-1/2}
            Real kgradT_ylo =
                0.5_rt * (coeff_arr(i, j, k) + coeff_arr(i, j - 1, k)) *
            ...
            // (k grad T)_{k+1/2}
            Real kgradT_zhi =
                0.5_rt * (coeff_arr(i, j, k + 1) + coeff_arr(i, j, k)) *
                (dat(i, j, k + 1, UTEMP) - dat(i, j, k, UTEMP)) / dx[2];

            // (k grad T)_{k-1/2}
            Real kgradT_zlo =
                0.5_rt * (coeff_arr(i, j, k) + coeff_arr(i, j, k - 1)) *
            ...
            if (coord_type == 0) {
                ...
                der(i, j, k, 0) =
                    (rp1 * kgradT_xhi - rm1 * kgradT_xlo) / (r * dx[0]);
#if AMREX_SPACEDIM == 2
                der(i, j, k, 0) += (kgradT_yhi - kgradT_ylo) / dx[1];
#endif
                ...
                der(i, j, k, 0) =
                    (rp1 * rp1 * kgradT_xhi - rm1 * rm1 * kgradT_xlo) /
                    (r * r * dx[0]);
            }
        });
}
...
#ifdef REACTIONS
void ca_derenuctimescale(const Box& bx, FArrayBox& derfab, int dcomp,
                         int /*ncomp*/, const FArrayBox& datfab,
                         const Geometry& geomdata, Real /*time*/,
                         const int* /*bcrec*/, int /*level*/) {
...
void ca_derradialvel(const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
    ...
    amrex::ParallelFor(
        bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
            Real x =
                problo[0] + (static_cast<Real>(i) + 0.5_rt) * dx[0] - center[0];
#if AMREX_SPACEDIM >= 2
            Real y =
                problo[1] + (static_cast<Real>(j) + 0.5_rt) * dx[1] - center[1];
#else
        Real y = 0.0_rt;
#endif
#if AMREX_SPACEDIM == 3
            Real z =
                problo[2] + (static_cast<Real>(k) + 0.5_rt) * dx[2] - center[2];
#else
        Real z = 0.0_rt;
#endif
          ...
          der(i,j,k,0) = (dat(i,j,k,1)*x + dat(i,j,k,2)*y) / (dat(i,j,k,0)*r);
                ...
                der(i, j, k, 0) = (dat(i, j, k, 1) * x + dat(i, j, k, 2) * y +
                                   dat(i, j, k, 3) * z) /
                                  (dat(i, j, k, 0) * r);
            }
        });
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        if (domain_is_plane_parallel) {
          ...
          der(i,j,k,0) = (-dat(i,j,k,1)*y + dat(i,j,k,2)*x) / (dat(i,j,k,0)*r);
            ...
            Real vr = (dat(i, j, k, 1) * x + dat(i, j, k, 2) * y +
                       dat(i, j, k, 3) * z) /
                      (dat(i, j, k, 0) * r);
...
void ca_derkineng(const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
    ...
    amrex::ParallelFor(
        bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
            kineng(i, j, k, 0) = 0.5_rt / dat(i, j, k, 0) *
                                 (dat(i, j, k, 1) * dat(i, j, k, 1) +
                                  dat(i, j, k, 2) * dat(i, j, k, 2) +
                                  dat(i, j, k, 3) * dat(i, j, k, 3));
        });
...
void ca_dernull(Real* der, const int* der_lo, const int* der_hi,
                ...
                const Real* delta, const Real* xlo, const Real* time,
...
void ca_dermagvort(const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        if (coord_type == 0) {
            ...
            der(i, j, k, 0) =
                std::sqrt(vphi_z * vphi_z + (vr_z - vz_r) * (vr_z - vz_r) +
                          (rvphi_r / r) * (rvphi_r / r));
...
void ca_derdivu(const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        if (coord_type == 0) {
            ...
            der(i, j, k, 0) =
                0.5_rt * (rp1 * rp1 * uhi - rm1 * rm1 * ulo) / (r * r * dx[0]);
        }
    });
}
