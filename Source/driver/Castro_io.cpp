...
void Castro::restart(Amr& papa, istream& is, bool bReadSpecial) {
    ...
    if (input_version == -1) {
        if (ParallelDescriptor::IOProcessor()) {
            ...
            CastroHeaderFile.open(FullPathCastroHeaderFile.c_str(),
                                  std::ios::in);
        ...
        ParallelDescriptor::Bcast(&input_version, 1,
    ...
    ParallelDescriptor::Bcast(&lastDtPlotLimited, 1,
                              ParallelDescriptor::IOProcessorNumber());
    ParallelDescriptor::Bcast(&lastDtBeforePlotLimiting, 1,
                              ParallelDescriptor::IOProcessorNumber());
    ...
    if ((grown_factor > 1) && (parent->maxLevel() < 1)) {
        ...
        amrex::Error(
            "Must have max_level > 0 if doing special restart with "
            "grown_factor");
    }

    if (grown_factor > 1 && level == 0) {
        if (verbose && ParallelDescriptor::IOProcessor()) {
            std::cout << "Doing special restart with grown_factor "
                      << grown_factor << std::endl;
        }
        ...
        if (star_at_center == 0) {
            ...
            if (ParallelDescriptor::IOProcessor()) {
                std::cout << "... invalid value of star_at_center: "
                          << star_at_center << std::endl;
            }
            amrex::Abort();
        }
...
#pragma gpu box(bx)
    ...
    if (grown_factor > 1 && level == 1) getLevel(0).avgDown();
    ...
    if (reset_checkpoint_time > -1.e199) {
        ...
        if (!parent->RegridOnRestart())
            amrex::Error(
                "reset_checkpoint_time only makes sense when "
    ...
    if (reset_checkpoint_step > -1) {
        ...
        if (!parent->RegridOnRestart())
            amrex::Error(
                "reset_checkpoint_step only makes sense when "
                "amr.regrid_on_restart=1");
...
void Castro::set_state_in_checkpoint(Vector<int>& state_in_checkpoint) {
    for (int i = 0; i < num_state_type; ++i) {
        state_in_checkpoint[i] = 1;
    }
}

void Castro::checkPoint(const std::string& dir, std::ostream& os,
                        VisMF::How how, bool dump_old_default) {
    ...
    if (level == 0 && ParallelDescriptor::IOProcessor()) {
        {
            ...
            CastroHeaderFile << "Checkpoint version: " << current_version
                             << std::endl;
            CastroHeaderFile.close();
...
std::string Castro::thePlotFileType() const {
    //
...
void Castro::setPlotVariables() {
    ...
    if (time_integration_method == SimplifiedSpectralDeferredCorrections) {
        for (int i = 0; i < desc_lst[Simplified_SDC_React_Type].nComp(); i++) {
            parent->deleteStatePlotVar(
                desc_lst[Simplified_SDC_React_Type].name(i));
        }
    }
#endif
#endif
...
void Castro::writeJobInfo(const std::string& dir, const Real io_time) {
    ...
    jobInfoFile << "     center: " << center[0] << " , " << center[1] << " , "
                << center[2] << "\n";
    jobInfoFile << "\n";
...
void Castro::writePlotFile(const std::string& dir, ostream& os,
                           VisMF::How how) {
    plotFileOutput(dir, os, how, 0);
}

void Castro::writeSmallPlotFile(const std::string& dir, ostream& os,
                                VisMF::How how) {
    plotFileOutput(dir, os, how, 1);
}

void Castro::plotFileOutput(const std::string& dir, ostream& os, VisMF::How how,
                            const int is_small) {
#ifdef AMREX_PARTICLES
    ...
    if (level == 0 && ParallelDescriptor::IOProcessor()) {
        ...
        int f_lev = parent->finestLevel();
        os << f_lev << '\n';
        for (int i = 0; i < BL_SPACEDIM; i++) os << geom.ProbLo(i) << ' ';
        os << '\n';
        for (int i = 0; i < BL_SPACEDIM; i++) os << geom.ProbHi(i) << ' ';
        os << '\n';
        for (int i = 0; i < f_lev; i++) os << parent->refRatio(i)[0] << ' ';
        os << '\n';
        for (int i = 0; i <= f_lev; i++) os << parent->Geom(i).Domain() << ' ';
        os << '\n';
        for (int i = 0; i <= f_lev; i++) os << parent->levelSteps(i) << ' ';
        os << '\n';
        for (int i = 0; i <= f_lev; i++) {
    ...
    if (dlist.size() > 0) {
        for (auto it = dlist.begin(); it != dlist.end(); ++it) {
            if ((parent->isDerivePlotVar(it->name()) && is_small == 0) ||
                ...
                MultiFab::Copy(plotMF, *derive_dat, 0, cnt, it->numDerive(),
                               nGrow);
    ...
    if (Radiation::nplotvar > 0) {
        MultiFab::Copy(plotMF, *(radiation->plotvar[level]), 0, cnt,
                       Radiation::nplotvar, 0);
