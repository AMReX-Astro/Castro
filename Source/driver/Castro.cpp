...
#include <castro_queries.H>
    ...
    if (dgeom.IsRZ() && (lo_bc[0] != Symmetry)) {
        std::cerr << "ERROR:Castro::read_params: must set r=0 boundary "
                     "condition to Symmetry for r-z\n";
        amrex::Error();
    }
    ...
    if (dgeom.IsSPHERICAL()) {
        if ((lo_bc[0] != Symmetry) && (dgeom.ProbLo(0) == 0.0)) {
            std::cerr << "ERROR:Castro::read_params: must set r=0 boundary "
                         "condition to Symmetry for spherical\n";
            amrex::Error();
        }
    }
#elif (BL_SPACEDIM == 2)
    ...
    if (riemann_solver == 2) {
        amrex::Abort(
            "HLLC Riemann solver unsupported when using hybrid momentum.");
    }
#endif
    ...
    if (jacobian == 1) {
        amrex::Abort(
            "Simplified SDC requires the numerical Jacobian now (jacobian = "
            "2)");
    }
#endif
#endif
    ...
    if (grown_factor < 1) amrex::Error("grown_factor must be integer >= 1");
    ...
    if (time_integration_method == SimplifiedSpectralDeferredCorrections) {
        amrex::Error(
            "Simplified SDC currently requires USE_SIMPLIFIED_SDC=TRUE when "
            "compiling.");
    }
#else
    if (time_integration_method != SimplifiedSpectralDeferredCorrections) {
        amrex::Error(
            "When building with USE_SIMPLIFIED_SDC=TRUE, only simplified SDC "
            "can be used.");
    }
#endif
    ...
    if (time_integration_method != SpectralDeferredCorrections) {
        amrex::Error(
            "When building with USE_TRUE_SDC=TRUE, only true SDC can be used.");
    }
#endif
    ...
    if (do_radiation && time_integration_method != CornerTransportUpwind) {
        amrex::Error(
            "Radiation is currently only supported for CTU time advancement.");
    }
#endif
    ...
    if (do_rotation) {
        if (rotational_period <= 0.0) {
            std::cerr << "Error:Castro::Rotation enabled but rotation period "
                         "less than zero\n";
            amrex::Error();
        }
    }
    if (dgeom.IsRZ()) rot_axis = 2;
#if (BL_SPACEDIM == 1)
    if (do_rotation) {
    ...
    if (do_scf_initial_model) {
        amrex::Error(
            "SCF initial model construction is only permitted if USE_GRAV=TRUE "
            "and USE_ROTATION=TRUE at compile time.");
    }
#endif
    ...
    if (do_scf_initial_model) {
        amrex::Error(
            "SCF initial model construction is currently not permitted if "
            "USE_CUDA=TRUE at compile time.");
    }
#endif
...
Castro::Castro() : prev_state(num_state_type) {}

Castro::Castro(Amr& papa, int lev, const Geometry& level_geom,
               const BoxArray& bl, const DistributionMapping& dm, Real time)
    ...
    if (do_hydro && do_reflux) {
        for (int ilev = 0; ilev <= parent->maxLevel(); ++ilev) {
            if (parent->nErrorBuf(ilev) == 0) {
                amrex::Error(
                    "n_error_buf = 0 is unsupported when using hydro.");
            }
        }
    }
    ...
    for (int dim = 0; dim < AMREX_SPACEDIM; ++dim) {
        for (int ilev = 0; ilev <= parent->maxLevel(); ++ilev) {
            if (parent->blockingFactor(ilev)[dim] % numBCThreadsMin[dim] != 0) {
                amrex::Error(
                    "Using CUDA requires a blocking factor that is a multiple "
                    "of 8.");
            }
        }
    }
    ...
    if (do_grav) {
        ...
        if (gravity == 0)
            gravity =
                new Gravity(parent, parent->finestLevel(), &phys_bc, URHO);
        ...
        if (verbose && level == 0 && ParallelDescriptor::IOProcessor())
            std::cout << "Setting the gravity type to "
                      << gravity->get_gravity_type() << std::endl;

#ifdef GRAVITY
        if (gravity->get_gravity_type() == "PoissonGrav" &&
            gravity->NoComposite() != 0 && gravity->NoSync() == 0) {
            std::cerr << "Error: not meaningful to have gravity.no_sync == 0 "
                         "without having gravity.no_composite == 0.";
            amrex::Error();
        }
#endif
    }
    ...
    if (diffusion == 0) diffusion = new Diffusion(parent, &phys_bc);
...
void Castro::initMFs() {
    ...
    for (int dir = 0; dir < BL_SPACEDIM; ++dir)
        fluxes[dir].reset(
            new MultiFab(getEdgeBoxArray(dir), dmap, NUM_STATE, 0));

    for (int dir = BL_SPACEDIM; dir < 3; ++dir)
        fluxes[dir].reset(new MultiFab(get_new_data(State_Type).boxArray(),
                                       dmap, NUM_STATE, 0));
    ...
    for (int dir = BL_SPACEDIM; dir < 3; ++dir)
        mass_fluxes[dir].reset(
            new MultiFab(get_new_data(State_Type).boxArray(), dmap, 1, 0));

#if (BL_SPACEDIM <= 2)
    if (!Geom().IsCartesian()) P_radial.define(getEdgeBoxArray(0), dmap, 1, 0);
#endif
    ...
    if (Radiation::rad_hydro_combined) {
        ...
        for (int dir = 0; dir < BL_SPACEDIM; ++dir) {
            rad_fluxes[dir].reset(new MultiFab(getEdgeBoxArray(dir), dmap,
                                               Radiation::nGroups, 0));
        }
    }
#endif
...
#ifdef RADIATION
        if (Radiation::rad_hydro_combined) {
            rad_flux_reg.define(grids, dmap, crse_ratio, level,
                                Radiation::nGroups);
...
void Castro::setTimeLevel(Real time, Real dt_old, Real dt_new) {
    AmrLevel::setTimeLevel(time, dt_old, dt_new);
}

void Castro::setGridInfo() {
    ...
    if (level == 0) {
        ...
        for (int lev = 1; lev <= max_level; lev++) {
            ...
            for (int dir = 0; dir < 3; dir++)
                if (dir < BL_SPACEDIM) {
                    ...
                    domhi_level[3 * lev + dir] =
                        domlo_level[3 * lev + dir] + ncell - 1;
        ...
        ca_set_grid_info(max_level, dx_level.data(), domlo_level.data(),
                         domhi_level.data(), ref_ratio_to_f.data(),
                         n_error_buf_to_f.data(), blocking_factor_to_f.data());
    }
}

void Castro::initData() {
...
#else
    {
        ...
        for (MFIter mfi(S_new); mfi.isValid(); ++mfi) {
            ...
            (level, cur_time, lo, hi, nbx, BL_TO_FORTRAN_3D(Bx_new[mfi]), nby,
             BL_TO_FORTRAN_3D(By_new[mfi]), nbz, BL_TO_FORTRAN_3D(Bz_new[mfi]),
...
#pragma gpu box(box)
            ca_initdata(AMREX_INT_ANYD(lo), AMREX_INT_ANYD(hi),
                        ...
                        AMREX_REAL_ANYD(prob_lo));

#else
            RealBox gridloc =
                RealBox(grids[mfi.index()], geom.CellSize(), geom.ProbLo());

            BL_FORT_PROC_CALL(CA_INITDATA, ca_initdata)
            (level, cur_time, ARLIM_3D(lo), ARLIM_3D(hi), NUM_STATE,
             BL_TO_FORTRAN_ANYD(S_new[mfi]), ZFILL(dx), ZFILL(gridloc.lo()),
             ZFILL(gridloc.hi()));
...
#pragma omp parallel
        ...
        for (MFIter mfi(S_new, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            reduce_op.eval(
                bx, reduce_data,
                [=] AMREX_GPU_HOST_DEVICE(int i, int j,
                                          int k) noexcept->ReduceTuple {
        ...
        for (MFIter mfi(S_new); mfi.isValid(); ++mfi) {
            ...
            amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
                ...
                if (std::abs(S_arr(i, j, k, URHO) - spec_sum) >
                    1.e-8_rt * S_arr(i, j, k, URHO)) {
#ifndef AMREX_USE_CUDA
                    std::cout << "Sum of (rho X)_i vs rho at (i,j,k): " << i
                              << " " << j << " " << k << " " << spec_sum << " "
                              << S_arr(i, j, k, URHO) << std::endl;
#endif
                    amrex::Error(
                        "Error: failed check of initial species summing to 1");
                }
            });
        }
        ...
        if (initialization_is_cell_average == 0) {
            ...
            if (sdc_order == 4) {
                ...
                AmrLevel::FillPatch(*this, Sborder, NUM_GROW, cur_time,
                ...
                for (MFIter mfi(S_new); mfi.isValid(); ++mfi) {
                    ...
                    make_fourth_in_place(box, Sborder.array(mfi), tmp_arr,
                                         domain_lo, domain_hi);
                }
            ...
            Sborder.define(grids, dmap, NUM_STATE, NUM_GROW);
            AmrLevel::FillPatch(*this, Sborder, NUM_GROW, cur_time, State_Type,
                                0, NUM_STATE);
            ...
            for (MFIter mfi(S_new); mfi.isValid(); ++mfi) {
                ...
                make_cell_center_in_place(box, Sborder.array(mfi), tmp_arr,
                                          domain_lo, domain_hi);
            }
            ...
            for (MFIter mfi(S_new); mfi.isValid(); ++mfi) {
                ...
                amrex::ParallelFor(box, [=] AMREX_GPU_HOST_DEVICE(
        ...
        if (ng > 0)
            AmrLevel::FillPatch(*this, S_new, ng, cur_time, State_Type, 0,
                                S_new.nComp());
    }
...
#pragma gpu box(box)
            ca_initrad(AMREX_INT_ANYD(lo), AMREX_INT_ANYD(hi),
                       BL_TO_FORTRAN_ANYD(Rad_new[mfi]), AMREX_REAL_ANYD(dx),
                       AMREX_REAL_ANYD(prob_lo));
            ...
            (level, cur_time, ARLIM_3D(lo), ARLIM_3D(hi), Radiation::nGroups,
             BL_TO_FORTRAN_ANYD(Rad_new[mfi]), ZFILL(dx), ZFILL(gridloc.lo()),
             ZFILL(gridloc.hi()));
...
void Castro::init(AmrLevel& old) {
    ...
    for (int s = 0; s < num_state_type; ++s) {
        ...
        FillPatch(old, state_MF, state_MF.nGrow(), cur_time, s, 0,
                  state_MF.nComp());
    }
}
...
void Castro::init() {
    ...
    if (getLevel(level - 1).post_step_regrid) time = prev_time;
...
Real Castro::estTimeStep(Real dt_old) {
...
#pragma omp parallel reduction(min : estdt_hydro)
#endif
            {
                ...
                for (MFIter mfi(stateMF, TilingIfNotGPU()); mfi.isValid();
                     ++mfi) {
                    const Box& tbox = mfi.tilebox();
                    const Box& vbox = mfi.validbox();

                    gPr.resize(tbox);
                    radiation->estimate_gamrPr(stateMF[mfi], radMF[mfi], gPr,
                                               dx, vbox);

                    ca_estdt_rad(tbox.loVect(), tbox.hiVect(),
                                 BL_TO_FORTRAN(stateMF[mfi]),
                                 BL_TO_FORTRAN(gPr), dx, &dt);
                }
    ...
    estdt_diffusion *= cfl;
    if (verbose) {
        amrex::Print() << "...estimated diffusion-limited timestep at level "
                       << level << ": " << estdt_diffusion << std::endl;
    }
...
#pragma gpu box(box)
                ca_estdt_burning(
                    ...
                    BL_TO_FORTRAN_ANYD(R_new[mfi]), AMREX_REAL_ANYD(dx),
        ...
        if (verbose && estdt_burn < max_dt) {
            amrex::Print() << "...estimated burning-limited timestep at level "
                           << level << ": " << estdt_burn << std::endl;
        }
    ...
    if (verbose) {
        amrex::Print() << "Castro::estTimeStep (" << limiter
                       << "-limited) at level " << level
                       << ":  estdt = " << estdt << '\n'
                       << std::endl;
    }
...
void Castro::computeNewDt(int finest_level, int sub_cycle, Vector<int>& n_cycle,
                ...
                for (int i = 0; i <= finest_level; i++) {
                    if (verbose && ParallelDescriptor::IOProcessor())
                        if (dt_min[i] > change_max * dt_level[i]) {
                            std::cout << "Castro::compute_new_dt : limiting dt "
                                         "at level "
                                      << i << '\n';
    ...
    for (int i = 0; i <= finest_level; ++i) {
        if (getLevel(i).lastDtRetryLimited == 1) {
            if (getLevel(i).lastDtFromRetry < dt_min[i]) {
                if (verbose && ParallelDescriptor::IOProcessor()) {
                    std::cout << " ... limiting dt at level " << i
                              << " to: " << getLevel(i).lastDtFromRetry
                              << " = retry-limited timestep\n";
                }
    ...
    if (plot_per_is_exact) {
        ...
        if (plot_per > 0.0) {
            ...
            if (newPlotDt < dt_0) {
                ...
                if (verbose)
                    amrex::Print() << " ... limiting dt to " << dt_0
                                   << " to hit the next plot interval.\n";
            }
        }
    }
    ...
    if (small_plot_per_is_exact) {
        ...
        if (small_plot_per > 0.0) {
            ...
            if (std::abs(dtMod - small_plot_per) <=
                std::numeric_limits<Real>::epsilon() * cur_time) {
            ...
            if (newSmallPlotDt < dt_0) {
                ...
                if (verbose)
                    amrex::Print() << " ... limiting dt to " << dt_0
                                   << " to hit the next smallplot interval.\n";
            }
        }
    }
...
void Castro::computeInitialDt(int finest_level, int sub_cycle,
                              Vector<int>& n_cycle,
                              const Vector<IntVect>& ref_ratio,
...
void Castro::post_timestep(int iteration) {
    ...
    if (do_reflux && level < parent->finestLevel()) reflux(level, level + 1);

    // Ensure consistency with finer grids.

    if (level < finest_level) avgDown();
    ...
    if (level == 0) {
        ...
        if (sum_interval > 0) {
            ...
            if (nstep % sum_interval == 0) sum_int_test = true;
        }
        ...
        if (sum_per > 0.0) {
            ...
            if (num_per_old != num_per_new) sum_per_test = true;
        }

        if (sum_int_test || sum_per_test) sum_integrated_quantities();
    ...
    if (level == 0) do_energy_diagnostics();
#endif
...
void Castro::post_restart() {
    ...
    if (do_grav) {
        if (level == 0) {
            ...
            for (int lev = 0; lev <= parent->finestLevel(); lev++) {
                ...
                gravity->install_level(lev, &this_level, cs_level.Volume(),
                                       cs_level.Area());
            }
            ...
            if (gravity->get_gravity_type() == "PoissonGrav") {
                if (gravity->NoComposite() != 1) {
                    ...
                    gravity->multilevel_solve_for_new_phi(
                        0, parent->finestLevel(), use_previous_phi);
                    if (gravity->test_results_of_solves() == 1)
                        gravity->test_composite_phi(level);
                }
            }

            if (grown_factor > 1) post_grown_restart();
        }
    }
#endif
    ...
    if (level == 0)
        for (int lev = 0; lev <= parent->finestLevel(); lev++) {
            AmrLevel& this_level = getLevel(lev);
            Castro& cs_level = getLevel(lev);
            diffusion->install_level(lev, &this_level, cs_level.Volume(),
                                     cs_level.Area());
        }
#endif
...
void Castro::postCoarseTimeStep(Real cumtime) {
    // postCoarseTimeStep() is only called by level 0.
    BL_ASSERT(level == 0);
    AmrLevel::postCoarseTimeStep(cumtime);
#ifdef GRAVITY
    if (do_grav) gravity->set_mass_offset(cumtime, 0);
#endif
}

void Castro::check_for_post_regrid(Real time) {
    ...
    if (level < parent->maxLevel()) {
        ...
        if (missing_on_fine_level) {
            ...
            if (amrex::ParallelDescriptor::IOProcessor()) {
                std::cout << "\n"
                          << "Current refinement pattern insufficient at level "
                          << level << ".\n"
...
void Castro::post_regrid(int lbase, int new_finest) {
    ...
    if (do_grav) {
        ...
        if (use_post_step_regrid && getLevel(lbase).post_step_regrid &&
            ...
            if (level > lbase) {
                ...
                Vector<MultiFab*> grad_phi_coarse =
                    amrex::GetVecOfPtrs(gravity->get_grad_phi_prev(level - 1));
                Vector<MultiFab*> grad_phi_fine =
                    amrex::GetVecOfPtrs(gravity->get_grad_phi_curr(level));

                Real time =
                    getLevel(lbase).get_state_data(Gravity_Type).prevTime();
                ...
                const Vector<BCRec>& gp_bcs =
                    getLevel(level).get_desc_lst()[Gravity_Type].getBCs();

                for (int n = 0; n < BL_SPACEDIM; ++n) {
                    amrex::InterpFromCoarseLevel(
                        *grad_phi_fine[n], time, *grad_phi_coarse[n], 0, 0, 1,
                        parent->Geom(level - 1), parent->Geom(level),
                        gp_phys_bc, 0, gp_phys_bc, 0,
                        parent->refRatio(level - 1), gp_interp, gp_bcs, 0);
                }
            }
            ...
            if ((level == lbase) && cur_time > 0.) {
                if (gravity->get_gravity_type() == "PoissonGrav" &&
                    (gravity->NoComposite() != 1)) {
                    int use_previous_phi = 1;

                    // Update the maximum density, used in setting the solver tolerance.

                    if (level == 0) gravity->update_max_rhs();
...
void Castro::post_init(Real stop_time) {
    ...
    //
    int finest_level = parent->finestLevel();
    for (int k = finest_level - 1; k >= 0; k--) getLevel(k).avgDown();
...
#ifdef GRAVITY
    if (level == 0 && moving_center == 1) write_center();
#endif
}

void Castro::post_grown_restart() {
...
int Castro::okToContinue() {
    ...
    if (signalStopJob) {
        ...
        test = 0;
        if (ParallelDescriptor::IOProcessor())
            std::cout << " Signalling a stop of the run because dt < dt_cutoff "
                         "* time."
                      << std::endl;
    }
...
#ifdef AUX_UPDATE
void Castro::advance_aux(Real time, Real dt) {
...
#pragma omp parallel
    ...
    for (MFIter mfi(S_old, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        void ca_auxupdate(BL_TO_FORTRAN(old_fab), BL_TO_FORTRAN(new_fab),
                          box.loVect(), box.hiVect(), &dt);
    }
}
#endif

void Castro::FluxRegCrseInit() {
    ...
    for (int i = 0; i < BL_SPACEDIM; ++i)
        fine_level.flux_reg.CrseInit(*fluxes[i], i, 0, 0, NUM_STATE,
                                     flux_crse_scale);
    ...
    if (Radiation::rad_hydro_combined)
        for (int i = 0; i < BL_SPACEDIM; ++i)
            fine_level.rad_flux_reg.CrseInit(
                *rad_fluxes[i], i, 0, 0, Radiation::nGroups, flux_crse_scale);
#endif
}

void Castro::FluxRegFineAdd() {
    ...
    if (Radiation::rad_hydro_combined)
        for (int i = 0; i < BL_SPACEDIM; ++i)
            getLevel(level).rad_flux_reg.FineAdd(
                *rad_fluxes[i], i, 0, 0, Radiation::nGroups, flux_fine_scale);
#endif
}

void Castro::reflux(int crse_level, int fine_level) {
    ...
    for (int lev = fine_level; lev > crse_level; --lev) {
        ...
        reg->Reflux(crse_state, crse_lev.volume, 1.0, 0, 0, NUM_STATE,
                    crse_lev.geom);
        ...
        if (do_grav && gravity->get_gravity_type() == "PoissonGrav" &&
            gravity->NoSync() == 0) {
            reg->Reflux(*drho[ilev], crse_lev.volume, 1.0, 0, URHO, 1,
                        crse_lev.geom);
            amrex::average_down(*drho[ilev + 1], *drho[ilev], 0, 1,
                                getLevel(lev).crse_ratio);
        }
#endif
        ...
        if (update_sources_after_reflux) {
            ...
            for (int i = 0; i < BL_SPACEDIM; ++i) {
                temp_fluxes[i].reset(new MultiFab(
                    crse_lev.fluxes[i]->boxArray(),
            ...
            for (OrientationIter fi; fi; ++fi) {
                ...
                fs.copyTo(*temp_fluxes[idir], 0, 0, 0,
                          temp_fluxes[idir]->nComp());
            }
            for (int i = 0; i < BL_SPACEDIM; ++i) {
                MultiFab::Add(*crse_lev.fluxes[i], *temp_fluxes[i], 0, 0,
                              crse_lev.fluxes[i]->nComp(), 0);
                MultiFab::Add(*crse_lev.mass_fluxes[i], *temp_fluxes[i], URHO,
                              0, 1, 0);
                temp_fluxes[i].reset();
            }
        }
        ...
        if (!Geom().IsCartesian()) {
            ...
            if (update_sources_after_reflux) {
                ...
                temp_fluxes[0].reset(new MultiFab(
                    crse_lev.P_radial.boxArray(),
                ...
                MultiFab::Add(crse_lev.P_radial, *temp_fluxes[0], 0, 0,
                              crse_lev.P_radial.nComp(), 0);
                temp_fluxes[0].reset();
            }
        ...
        if (Radiation::rad_hydro_combined) {
            ...
            reg->Reflux(crse_lev.get_new_data(Rad_Type), crse_lev.volume, 1.0,
                        0, 0, Radiation::nGroups, crse_lev.geom);

            if (update_sources_after_reflux) {

                for (int i = 0; i < BL_SPACEDIM; ++i) {
                    temp_fluxes[i].reset(
                        new MultiFab(crse_lev.rad_fluxes[i]->boxArray(),
                                     crse_lev.rad_fluxes[i]->DistributionMap(),
                                     crse_lev.rad_fluxes[i]->nComp(),
                                     crse_lev.rad_fluxes[i]->nGrow()));
                    temp_fluxes[i]->setVal(0.0);
                }
                for (OrientationIter fi; fi; ++fi) {
                    const FabSet& fs = (*reg)[fi()];
                ...
                for (int i = 0; i < BL_SPACEDIM; ++i) {
                    MultiFab::Add(*crse_lev.rad_fluxes[i], *temp_fluxes[i], 0,
                                  0, crse_lev.rad_fluxes[i]->nComp(), 0);
                    temp_fluxes[i].reset();
                }
            }
        ...
        if (do_grav && gravity->get_gravity_type() == "PoissonGrav" &&
            ...
            for (int i = 0; i < BL_SPACEDIM; ++i) {
                reg->CrseInit(*(gravity->get_grad_phi_curr(lev - 1)[i]),
                              crse_lev.area[i], i, 0, 0, 1, -1.0);
                reg->FineAdd(*(gravity->get_grad_phi_curr(lev)[i]),
                             fine_lev.area[i], i, 0, 0, 1, 1.0);
            }

            reg->Reflux(*dphi[ilev], crse_lev.volume, 1.0, 0, 0, 1,
                        crse_lev.geom);

            amrex::average_down(*dphi[ilev + 1], *dphi[ilev], 0, 1,
                                getLevel(lev).crse_ratio);
    ...
    if (do_grav && gravity->get_gravity_type() == "PoissonGrav" &&
        gravity->NoSync() == 0)
        gravity->gravity_sync(crse_level, fine_level, amrex::GetVecOfPtrs(drho),
                              amrex::GetVecOfPtrs(dphi));
#endif
    ...
    if (update_sources_after_reflux &&
        ...
        for (int lev = fine_level; lev >= crse_level; --lev) {
            ...
            Real dt_advance_local =
                getLevel(lev)
                    .dt_advance;  // Note that this may be shorter than the full timestep due to subcycling.
            Real dt_amr = parent->dtLevel(
                lev);  // The full timestep expected by the Amr class.

            ca_set_amr_info(lev, -1, -1, time, dt_advance_local);

            if (getLevel(lev).apply_sources()) {

                getLevel(lev).apply_source_to_state(S_new, source,
                                                    -dt_advance_local, 0);
            ...
            if (use_retry && dt_advance_local < dt_amr &&
                getLevel(lev).keep_prev_state) {
                ...
                for (int k = 0; k < num_state_type; k++) {
                    ...
                    if (getLevel(lev).prev_state[k]->hasOldData()) {
                        ...
                        MultiFab::Copy(getLevel(lev).prev_state[k]->newData(),
                                       old, 0, 0, old.nComp(), old.nGrow());
                        MultiFab::Copy(old,
                                       getLevel(lev).prev_state[k]->oldData(),
                                       0, 0, old.nComp(), old.nGrow());

                        getLevel(lev).state[k].setTimeLevel(
                            time, dt_advance_local, 0.0);
                        getLevel(lev).prev_state[k]->setTimeLevel(time, dt_amr,
                                                                  0.0);
                    }
                }
            }
            ...
            if (getLevel(lev).apply_sources()) {
                ...
                getLevel(lev).do_new_sources(
                    ...
                    Bx_new, By_new, Bz_new,
#endif
                    source, S_old, S_new, time, dt_advance_local,
                    apply_sources_to_state);
            }
            ...
            if (use_retry && dt_advance_local < dt_amr &&
                ...
                for (int k = 0; k < num_state_type; k++) {
                    ...
                    if (getLevel(lev).prev_state[k]->hasOldData()) {
                        ...
                        MultiFab& old = getLevel(lev).get_old_data(k);
                        MultiFab::Copy(old,
                                       getLevel(lev).prev_state[k]->newData(),
                                       0, 0, old.nComp(), old.nGrow());

                        getLevel(lev).state[k].setTimeLevel(time, dt_amr, 0.0);
                        getLevel(lev).prev_state[k]->setTimeLevel(
                            time, dt_advance_local, 0.0);
                    }
                }
    ...
    if (verbose) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Castro::reflux() at level " << level
                          << " : time = " << end << std::endl;
#ifdef BL_LAZY
        });
#endif
...
void Castro::normalize_species(MultiFab& S_new, int ng) {
    ...
    for (MFIter mfi(S_new, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
                ...
                for (int n = 0; n < NumSpec; ++n) {
                    u(i, j, k, UFS + n) = amrex::max(
                        lsmall_x * u(i, j, k, URHO),
                        amrex::min(u(i, j, k, URHO), u(i, j, k, UFS + n)));
...
void Castro::enforce_consistent_e(
#ifdef MHD
...
#pragma omp parallel
    ...
    for (MFIter mfi(S, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        ParallelFor(box, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
                                                   int k) noexcept {
            Real rhoInv = 1.0_rt / S_arr(i, j, k, URHO);
            Real u = S_arr(i, j, k, UMX) * rhoInv;
            Real v = S_arr(i, j, k, UMY) * rhoInv;
            ...
            S_arr(i, j, k, UEDEN) +=
                0.5_rt * (bx_cell_c * bx_cell_c + by_cell_c * by_cell_c +
                          bz_cell_c * bz_cell_c);
#endif
        });
    }
...
void Castro::enforce_min_density(MultiFab& state_in, int ng) {
    ...
    if (print_update_diagnostics) {
        ...
        reset_source.define(state_in.boxArray(), state_in.DistributionMap(),
                            state_in.nComp(), 0);
...
#pragma gpu box(bx)
        ca_enforce_minimum_density(AMREX_INT_ANYD(bx.loVect()),
                                   AMREX_INT_ANYD(bx.hiVect()),
                                   BL_TO_FORTRAN_ANYD(state_in[mfi]), verbose);
    }
    ...
    if (print_update_diagnostics) {
        ...
        bool local = true;
        Vector<Real> reset_update =
            evaluate_source_change(reset_source, 1.0, local);

#ifdef BL_LAZY
        Lazy::QueueReduction([=]() mutable {
#endif
            ParallelDescriptor::ReduceRealSum(
                reset_update.dataPtr(), reset_update.size(),
                ParallelDescriptor::IOProcessorNumber());

            if (ParallelDescriptor::IOProcessor()) {
                if (std::abs(reset_update[0]) != 0.0) {
                    std::cout << std::endl
                              << "  Contributions to the state from negative "
                                 "density resets:"
                              << std::endl;
...
void Castro::avgDown(int state_indx) {
    ...
    amrex::average_down(S_fine, S_crse, fgeom, cgeom, 0, S_fine.nComp(),
                        fine_ratio);
}

void Castro::allocOldData() {
    MultiFab::RegionTag amrlevel_tag("AmrLevel_Level_" + std::to_string(level));
    MultiFab::RegionTag statedata_tag("StateData_Level_" +
                                      std::to_string(level));
    for (int k = 0; k < num_state_type; k++) state[k].allocOldData();
}

void Castro::removeOldData() { AmrLevel::removeOldData(); }

void Castro::errorEst(TagBoxArray& tags, int clearval, int tagval, Real time,
    ...
    if (post_step_regrid) ltime = get_state_data(State_Type).curTime();
...
void Castro::apply_problem_tags(TagBoxArray& tags, Real time) {
...
#pragma gpu box(bx)
            set_problem_tags(
                ...
                BL_TO_FORTRAN_ANYD(S_new[mfi]), AMREX_REAL_ANYD(dx),
                AMREX_REAL_ANYD(prob_lo), tagval, clearval, time, level);
#else
            set_problem_tags(
                AMREX_ARLIM_ANYD(bx.loVect()), AMREX_ARLIM_ANYD(bx.hiVect()),
                (int8_t*)BL_TO_FORTRAN_ANYD(tagfab),
                BL_TO_FORTRAN_ANYD(S_new[mfi]), AMREX_ZFILL(dx),
                AMREX_ZFILL(prob_lo), tagval, clearval, time, level);
#endif
        }
    }
}

void Castro::apply_tagging_func(TagBoxArray& tags, Real time, int j) {
...
#pragma gpu box(bx)
                ca_presserror(AMREX_INT_ANYD(lo), AMREX_INT_ANYD(hi),
                              ...
                              AMREX_REAL_ANYD(dx), AMREX_REAL_ANYD(prob_lo),
                              tagval, clearval, time, level);
            } else if (err_list_names[j] == "x_velocity" ||
                       err_list_names[j] == "y_velocity" ||
                       err_list_names[j] == "z_velocity") {
#pragma gpu box(bx)
...
std::unique_ptr<MultiFab> Castro::derive(const std::string& name, Real time,
                                         int ngrow) {
...
void Castro::derive(const std::string& name, Real time, MultiFab& mf,
                    int dcomp) {
...
void Castro::amrinfo_init() { ca_amrinfo_init(); }

void Castro::amrinfo_finalize() { ca_amrinfo_finalize(); }

void Castro::extern_init() {
...
void Castro::reset_internal_energy(const Box& bx,
#ifdef MHD
                                   Array4<Real> const Bx, Array4<Real> const By,
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        Real B_ener = 0.5_rt * (bx_cell_c * bx_cell_c + by_cell_c * by_cell_c +
                                bz_cell_c * bz_cell_c);
#else
        Real B_ener = 0.0_rt;
#endif
        ...
        u(i, j, k, UEINT) =
            amrex::max(u(i, j, k, UEINT), u(i, j, k, URHO) * small_e);
        u(i, j, k, UEDEN) = amrex::max(
            u(i, j, k, UEDEN), u(i, j, k, URHO) * (small_e + ke) + B_ener);
...
void Castro::reset_internal_energy(
#ifdef MHD
    MultiFab& Bx, MultiFab& By, MultiFab& Bz,
#endif
...

{
    ...
    if (print_update_diagnostics) {
        old_state.define(S_new.boxArray(), S_new.DistributionMap(),
                         S_new.nComp(), 0);
...
#pragma omp parallel
    ...
    if (print_update_diagnostics) {
        ...
        MultiFab reset_source(S_new.boxArray(), S_new.DistributionMap(),
                              S_new.nComp(), 0);
...

{
    ...
    if (sdc_order == 4) {
        ...
        for (MFIter mfi(Stemp); mfi.isValid(); ++mfi) {
            ...
            make_cell_center_in_place(bx, Stemp.array(mfi), tmp_arr, domain_lo,
                                      domain_hi);
        }
    }
#endif
...
#pragma gpu box(bx)
            ca_clamp_temp(AMREX_INT_ANYD(bx.loVect()),
                          AMREX_INT_ANYD(bx.hiVect()),
    ...
    if (sdc_order == 4) {
        ...
        for (MFIter mfi(Stemp); mfi.isValid(); ++mfi) {
            ...
            // only temperature
            make_fourth_in_place_n(bx, Stemp.array(mfi), UTEMP, tmp_arr,
                                   domain_lo, domain_hi);
        }
        ...
        if (ng > 0) {
            AmrLevel::FillPatch(*this, State, State.nGrow(), time, State_Type,
                                0, NUM_STATE);
        }
...
void Castro::create_source_corrector() {
    ...
    if (time_integration_method == CornerTransportUpwind &&
        ...
        AmrLevel::FillPatch(*this, source_corrector, NUM_GROW, time,
                            Source_Type, UMX, 3, UMX);
...
void Castro::swap_state_time_levels(const Real dt) {
...
int Castro::get_numpts() {
    ...
    if (verbose && ParallelDescriptor::IOProcessor())
        std::cout << "Castro::numpts_1d at level  " << level << " is "
                  << numpts_1d << std::endl;

    return numpts_1d;
}

void Castro::make_radial_data(int is_new) {
#if (BL_SPACEDIM > 1)
    ...
    if (is_new == 1) {
        ...
        for (MFIter mfi(S); mfi.isValid(); ++mfi) {
            ...
            ca_compute_avgstate(
                ...
                &nc, BL_TO_FORTRAN_ANYD(S[mfi]), radial_state.dataPtr(),
        ...
        ParallelDescriptor::ReduceRealSum(radial_state.dataPtr(),
                                          numpts_1d * nc);
        ...
        set_new_outflow_data(radial_state_short.dataPtr(), &new_time, &np_max,
                             &nc);
    } else {
        ...
        set_old_outflow_data(radial_state_short.dataPtr(), &old_time, &np_max,
                             &nc);
    }
...
void Castro::define_new_center(MultiFab& S, Real time) {
    ...
    for (MFIter mfi(mf); mfi.isValid(); ++mfi) {
        ca_find_center(mf[mfi].dataPtr(), &center[0], ARLIM_3D(mi), ZFILL(dx),
                       ZFILL(geom.ProbLo()));
    }
...
void Castro::write_center() {
    int ndatalogs = parent->NumDataLogs();

    if ((moving_center == 1) && (ndatalogs > 0) &&
...
MultiFab& Castro::build_fine_mask() {
    BL_PROFILE("Castro::build_fine_mask()");

    BL_ASSERT(level >
              0);  // because we are building a mask for the coarser level
...
iMultiFab& Castro::build_interior_boundary_mask(int ng) {
    ...
    ib_mask.push_back(
        std::unique_ptr<iMultiFab>(new iMultiFab(grids, dmap, 1, ng)));
    ...
    imf.BuildMask(geom.Domain(), geom.periodicity(), ghost_covered_by_valid,
                  other_cells, other_cells, other_cells);
...
void Castro::expand_state(MultiFab& S, Real time, int ng) {
...
void Castro::check_for_nan(MultiFab& state_in, int check_ghost) {
        ...
        for (int i = 0; i < state_in.nComp(); i++) {
            if (state_in.contains_nan(URHO + i, 1, ng, true)) {
                std::string abort_string =
                    std::string("State has NaNs in the ") +
                    desc_lst[State_Type].name(i) +
                    std::string(" component::check_for_nan()");
