...
int main(int argc, char* argv[]) {
    ...
    amrex::Initialize(argc, argv);
    {
        ...
        if (max_step < 0 && stop_time < 0.0) {
            amrex::Abort(
                "Exiting because neither max_step nor stop_time is "
                "non-negative.");
        }
        ...
        if (amrptr->RegridOnRestart() && ((amrptr->levelSteps(0) >= max_step) ||
        ...
        if (ParallelDescriptor::IOProcessor()) {
            ...
            std::cout << "Run time without initialization = "
                      << runtime_timestep << std::endl;

            fom = fom / runtime_timestep / 1.e6;

            std::cout << "\n";
            std::cout << "  Average number of zones advanced per microsecond: "
