...
Real Castro::do_advance_sdc(Real time, Real dt, int amr_iteration,
                            int amr_ncycle) {
    ...
    for (int m = 0; m < SDC_NODES; m++) {
            ...
            if (apply_sources()) {
                ...
                if (sdc_order == 4) {
                    ...
                    for (MFIter mfi(S_new); mfi.isValid(); ++mfi) {
                        ...
                        make_cell_center(gbx, Sborder.array(mfi),
                                         Sburn.array(mfi), domain_lo,
                                         domain_hi);
                    }

                    // we pass in the stage time here
                    do_old_sources(old_source, Sburn, Sburn, node_time, dt,
                                   apply_sources_to_state, amr_iteration,
                                   amr_ncycle);
                    ...
                    // the node time (time)
                    AmrLevel::FillPatch(*this, old_source, old_source.nGrow(),
                                        prev_time, Source_Type, 0, NSRC);
                    ...
                    for (MFIter mfi(S_new); mfi.isValid(); ++mfi) {
                        ...
                        make_fourth_in_place(bx, old_source.array(mfi), tmp_arr,
                                             domain_lo, domain_hi);
                    }
                    ...
                    do_old_sources(old_source, Sborder, Sborder, cur_time, dt,
                                   apply_sources_to_state, amr_iteration,
                                   amr_ncycle);
                }
                ...
                MultiFab::Copy(sources_for_hydro, old_source, 0, 0, NSRC,
                               old_source.nGrow());
        ...
        if (sdc_iteration == 0 && m == 0) {
            ...
            for (int n = 1; n < SDC_NODES; n++) {
                MultiFab::Copy(*(R_old[n]), *(R_old[0]), 0, 0,
                               R_old[0]->nComp(), 0);
            }
#endif
        }
        ...
        if (m < SDC_NODES - 1) {
            ...
            amrex::Print() << "... doing the SDC update, iteration = "
                           << sdc_iteration << " from node " << m << " to "
                           << m + 1 << std::endl;
    ...
    if (sdc_iteration == sdc_order + sdc_extra - 1) {
        ...
        do_old_sources(old_source, Sborder, Sborder, prev_time, dt,
                       apply_sources_to_state, amr_iteration, amr_ncycle);
        AmrLevel::FillPatch(*this, old_source, old_source.nGrow(), prev_time,
                            Source_Type, 0, NSRC);

        clean_state(S_new, cur_time, 0);
        expand_state(Sborder, cur_time, Sborder.nGrow());
        do_old_sources(new_source, Sborder, Sborder, cur_time, dt,
                       apply_sources_to_state, amr_iteration, amr_ncycle);
        AmrLevel::FillPatch(*this, new_source, new_source.nGrow(), cur_time,
                            Source_Type, 0, NSRC);
    }
    ...
    for (MFIter mfi(R_new); mfi.isValid(); ++mfi) {
        ...
        if (sdc_order == 4) {
            ...
            make_cell_center(obx, Sborder.array(mfi), U_center_arr, domain_lo,
                             domain_hi);
            ...
            Array4<const Real> const R_old_arr =
                R_old[SDC_NODES - 1]->array(mfi);
            Array4<const Real> const S_new_arr = S_new.array(mfi);
            Array4<Real> const R_new_arr = R_new.array(mfi);
            // we don't worry about the difference between centers and averages
            ca_store_reaction_state(bx, R_old_arr, S_new_arr, R_new_arr);
        }
    }
