...
Real Castro::advance(Real time, Real dt, int amr_iteration, int amr_ncycle)
...

{
    ...
    if (time_integration_method == CornerTransportUpwind ||
        ...
        dt_new = std::min(
            dt_new, subcycle_advance_ctu(time, dt, amr_iteration, amr_ncycle));
...
void Castro::initialize_do_advance(Real time, Real dt, int amr_iteration,
                                   int amr_ncycle) {
    ...
    if (time_integration_method == CornerTransportUpwind ||
        ...
        Bx_old_tmp.define(Bx_old.boxArray(), Bx_old.DistributionMap(), 1,
                          NUM_GROW);
        By_old_tmp.define(By_old.boxArray(), By_old.DistributionMap(), 1,
                          NUM_GROW);
        Bz_old_tmp.define(Bz_old.boxArray(), Bz_old.DistributionMap(), 1,
                          NUM_GROW);
...
void Castro::finalize_do_advance(Real time, Real dt, int amr_iteration,
                                 int amr_ncycle) {
...
void Castro::initialize_advance(Real time, Real dt, int amr_iteration,
                                int amr_ncycle) {
    ...
    clean_state(
#ifdef MHD
        get_old_data(Mag_Type_x), get_old_data(Mag_Type_y),
    ...
    if (time_integration_method == SpectralDeferredCorrections) {
        ...
        A_new[0].reset(
            new MultiFab(*A_old[0], amrex::make_alias, 0, NUM_STATE));
        for (int n = 1; n < SDC_NODES; ++n) {
...
void Castro::finalize_advance(Real time, Real dt, int amr_iteration,
                              int amr_ncycle) {
    ...
    if (track_grid_losses) {
        ...
        ParallelDescriptor::ReduceRealSum(material_lost_through_boundary_temp,
                                          n_lost);
    ...
    num_zones_advanced +=
        static_cast<Real>(grids.numPts()) / getLevel(0).grids.numPts();
