...
void Castro::ca_sdc_update_advection_o2_lobatto(
    const Box& bx, Real dt_m, Real dt, Array4<const Real> const& k_m,
    Array4<Real> const& k_n, Array4<const Real> const& A_m,
    Array4<const Real> const& A_0_old, Array4<const Real> const& A_1_old,
    int m_start) {
    ...
    AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
        k_n(i, j, k, n) =
            k_m(i, j, k, n) +
            0.5_rt * dt * (A_0_old(i, j, k, n) + A_1_old(i, j, k, n));
    });
}

void Castro::ca_sdc_update_advection_o2_radau(
    const Box& bx, Real dt_m, Real dt, Array4<const Real> const& k_m,
    Array4<Real> const& k_n, Array4<const Real> const& A_m,
    Array4<const Real> const& A_0_old, Array4<const Real> const& A_1_old,
    Array4<const Real> const& A_2_old, int m_start) {
    // update k_m to k_n via advection -- this is a second-order accurate update
    ...
    if (m_start == 0) {
        AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
            k_n(i, j, k, n) =
                ...
                dt / 12.0_rt *
                    (5.0_rt * A_1_old(i, j, k, n) - A_2_old(i, j, k, n));
        });
    } else if (m_start == 1) {
        AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
            k_n(i, j, k, n) =
...
void Castro::ca_sdc_update_advection_o4_lobatto(
    const Box& bx, Real dt_m, Real dt, Array4<const Real> const& k_m,
    Array4<Real> const& k_n, Array4<const Real> const& A_m,
    Array4<const Real> const& A_0_old, Array4<const Real> const& A_1_old,
    Array4<const Real> const& A_2_old, int m_start) {
    ...
    if (m_start == 0) {
        AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
            k_n(i, j, k, n) =
                ...
                dt_m * (A_m(i, j, k, n) - A_0_old(i, j, k, n)) +
                dt / 24.0_rt *
                    (5.0_rt * A_0_old(i, j, k, n) +
                     8.0_rt * A_1_old(i, j, k, n) - A_2_old(i, j, k, n));
        });
    } else if (m_start == 1) {
        AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
            k_n(i, j, k, n) =
                ...
                dt_m * (A_m(i, j, k, n) - A_1_old(i, j, k, n)) +
                dt / 24.0_rt *
                    (-A_0_old(i, j, k, n) + 8.0_rt * A_1_old(i, j, k, n) +
                     5.0_rt * A_2_old(i, j, k, n));
        });
    } else {
        amrex::Abort(
            "error in ca_sdc_update_advection_o4_lobatto -- should not be "
            "here");
    }
}

void Castro::ca_sdc_update_advection_o4_radau(
    const Box& bx, Real dt_m, Real dt, Array4<const Real> const& k_m,
    Array4<Real> const& k_n, Array4<const Real> const& A_m,
    Array4<const Real> const& A_0_old, Array4<const Real> const& A_1_old,
    Array4<const Real> const& A_2_old, Array4<const Real> const& A_3_old,
    int m_start) {
    // update k_m to k_n via advection -- this is a fourth-order accurate update
    ...
    if (m_start == 0) {
        AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
            k_n(i, j, k, n) = k_m(i, j, k, n) +
                              dt_m * (A_m(i, j, k, n) - A_0_old(i, j, k, n)) +
                              dt / 1800.0_rt *
                                  ((-35.0_rt * std::sqrt(6.0_rt) + 440.0_rt) *
                                       A_1_old(i, j, k, n) +
                                   (-169.0_rt * std::sqrt(6.0_rt) + 296.0_rt) *
                                       A_2_old(i, j, k, n) +
                                   (-16.0_rt + 24.0_rt * std::sqrt(6.0_rt)) *
                                       A_3_old(i, j, k, n));
        });
    } else if (m_start == 1) {
        AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
            k_n(i, j, k, n) =
                ...
                dt_m * (A_m(i, j, k, n) - A_1_old(i, j, k, n)) +
                dt / 150.0_rt *
                    ((-12.0_rt + 17.0_rt * std::sqrt(6.0_rt)) *
                         A_1_old(i, j, k, n) +
                     (12.0_rt + 17.0_rt * std::sqrt(6.0_rt)) *
                         A_2_old(i, j, k, n) +
        ...
        AMREX_PARALLEL_FOR_4D(bx, k_n.nComp(), i, j, k, n, {
            k_n(i, j, k, n) = k_m(i, j, k, n) +
                              ...
                              dt / 600.0_rt *
                                  ((168.0_rt - 73.0_rt * std::sqrt(6.0_rt)) *
                                       A_1_old(i, j, k, n) +
                                   (120.0_rt + 5.0_rt * std::sqrt(6.0_rt)) *
                                       A_2_old(i, j, k, n) +
                                   (72.0_rt + 8.0_rt * std::sqrt(6.0_rt)) *
                                       A_3_old(i, j, k, n));
        });
    } else {
        amrex::Abort(
            "error in ca_sdc_update_advection_o4_radau -- should not be here");
    }
}

#ifdef REACTIONS
void Castro::ca_sdc_compute_C2_lobatto(const Box& bx, Real dt_m, Real dt,
                                       ...
                                       Array4<Real> const& C, int m_start) {
...
void Castro::ca_sdc_compute_C2_radau(
    const Box& bx, Real dt_m, Real dt, Array4<const Real> const& A_m,
    Array4<const Real> const& A_0_old, Array4<const Real> const& A_1_old,
    Array4<const Real> const& A_2_old, Array4<const Real> const& R_0_old,
    Array4<const Real> const& R_1_old, Array4<const Real> const& R_2_old,
    Array4<Real> const& C, int m_start) {
...
void Castro::ca_sdc_compute_C4_lobatto(
    const Box& bx, Real dt_m, Real dt, Array4<const Real> const& A_m,
    Array4<const Real> const& A_0_old, Array4<const Real> const& A_1_old,
    Array4<const Real> const& A_2_old, Array4<const Real> const& R_0_old,
    Array4<const Real> const& R_1_old, Array4<const Real> const& R_2_old,
    ...
    if (m_start == 0) {
        ...
        AMREX_PARALLEL_FOR_4D(bx, C.nComp(), i, j, k, n, {
            Real integral =
                1.0_rt / 12.0_rt *
                (5.0_rt * (A_0_old(i, j, k, n) + R_0_old(i, j, k, n)) +
                 8.0_rt * (A_1_old(i, j, k, n) + R_1_old(i, j, k, n)) -
                 (A_2_old(i, j, k, n) + R_2_old(i, j, k, n)));

            C(i, j, k, n) = (A_m(i, j, k, n) - A_0_old(i, j, k, n)) -
                            R_1_old(i, j, k, n) + integral;
        });
    } else if (m_start == 1) {
        // compute the integral from [t_m, t_{m+1}], normalized by dt_m
        AMREX_PARALLEL_FOR_4D(bx, C.nComp(), i, j, k, n, {
            Real integral =
                1.0_rt / 12.0_rt *
                (-(A_0_old(i, j, k, n) + R_0_old(i, j, k, n)) +
                 8.0_rt * (A_1_old(i, j, k, n) + R_1_old(i, j, k, n)) +
                 5.0_rt * (A_2_old(i, j, k, n) + R_2_old(i, j, k, n)));

            C(i, j, k, n) = (A_m(i, j, k, n) - A_1_old(i, j, k, n)) -
                            R_2_old(i, j, k, n) + integral;
        });
    } else {
...
void Castro::ca_sdc_compute_C4_radau(
    const Box& bx, Real dt_m, Real dt, Array4<const Real> const& A_m,
    Array4<const Real> const& A_0_old, Array4<const Real> const& A_1_old,
    Array4<const Real> const& A_2_old, Array4<const Real> const& A_3_old,
    Array4<const Real> const& R_0_old, Array4<const Real> const& R_1_old,
    Array4<const Real> const& R_2_old, Array4<const Real> const& R_3_old,
    Array4<Real> const& C, int m_start) {
    // compute the 'C' term for the 4th-order solve with reactions
    // note: this 'C' is cell-averages
    ...
    if (m_start == 0) {
            ...
            Real integral = (dt / dt_m) * (1.0_rt / 1800.0_rt) *
                            ((-35.0_rt * std::sqrt(6.0_rt) + 440.0_rt) *
                             ...
                             (-169.0_rt * std::sqrt(6.0_rt) + 296.0_rt) *
                                 (A_2_old(i, j, k, n) + R_2_old(i, j, k, n)) +
                             (-16.0_rt + 24.0_rt * std::sqrt(6.0_rt)) *
                                 (A_3_old(i, j, k, n) + R_3_old(i, j, k, n)));
        ...
        AMREX_PARALLEL_FOR_4D(bx, C.nComp(), i, j, k, n, {
            ...
            C(i, j, k, n) = (A_m(i, j, k, n) - A_2_old(i, j, k, n)) -
                            R_3_old(i, j, k, n) + integral;
        });
    } else {
        amrex::Abort("error in ca_sdc_compute_C4 -- should not be here");
    }
...
void Castro::ca_sdc_conservative_update(const Box& bx, Real const dt_m,
    ...
    AMREX_PARALLEL_FOR_4D(bx, U_new.nComp(), i, j, k, n, {
        U_new(i, j, k, n) =
            U_old(i, j, k, n) + dt_m * R_new(i, j, k, n) + dt_m * C(i, j, k, n);
    });
}  // end subroutine ca_sdc_conservative_update
#endif

void Castro::ca_sdc_compute_initial_guess(
    const Box& bx, Array4<const Real> const& U_old,
    Array4<const Real> const& U_new, Array4<const Real> const& A_old,
    Array4<const Real> const& R_old, Array4<Real> const& U_guess,
    ...
    if (sdc_iteration == 0) {
        AMREX_PARALLEL_FOR_4D(bx, U_guess.nComp(), i, j, k, n, {
            U_guess(i, j, k, n) = U_old(i, j, k, n) + dt_m * A_old(i, j, k, n) +
                                  dt_m * R_old(i, j, k, n);
        });
    } else {
        AMREX_PARALLEL_FOR_4D(bx, U_guess.nComp(), i, j, k, n,
...
void Castro::ca_store_reaction_state(const Box& bx,
    ...
    AMREX_PARALLEL_FOR_3D(bx, i, j, k, {
        R_store(i, j, k, NumSpec + 1 - 1) =
            R_old(i, j, k, UEDEN) / state(i, j, k, URHO);
