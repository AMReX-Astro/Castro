...
#include <radiation_queries.H>
    ...
    if (filter_lambda_T > 0) {
        if (filter_lambda_S >= filter_lambda_T) {
            amrex::Error(
                "Invalid filter_lambda_S; S must be less than T when T > 0.");
        }
    }
    ...
    if (filter_prim_T > 0) {
        if (filter_prim_S >= filter_prim_T) {
            amrex::Error(
                "Invalid filter_prim_S; S must be less than T when T > 0.");
        }
    }
    ...
    if (Radiation::SolverType == Radiation::MGFLDSolver) {
        ...
        if (test_groups > 0 && test_groups != Radiation::nGroups) {
            amrex::Error(
                "you set the number of groups at runtime, but this does not "
                "match the compiled value");
        }
    ...
    std::cout << "radiation initialized, nGroups = " << Radiation::nGroups
              << std::endl;
}
...
Radiation::Radiation(Amr* Parent, Castro* castro, int restart)
...

    {
        ...
        ca_initradconstants(M_PI, clight, hPlanck, kBoltz, stefbol, Avogadro,
                            convert_MeV_erg, J_is_used);
    ...
    dT = 1.0;
    pp.query("delta_temp", dT);
    surface_average = 2;
    pp.query("surface_average", surface_average);

    // for inner iterations of neutrino J equation
    relInTol = 1.e-4;
    pp.query("relInTol", relInTol);
    if (SolverType == SGFLDSolver || SolverType == MGFLDSolver) {
        absInTol = 0.0;
    } else {
        absInTol = 1.e-4;
    ...
    maxInIter = 30;
    pp.query("maxInIter", maxInIter);
    minInIter = 1;
    pp.query("minInIter", minInIter);
    ...
    if (verbose >= 1 && ParallelDescriptor::IOProcessor()) {
        std::cout << "processors = " << ParallelDescriptor::NProcs()
                  << std::endl;
        ...
        if (SolverType == MGFLDSolver || SolverType == SGFLDSolver) {
            std::cout << "rad_hydro_combined = " << rad_hydro_combined
                      << std::endl;
            std::cout << "comoving = " << comoving << std::endl;
        }
        if (SolverType == MGFLDSolver) {
            std::cout << "fspace_advection_type = " << fspace_advection_type
                      << std::endl;
            std::cout << "do_inelastic_scattering = " << do_inelastic_scattering
                      << std::endl;
        }
    ...
    for (int i = 0; i < BL_SPACEDIM; i++) {
        ...
        if (verbose > 1 && ParallelDescriptor::IOProcessor()) {
            std::cout << "dimension " << i
                      << " rad boundary conditions = " << lo_bc[i] << ", "
                      << hi_bc[i] << std::endl;
        }
    }
    ...
    if (pure_hydro || limiter == 0) {
        if (verbose > 1 && ParallelDescriptor::IOProcessor()) {
            std::cout << "turning off inelastic scattering when (pure_hydro || "
                         "limiter == 0)"
                      << std::endl;
        }
...
void Radiation::regrid(int level, const BoxArray& grids,
    ...
    if (level > 0) {
        ...
        flux_cons[level].reset(
            new FluxRegister(grids, dmap, crse_ratio, level, nGroups));
...
void Radiation::restart(int level, const BoxArray& grids,
                        const DistributionMapping& dmap, const std::string& dir,
                        std::istream& is) {
    //
    ...
    if (Flag == "Present") {
        ...
        if (!dir.empty() && dir[dir.length() - 1] != '/') FullPathName += '/';
        FullPathName += Path;
        //
        // Input conservation flux register.
        //
        const IntVect& crse_ratio = parent->refRatio(level - 1);
        flux_cons_old[level].reset(
            new FluxRegister(grids, dmap, crse_ratio, level, nGroups));
        flux_cons_old[level]->read(FullPathName, is);
    }
}

void Radiation::checkPoint(int level, const std::string& dir, std::ostream& os,
                           VisMF::How how) {
    //
    // With the deferred sync option, we have to restart the rad flux register
    //
...
void Radiation::post_init(int level) { return; }
...
void Radiation::pre_timestep(int level) {
    ...
    if (level < fine_level) {
        ...
        if (!flux_cons[flevel]) {
            ...
            const DistributionMapping& dmap =
                parent->getLevel(flevel).DistributionMap();
            const IntVect& crse_ratio = parent->refRatio(level);
            flux_cons[flevel].reset(
                new FluxRegister(grids, dmap, crse_ratio, flevel, nGroups));
...
void Radiation::compute_exchange(MultiFab& exch, MultiFab& Er, MultiFab& fkp,
                                 int igroup) {
...
#pragma gpu box(bx)
        cexch(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
              BL_TO_FORTRAN_ANYD(exch[mfi]), BL_TO_FORTRAN_ANYD(Er[mfi]),
              BL_TO_FORTRAN_ANYD(fkp[mfi]), sigma, c);
    }
}

void Radiation::compute_eta(MultiFab& eta, MultiFab& etainv, MultiFab& state,
...
#pragma gpu box(bx) sync
                ca_compute_planck(
                    ...
                    BL_TO_FORTRAN_ANYD(state[mfi]), igroup, igroup, 1, dT);
            }
...
#pragma gpu box(bx) sync
            ceta2(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                  BL_TO_FORTRAN_ANYD(eta[mfi]), BL_TO_FORTRAN_ANYD(etainv[mfi]),
                  BL_TO_FORTRAN_N_ANYD(state[mfi], URHO),
                  BL_TO_FORTRAN_ANYD(temp[mfi]), BL_TO_FORTRAN_ANYD(c_v),
                  BL_TO_FORTRAN_ANYD(fkp[mfi]),
                  BL_TO_FORTRAN_N_ANYD(Er[mfi], igroup), dT, delta_t, sigma, c,
                  underrel, lag_planck);
        }
    }
}
...
#pragma gpu box(bx)
        ceup(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
             ...
             BL_TO_FORTRAN_ANYD(frhoes[mfi]), BL_TO_FORTRAN_ANYD(frhoem[mfi]),
             BL_TO_FORTRAN_ANYD(eta[mfi]), BL_TO_FORTRAN_ANYD(etainv[mfi]),
             BL_TO_FORTRAN_ANYD(dflux_old[mfi]),
             BL_TO_FORTRAN_ANYD(dflux_new[mfi]), BL_TO_FORTRAN_ANYD(exch[mfi]),
             delta_t, theta);
    }
...
#pragma omp parallel
#endif
    {
        ...
        for (MFIter mfi(eta, true); mfi.isValid(); ++mfi) {
            ...
            ceupdterm(ARLIM(reg.loVect()), ARLIM(reg.hiVect()), relative_priv,
                      absolute_priv, BL_TO_FORTRAN(frhoes[mfi]),
                      frhoem[mfi].dataPtr(), eta[mfi].dataPtr(),
                      etainv[mfi].dataPtr(), dflux_old[mfi].dataPtr(),
                      dflux_new[mfi].dataPtr(), exch[mfi].dataPtr(),
                      Dterm[mfi].dataPtr(), delta_t, theta);
        }
#ifdef _OPENMP
...
void Radiation::nonconservative_energy_update(
    Real& relative, Real& absolute, MultiFab& frhoes, MultiFab& frhoem,
    MultiFab& eta, MultiFab& etainv, MultiFab& Er_new, MultiFab& dflux_old,
    MultiFab& dflux_new, MultiFab& temp, MultiFab& fkp, MultiFab& state,
    Real delta_t) {
...
#pragma omp parallel
#endif
    {
        ...
        for (MFIter mfi(eta, true); mfi.isValid(); ++mfi) {
            ...
            nceup(ARLIM(reg.loVect()), ARLIM(reg.hiVect()), relative_priv,
                  absolute_priv, BL_TO_FORTRAN(frhoes[mfi]),
                  frhoem[mfi].dataPtr(), eta[mfi].dataPtr(),
                  etainv[mfi].dataPtr(), BL_TO_FORTRAN(Er_new[mfi]),
                  dflux_old[mfi].dataPtr(), dflux_new[mfi].dataPtr(),
                  temp[mfi].dataPtr(), fkp[mfi].dataPtr(), c_v.dataPtr(),
                  BL_TO_FORTRAN(state[mfi]), sigma, c, delta_t, theta);
        }
#ifdef _OPENMP
#pragma omp critical(rad_ceupdterm)
#endif
...
#pragma gpu box(bx)
        cetot(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
              BL_TO_FORTRAN_ANYD(state[mfi]), BL_TO_FORTRAN_ANYD(frhoes[mfi]));
...
#pragma gpu box(bx)
        ca_compute_temp_given_rhoe(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(frhoes[mfi]), BL_TO_FORTRAN_ANYD(state[mfi]), 1);
    }
}
...
#pragma omp parallel
    ...
    for (MFIter mfi(f); mfi.isValid(); ++mfi) {
        ...
        bextrp(BL_TO_FORTRAN_N(f[mfi], indx), ARLIM(reg.loVect()),
               ARLIM(reg.hiVect()));
    }
}

void Radiation::getBndryData(RadBndry& bd, MultiFab& Er, Real time, int level) {
...
void Radiation::getBndryDataMG(MGRadBndry& mgbd, MultiFab& Er, Real time,
                               int level) {
    ...
    if (level == 0) {
        ...
        mgbd.setBndryValues(crse_br, 0, Er, 0, 0, Radiation::nGroups,
                            crse_ratio, rad_bc);
    }
...
void Radiation::getBndryDataMG_ga(MGRadBndry& mgbd, MultiFab& Er, int level) {
    ...
    if (level == 0) {
        ...
        mgbd.setBndryValues(crse_br, 0, Er, 0, 0, 1, crse_ratio, rad_bc);
    }
...
void Radiation::filBndry(BndryRegister& bdry, int level, Real time) {
    ...
    if (!geom.isAnyPeriodic()) {
        ...
        if (time > new_time - eps) {
            ...
            bdry.linComb(a, S_old, 0, b, S_new, 0, 0, Radiation::nGroups,
                         n_ghost);
        }
    } else {
...
#pragma gpu box(reg) sync
    ca_compute_c_v(AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                   BL_TO_FORTRAN_ANYD(c_v), BL_TO_FORTRAN_ANYD(temp),
                   BL_TO_FORTRAN_ANYD(state));
}

void Radiation::get_frhoe(MultiFab& frhoe, MultiFab& state) {
#ifdef _OPENMP
#pragma omp parallel
#endif
...
#pragma gpu box(reg)
        cfrhoe(AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
               BL_TO_FORTRAN_ANYD(frhoe[si]), BL_TO_FORTRAN_ANYD(state[si]));
    }
}

// temp contains frhoe on input:

void Radiation::get_planck_and_temp(MultiFab& fkp, MultiFab& temp,
                                    MultiFab& state, int igroup, Real delta_t) {
...
#pragma gpu box(bx)
        ca_compute_temp_given_rhoe(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(temp[mfi]), BL_TO_FORTRAN_ANYD(state[mfi]), 0);

#pragma gpu box(bx)
        ca_compute_planck(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_N_ANYD(fkp[mfi], igroup),
            BL_TO_FORTRAN_ANYD(state[mfi]), igroup, igroup, 1, 0.0);

#pragma gpu box(bx) sync
        nfloor(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
               BL_TO_FORTRAN_ANYD(temp[mfi]), temp[mfi].nComp());
    }
}
...
void Radiation::get_rosseland(MultiFab& kappa_r, AmrLevel* castro, int igroup) {
...
#pragma gpu box(bx)
            cfrhoe(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                   BL_TO_FORTRAN_ANYD(frhoe), BL_TO_FORTRAN_ANYD(state[mfi]));

#pragma gpu box(bx)
            ca_compute_temp_given_rhoe(
                AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                BL_TO_FORTRAN_ANYD(frhoe), BL_TO_FORTRAN_ANYD(state[mfi]), 1);

#pragma gpu box(bx)
            ca_compute_rosseland(
                AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                BL_TO_FORTRAN_N_ANYD(kappa_r[mfi], igroup),
                BL_TO_FORTRAN_ANYD(state[mfi]), igroup, igroup, 1);
        }
    }
}
...
void Radiation::update_rosseland_from_temp(MultiFab& kappa_r, MultiFab& temp,
                                           MultiFab& state,
                                           const Geometry& geom, int igroup) {
...
#pragma omp parallel
    ...
    for (MFIter mfi(state, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        AMREX_PARALLEL_FOR_3D(
            bx, i, j, k, { state_arr(i, j, k, UTEMP) = temp_arr(i, j, k); });

#pragma gpu box(bx)
        ca_compute_rosseland(AMREX_INT_ANYD(bx.loVect()),
...
void Radiation::SGFLD_compute_rosseland(MultiFab& kappa_r,
...
#pragma gpu box(kbox)
        ca_compute_rosseland(AMREX_INT_ANYD(kbox.loVect()),
                             AMREX_INT_ANYD(kbox.hiVect()),
                             BL_TO_FORTRAN_ANYD(kappa_r[mfi]),
                             BL_TO_FORTRAN_ANYD(state[mfi]), 0, 0, 1);
    }
}

void Radiation::SGFLD_compute_rosseland(FArrayBox& kappa_r,
...
#pragma gpu box(kbox) sync
    ca_compute_rosseland(
        ...
        BL_TO_FORTRAN_ANYD(kappa_r), BL_TO_FORTRAN_ANYD(state), 0, 0, 1);
}

void Radiation::deferred_sync_setup(int crse_level) {
    BL_PROFILE("Radiation::deferred_sync_setup");
    if (verbose && ParallelDescriptor::IOProcessor()) {
        std::cout << "Radiation deferred sync setup for coarse level "
                  << crse_level << "..." << std::endl;
    }
...
void Radiation::deferred_sync(int level, MultiFab& rhs, int indx) {
    ...
    if (level < parent->maxLevel() && flux_cons_old[level + 1]) {
        ...
        if (level + 1 < parent->maxLevel() && flux_cons_old[level + 2]) {
                ...
                for (int flev = level + 1; flev < parent->maxLevel(); flev++) {
                    ...
                    if (flux_cons_old[flev + 1]) {
                        ...
                        if (fgrids.contains(ffgr)) {
                            ...
                            const DistributionMapping& dm_ff_sync =
                                ff_sync.DistributionMap();
                            ...
                            amrex::average_down(flev_data, rhs_tmp, 0, 1,
                                                ref_rat);
    ...
    for (int flev = level; flev > 0; flev--) {
        ...
        if (flux_cons_old[flev] && delta_t_old[flev - 1] > 0.0) {
            ...
            if (fgrids == old_boxes) {
                ...
                const DistributionMapping& dm_crse_sync_flux =
                ...
                for (int dir = 0; dir < BL_SPACEDIM; dir++) {
                    ...
                    for (FabSetIter fsi(ref_sync_flux[lo_face]); fsi.isValid();
                         ++fsi) {

                        rfface(
                            BL_TO_FORTRAN(ref_sync_flux[lo_face][fsi]),
                            BL_TO_FORTRAN_N(crse_sync_flux[lo_face][fsi], indx),
                            dir, ref_rat.getVect());
                    }

                    for (FabSetIter fsi(ref_sync_flux[hi_face]); fsi.isValid();
                         ++fsi) {

                        rfface(
                            BL_TO_FORTRAN(ref_sync_flux[hi_face][fsi]),
...
void Radiation::reflux(int level) { return; }

// Computes the scaled gradient for use in flux limiters

void Radiation::scaledGradient(int level, Array<MultiFab, BL_SPACEDIM>& R,
                               MultiFab& kappa_r, int kcomp, MultiFab& Er,
                               int igroup, int limiter, int nGrow_Er,
                               int Rcomp) {
...
#pragma gpu box(bx)
                scgrd(AMREX_INT_ANYD(nbx.loVect()),
                      AMREX_INT_ANYD(nbx.hiVect()),
                      BL_TO_FORTRAN_N_ANYD(R[idim][mfi], Rcomp), idim,
                      AMREX_REAL_ANYD(dx),
                      BL_TO_FORTRAN_N_ANYD(kappa_r[mfi], kcomp),
                      BL_TO_FORTRAN_ANYD(Erborder[mfi]), include_cross_terms);
            }
        }
    }
}
...
void Radiation::fluxLimiter(int level, Array<MultiFab, BL_SPACEDIM>& lambda,
...
#pragma gpu box(bx)
            flxlim(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                   BL_TO_FORTRAN_N_ANYD(lambda[idim][mfi], lamcomp), limiter);
        }
    }
}

void Radiation::get_rosseland_v_dcf(MultiFab& kappa_r, MultiFab& v,
                                    MultiFab& dcf, Real delta_t, Real c,
...
#pragma gpu box(reg) sync
            cfrhoe(AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                   BL_TO_FORTRAN_ANYD(temp), BL_TO_FORTRAN_ANYD(S[mfi]));

#pragma gpu box(reg) sync
            ca_compute_temp_given_rhoe(
                AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                BL_TO_FORTRAN_ANYD(temp), BL_TO_FORTRAN_ANYD(S[mfi]), 0);
...
#pragma gpu box(reg) sync
            ca_compute_rosseland(AMREX_INT_ANYD(reg.loVect()),
                                 AMREX_INT_ANYD(reg.hiVect()),
                                 BL_TO_FORTRAN_N_ANYD(kappa_r[mfi], igroup),
                                 BL_TO_FORTRAN_ANYD(S[mfi]), igroup, igroup, 1);

            kp.resize(reg);
#pragma gpu box(reg) sync
            ca_compute_planck(
                AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                BL_TO_FORTRAN_ANYD(kp), BL_TO_FORTRAN_ANYD(S[mfi]), igroup,
                igroup, 1, 0.0);
...
#pragma gpu box(reg) sync
            ca_compute_planck(
                AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                BL_TO_FORTRAN_ANYD(kp2), BL_TO_FORTRAN_ANYD(S[mfi]), igroup,
                igroup, 1, 0.0);

            S[mfi].plus<RunOn::Device>(-dT, UTEMP, 1);
            Gpu::synchronize();

            ca_get_v_dcf(reg.loVect(), reg.hiVect(), BL_TO_FORTRAN(Er[mfi]),
                         BL_TO_FORTRAN(S[mfi]), BL_TO_FORTRAN(temp),
                         BL_TO_FORTRAN(c_v), BL_TO_FORTRAN(kappa_r[mfi]),
                         BL_TO_FORTRAN(kp), BL_TO_FORTRAN(kp2), &dT, &delta_t,
                         &sigma, &c, BL_TO_FORTRAN(v[mfi]),
                         BL_TO_FORTRAN(dcf[mfi]));
        }
    }
}

void Radiation::update_dcf(MultiFab& dcf, MultiFab& etainv, MultiFab& kp,
                           MultiFab& kr, const Geometry& geom) {
#ifdef _OPENMP
#pragma omp parallel
#endif
    for (MFIter mfi(dcf, true); mfi.isValid(); ++mfi) {
        const Box& bx = mfi.growntilebox();
        ca_update_dcf(bx.loVect(), bx.hiVect(), BL_TO_FORTRAN(dcf[mfi]),
                      BL_TO_FORTRAN(etainv[mfi]), BL_TO_FORTRAN(kp[mfi]),
...
void Radiation::set_current_group(int igroup) { current_group_number = igroup; }
...
#pragma omp parallel
    ...
    for (MFIter mfi(State, true); mfi.isValid(); ++mfi) {
        ...
        ca_filt_prim(bx.loVect(), bx.hiVect(), BL_TO_FORTRAN(S_fp[mfi]),
                     BL_TO_FORTRAN(State[mfi]), BL_TO_FORTRAN(mask[mfi]),
                     &filter_prim_T, &filter_prim_S, domain_lo, domain_hi, dx,
                     xlo, prob_lo, &time, &level);
    }
}
