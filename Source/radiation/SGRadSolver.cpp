...
void Radiation::single_group_update(int level, int iteration, int ncycle) {
    ...
    if (verbose && ParallelDescriptor::IOProcessor()) {
        std::cout << "Radiation implicit update, level " << level << "..."
                  << std::endl;
    }
...
#pragma gpu box(reg)
        cfrhoe(AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
               BL_TO_FORTRAN_ANYD(frhoem[mfi]), BL_TO_FORTRAN_ANYD(S_new[mfi]));
    }
    ...
    MultiFab etainv(grids, dmap, 1,
                    0);  // this is 1-eta, to avoid loss of accuracy
    ...
    do {
        ...
        compute_eta(eta, etainv, S_new, temp, fkp, Er_new, delta_t, c, underrel,
                    it > update_planck);
...

        {
            ...
            solver->levelRhs(level, rhs, temp, fkp, eta, etainv, frhoem, frhoes,
                             dflux_old, Er_old, dflux_new, delta_t, sigma, 0.0,
                             1.0, NULL);
        ...
        if (use_conservative_form) {
            ...
            if (has_dcoefs) {
                internal_energy_update(relative, absolute, frhoes, frhoem, eta,
                                       etainv, dflux_old, dflux_new, exch,
                                       Dterm, delta_t);
            } else {
                internal_energy_update(relative, absolute, frhoes, frhoem, eta,
                                       etainv, dflux_old, dflux_new, exch,
            ...
            nonconservative_energy_update(relative, absolute, frhoes, frhoem,
                                          eta, etainv, Er_new, dflux_old,
                                          dflux_new, temp, fkp, S_new, delta_t);
            if (has_dcoefs) {
                amrex::Error(
                    "Radiation::single_group_update: must do conservative "
                    "energy update when has_dcoefs is true");
            }
        }
    ...
    if (it == maxiter && (relative > reltol && absolute > abstol)) {
    ...
    if (flux_in) {
        for (OrientationIter face; face; ++face) {
            ...
            (*flux_cons[level + 1])[ori].linComb(1.0, -1.0, (*flux_in)[ori],
                                                 group, group, 1);
        }
    }
    ...
    if (plot_lab_Er || plot_lab_flux || plot_com_flux) {
        ...
        if (plot_lab_flux) {
            if (comoving) {
                ...
                MultiFab::Copy(*plotvar[level], flx, 0, icomp_lab_Fr,
                               BL_SPACEDIM, 0);
            }
        }
        ...
        if (plot_com_flux) {
            if (comoving) {
                MultiFab::Copy(*plotvar[level], flx, 0, icomp_com_Fr,
                               BL_SPACEDIM, 0);
            } else {
