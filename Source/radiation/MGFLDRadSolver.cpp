...
void Radiation::MGFLD_implicit_update(int level, int iteration, int ncycle) {
    ...
    if (verbose) {
        amrex::Print() << "Radiation MGFLD implicit update, level " << level
                       << "..." << std::endl;
    }
    ...
    AmrLevel::FillPatch(*castro, S_new, ngrow, time, State_Type, 0,
                        S_new.nComp(), 0);

    Array<MultiFab, BL_SPACEDIM> lambda;
    if (limiter > 0) {
        for (int idim = 0; idim < BL_SPACEDIM; idim++) {
            lambda[idim].define(castro->getEdgeBoxArray(idim), dmap, nGroups,
                                0);
        }
        ...
        if (inner_update_limiter == -1) {
            ...
            for (FillPatchIterator fpi(*castro, S_lag, ngrow, oldtime,
                                       State_Type, 0, S_lag.nComp());
                 fpi.isValid(); ++fpi) {
            ...
            for (int igroup = 0; igroup < nGroups; ++igroup) {
                scaledGradient(level, lambda, kpr_lag, igroup, Er_lag, igroup,
                               limiter, 1, igroup);
...
#pragma omp parallel
    ...
    do {
        ...
        if (limiter > 0 && inner_update_limiter == 0) {
            ...
            for (int igroup = 0; igroup < nGroups; ++igroup) {
                scaledGradient(level, lambda, kappa_r, igroup, Er_star, igroup,
                               limiter, 1, igroup);
                // lambda now contains scaled gradient
                fluxLimiter(level, lambda, limiter, igroup);
                // lambda now contains flux limiter
        ...
        compute_etat(etaT, etaTz, eta1, djdT, dkdT, dedT, Er_star, rho, delta_t,
                     ptc_tau);
        ...
        do {
            ...
            if (limiter > 0 && inner_update_limiter > 0) {
                if (innerIteration <= inner_update_limiter) {
                    ...
                    for (int igroup = 0; igroup < nGroups; ++igroup) {
                        scaledGradient(level, lambda, kappa_r, igroup, Er_pi,
                                       igroup, limiter, 1, igroup);
                        // lambda now contains scaled gradient
                        fluxLimiter(level, lambda, limiter, igroup);
                        // lambda now contains flux limiter
                    }
            ...
            for (int igroup = 0; igroup < nGroups; ++igroup) {
                ...
                solver->levelACoeffs(level, kappa_p, delta_t, c, igroup,
                                     ptc_tau);

                int lamcomp = (limiter == 0) ? 0 : igroup;
                solver->levelBCoeffs(level, lambda, kappa_r, igroup, c,
                                     lamcomp);
                ...
                {  // src and rhd block
                    ...
                    solver->levelRhs(level, rhs, jg, mugT, coupT, etaT, Er_step,
                                     rhoe_step, Er_star, rhoe_star, delta_t,
                                     igroup, it, ptc_tau);
                ...
                if (icomp_flux >= 0)
                    solver->levelFluxFaceToCenter(level, Flux, *flxcc,
                                                  icomp_flux + igroup);

            }  // end loop over groups

            // Check for convergence *before* acceleration step:
            check_convergence_er(relative_in, absolute_in, error_er, Er_new,
                                 Er_pi, kappa_p, etaTz, temp_new, delta_t);

            if (verbose >= 2) {
                int oldprec = std::cout.precision(3);
                amrex::Print()
                    << "Outer = " << it << ", Inner = " << innerIteration
                    << ", inner err =  " << std::setw(8) << relative_in
                    << " (rel),  " << std::setw(8) << absolute_in << " (abs)"
                    << std::endl;
            ...
            if (!inner_converged) {
                ...
                if (accel_allowed) {
                    if (accelerate == 1) {
                        local_accel(Er_new, Er_pi, kappa_p, etaT, mugT, delta_t,
                                    ptc_tau);
                    } else if (accelerate == 2) {
                        gray_accel(Er_new, Er_pi, kappa_p, kappa_r, etaT, eta1,
                                   mugT, lambda, solver, mgbd, grids, level,
                                   time, delta_t, ptc_tau);
                    }
                }
            }
        ...
        update_matter(rhoe_new, temp_new, Er_new, Er_pi, rhoe_star, rhoe_step,
                      etaT, etaTz, eta1, coupT, kappa_p, jg, mugT, S_new, level,
                      delta_t, ptc_tau, it, conservative_update);
        ...
        check_convergence_matt(rhoe_new, rhoe_star, rhoe_step, Er_new, temp_new,
                               temp_star, rho, kappa_p, jg, dedT, rel_rhoe,
                               abs_rhoe, rel_FT, abs_FT, rel_T, abs_T, delta_t);
        ...
        switch (convergence_check_type) {
            ...
            default:
                ...
                if (conservative_update) {
                    ...
                    relative_out =
                        (relative_out > rel_FT) ? relative_out : rel_FT;
                }
                absolute_out = abs_T;
        }
        ...
        if (!converged && it > n_bisect) {
            bisect_matter(rhoe_new, temp_new, rhoe_star, temp_star, S_new,
                          grids, level);
    ...
    if (verbose == 1) {
        ...
        amrex::Print() << "Update Errors for      rhoe,        FT,         T"
                       << std::endl;
        amrex::Print() << "       Relative = " << std::setw(9) << rel_rhoe
                       << ", " << std::setw(9) << rel_FT << ", " << std::setw(9)
                       << rel_T << std::endl;
        amrex::Print() << "       Absolute = " << std::setw(9) << abs_rhoe
                       << ", " << std::setw(9) << abs_FT << ", " << std::setw(9)
                       << abs_T << std::endl;
        std::cout.precision(oldprec);
    }
    ...
    if (flux_in) {
        for (OrientationIter face; face; ++face) {
            ...
            (*flux_cons[level + 1])[ori].linComb(1.0, -1.0, (*flux_in)[ori], 0,
                                                 0, nGroups);
        }
    }
    ...
    if (plot_lab_flux) {
        save_lab_flux_in_plotvar(level, S_new, lambda, Er_new, *flxcc,
                                 icomp_flux);
    }
