...
RadSolve::RadSolve(Amr* Parent, int level, const BoxArray& grids,
                   const DistributionMapping& dmap)
    : parent(Parent) {
    read_params();

    if (radsolve::level_solver_flag < 100) {
        hd.reset(new HypreABec(grids, dmap, parent->Geom(level),
                               radsolve::level_solver_flag));
    } else {
        if (radsolve::use_hypre_nonsymmetric_terms == 0) {
            hm.reset(
                new HypreMultiABec(level, level, radsolve::level_solver_flag));
            hm->addLevel(level, parent->Geom(level), grids, dmap,
                         IntVect::TheUnitVector());
            hm->buildMatrixStructure();
        } else {
            hem.reset(new HypreExtMultiABec(level, level,
                                            radsolve::level_solver_flag));
...
#include "radsolve_queries.H"
    ...
    if (BL_SPACEDIM == 1) {
        if (radsolve::level_solver_flag == 1) {
            amrex::Error(
                "radsolve.level_solver_flag = 1 is not supported in 1D");
        }
    }
    ...
    if (Radiation::SolverType == Radiation::SGFLDSolver &&
        ...
        if (radsolve::level_solver_flag < 100) {
            amrex::Error(
                "To do Lorentz term implicitly level_solver_flag must be >= "
                "100.");
        }
    }
    ...
    if (Radiation::SolverType == Radiation::MGFLDSolver &&
        ...
        if (radsolve::level_solver_flag < 100) {
            amrex::Error(
                "When accelerate is 2, level_solver_flag must be >= 100.");
        }
    }
}

void RadSolve::levelInit(int level) { BL_PROFILE("RadSolve::levelInit"); }
...
void RadSolve::levelACoeffs(int level, MultiFab& fkp, MultiFab& eta,
                            MultiFab& etainv, Real c, Real delta_t,
                            Real theta) {
...
#pragma gpu box(bx)
        lacoef(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
               BL_TO_FORTRAN_ANYD(acoefs[mfi]), BL_TO_FORTRAN_ANYD(fkp[mfi]),
               BL_TO_FORTRAN_ANYD(eta[mfi]), BL_TO_FORTRAN_ANYD(etainv[mfi]),
               AMREX_REAL_ANYD(dx), c, delta_t, theta);
    }
...
void RadSolve::levelSPas(int level, Array<MultiFab, BL_SPACEDIM>& lambda,
                         int igroup, int lo_bc[3], int hi_bc[3]) {
    const BoxArray& grids = parent->boxArray(level);
    const DistributionMapping& dmap = parent->DistributionMap(level);
    const Geometry& geom = parent->Geom(level);
...
#pragma omp parallel
    ...
    for (MFIter mfi(spa, true); mfi.isValid(); ++mfi) {
        ...
        if (nexttoboundary) {
            ca_spalpha(reg.loVect(), reg.hiVect(), BL_TO_FORTRAN(spa[mfi]),
...
void RadSolve::levelBCoeffs(int level, Array<MultiFab, BL_SPACEDIM>& lambda,
                            MultiFab& kappa_r, int kcomp, Real c, int lamcomp) {
    ...
    for (int idim = 0; idim < BL_SPACEDIM; ++idim) {
        ...
        MultiFab bcoefs(lambda[idim].boxArray(), lambda[idim].DistributionMap(),
                        1, 0);
...
#pragma gpu box(bx)
            bclim(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                  ...
                  BL_TO_FORTRAN_N_ANYD(lambda[idim][mfi], lamcomp), idim,
                  BL_TO_FORTRAN_N_ANYD(kappa_r[mfi], kcomp), c,
                  AMREX_REAL_ANYD(dx));
        }
...
#pragma gpu box(bx)
            ca_compute_dcoefs(
                ...
                BL_TO_FORTRAN_ANYD(vel[mfi]), BL_TO_FORTRAN_ANYD(dcf[mfi]),
...
void RadSolve::levelRhs(int level, MultiFab& rhs, MultiFab& temp, MultiFab& fkp,
                        MultiFab& eta, MultiFab& etainv, MultiFab& rhoem,
                        MultiFab& rhoes, MultiFab& dflux_old, MultiFab& Er_old,
                        MultiFab& Edot, Real delta_t, Real sigma, Real c,
                        Real theta, FluxRegister* fine_corr, Real scale,
...
#pragma gpu box(bx)
        lrhs(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
             BL_TO_FORTRAN_ANYD(rhs[mfi]), BL_TO_FORTRAN_ANYD(temp[mfi]),
             BL_TO_FORTRAN_ANYD(fkp[mfi]), BL_TO_FORTRAN_ANYD(eta[mfi]),
             BL_TO_FORTRAN_ANYD(etainv[mfi]), BL_TO_FORTRAN_ANYD(rhoem[mfi]),
             BL_TO_FORTRAN_ANYD(rhoes[mfi]), BL_TO_FORTRAN_ANYD(dflux_old[mfi]),
             BL_TO_FORTRAN_N_ANYD(Er_old[mfi], 0),
             BL_TO_FORTRAN_ANYD(Edot[mfi]), delta_t, AMREX_REAL_ANYD(dx), sigma,
             c, theta);
    }
}

void RadSolve::levelSolve(int level, MultiFab& Er, int igroup, MultiFab& rhs,
...
void RadSolve::levelFluxFaceToCenter(int level,
        ...
        for (int idim = 0; idim < BL_SPACEDIM; idim++) {
            for (MFIter mfi(flx, true); mfi.isValid(); ++mfi) {
                ...
                ca_flux_face2center(ccbx.loVect(), ccbx.hiVect(),
                                    ...
                                    BL_TO_FORTRAN(Flux[idim][mfi]), r.dataPtr(),
                                    &rlo, &rhi, &nflx, &idim, &iflx);
            }
        }
    }
}

void RadSolve::levelFlux(int level, Array<MultiFab, BL_SPACEDIM>& Flux,
    ...
    Erborder.FillBoundary(
        parent->Geom(level)
            .periodicity());  // zeroes left in off-level boundaries
...
#pragma gpu box(bx)
            set_abec_flux(
                AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()), n,
                BL_TO_FORTRAN_ANYD(Erborder[mfi]),
                BL_TO_FORTRAN_ANYD(bcoef[mfi]), radsolve::beta,
                AMREX_REAL_ANYD(dx), BL_TO_FORTRAN_ANYD(Flux[n][mfi]));
        }
    }
...
void RadSolve::levelFluxReg(int level, FluxRegister* flux_in,
...
void RadSolve::levelDterm(int level, MultiFab& Dterm, MultiFab& Er,
                          int igroup) {
    ...
    for (int n = 0; n < BL_SPACEDIM; n++) {
        ...
        for (MFIter fi(dcoef, true); fi.isValid(); ++fi) {
            ...
            ca_set_dterm_face(bx.loVect(), bx.hiVect(),
                              ...
                              BL_TO_FORTRAN(Dterm_face[n][fi]), dx, &n);
        }
    }
...
#pragma omp parallel
#endif
    {
        ...
        if (geom.IsSPHERICAL()) {
            for (MFIter fi(Dterm_face[0]); fi.isValid();
                 ++fi) {  // omp over boxes
                int i = fi.index();
                ...
                sphe(re.dataPtr(), s.dataPtr(), 0, ARLIM(dbox.loVect()),
                     ARLIM(dbox.hiVect()), dx);
...
#pragma omp barrier
        ...
        for (MFIter fi(Dterm, true); fi.isValid(); ++fi) {
            ...
            int nf = 1;
            int nc = 1;
            ca_face2center(bx.loVect(), bx.hiVect(), scomp, dcomp, ncomp, nf,
                           nc,
...
// <MGFLD routines>
void RadSolve::computeBCoeffs(MultiFab& bcoefs, int idim, MultiFab& kappa_r,
                              int kcomp, MultiFab& lambda, int lamcomp, Real c,
                              const Geometry& geom) {
...
#pragma gpu box(bx)
        bclim(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
              BL_TO_FORTRAN_ANYD(bcoefs[mfi]),
              BL_TO_FORTRAN_N_ANYD(lambda[mfi], lamcomp), idim,
              BL_TO_FORTRAN_N_ANYD(kappa_r[mfi], kcomp), c,
              AMREX_REAL_ANYD(dx));
    }
}

void RadSolve::levelACoeffs(int level, MultiFab& kpp, Real delta_t, Real c,
                            int igroup, Real ptc_tau) {
...
#pragma gpu box(bx)
        lacoefmgfld(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                    ...
                    BL_TO_FORTRAN_N_ANYD(kpp[mfi], igroup), AMREX_REAL_ANYD(dx),
                    dt_ptc, c);
    }
...
void RadSolve::levelRhs(int level, MultiFab& rhs, const MultiFab& jg,
                        const MultiFab& mugT, const MultiFab& coupT,
                        const MultiFab& etaT, const MultiFab& Er_step,
                        const MultiFab& rhoe_step, const MultiFab& Er_star,
                        const MultiFab& rhoe_star, Real delta_t, int igroup,
                        int it, Real ptc_tau) {
...
#pragma gpu box(bx)
        ca_compute_rhs(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(rhs[ri]), BL_TO_FORTRAN_ANYD(jg[ri]),
            BL_TO_FORTRAN_ANYD(mugT[ri]), BL_TO_FORTRAN_ANYD(coupT[ri]),
            BL_TO_FORTRAN_ANYD(etaT[ri]), BL_TO_FORTRAN_ANYD(Er_step[ri]),
            BL_TO_FORTRAN_ANYD(rhoe_step[ri]), BL_TO_FORTRAN_ANYD(Er_star[ri]),
            BL_TO_FORTRAN_ANYD(rhoe_star[ri]), AMREX_REAL_ANYD(dx), delta_t,
            igroup, ptc_tau);

#pragma gpu box(bx)
        ca_rad_source(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                      BL_TO_FORTRAN_ANYD(rhs[ri]), AMREX_REAL_ANYD(dx), delta_t,
                      time, igroup);
    }
}
...
void RadSolve::getCellCenterMetric(const Geometry& geom, const Box& reg,
    ...
    if (geom.IsCartesian()) {
        ...
        const Real* dx = geom.CellSize();
        sphc(r.dataPtr(), s.dataPtr(), ARLIM(reg.loVect()), ARLIM(reg.hiVect()),
             dx);
    }
}
...
void RadSolve::getEdgeMetric(int idim, const Geometry& geom, const Box& edgebox,
    ...
    if (geom.IsCartesian()) {
        ...
        const Real* dx = geom.CellSize();
        sphe(r.dataPtr(), s.dataPtr(), idim, ARLIM(edgebox.loVect()),
             ARLIM(edgebox.hiVect()), dx);
    }
}
