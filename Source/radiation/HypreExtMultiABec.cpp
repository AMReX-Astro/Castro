...
void HypreExtMultiABec::d1Coefficients(int level, const MultiFab& d1, int dir) {
    ...
    if (!d1coefs[level]) {
        ...
        for (int i = 0; i < BL_SPACEDIM; i++) {
            (*d1coefs[level])[i].define(grids[level], dmap[level], ncomp,
                                        ngrow);
...
static void FaceValue(AuxVarBox& evalue, AuxVarBox& cintrp, const Mask& msk,
                      const Box& reg, const IntVect& vin, int r, int bho,
                      int flevel) {
    if (bho == 1) {
...
void HypreExtMultiABec::loadMatrix() {
    ...
    if (0 && verbose >= 1 && ParallelDescriptor::IOProcessor()) {
        std::cout << "In HypreExtMultiABec::loadMatrix(), the multipliers are:"
                  << std::endl;
...
#pragma gpu box(reg)
                    hma2c(AMREX_INT_ANYD(reg.loVect()),
            ...
            if (ccoefs[level]) {
                for (int idim = 0; idim < BL_SPACEDIM; idim++) {
#pragma gpu box(reg)
                    hmcc(AMREX_INT_ANYD(reg.loVect()),
                         AMREX_INT_ANYD(reg.hiVect()),
                         BL_TO_FORTRAN_ANYD(matfab),
                         BL_TO_FORTRAN_ANYD((*ccoefs[level])[idim][mfi]), gamma,
            ...
            if (d1coefs[level]) {
                for (int idim = 0; idim < BL_SPACEDIM; idim++) {
#pragma gpu box(reg)
                    hmd1c(AMREX_INT_ANYD(reg.loVect()),
                          AMREX_INT_ANYD(reg.hiVect()),
                          BL_TO_FORTRAN_ANYD(matfab),
                          BL_TO_FORTRAN_ANYD((*d1coefs[level])[idim][mfi]),
                          delta1, AMREX_REAL_ANYD(geom[level].CellSize()),
                          idim);
                }
            }
            ...
            if (d2coefs[level]) {
                for (int idim = 0; idim < BL_SPACEDIM; idim++) {
#pragma gpu box(reg)
                    hmd2c(AMREX_INT_ANYD(reg.loVect()),
                          AMREX_INT_ANYD(reg.hiVect()),
                          BL_TO_FORTRAN_ANYD(matfab),
                          BL_TO_FORTRAN_ANYD((*d2coefs[level])[idim][mfi]),
                          delta2, AMREX_REAL_ANYD(geom[level].CellSize()),
                          idim);
                }
            }
            ...
            for (int s = 0; s < size; s++) {
                ...
                HYPRE_SStructMatrixAddToBoxValues(A, part, loV(reg), hiV(reg),
                                                  0, 1, &stencil_indices[s],
                                                  mat_tmp);
            }
        }
        ...
        for (OrientationIter oitr; oitr; oitr++) {
            ...
            for (int i = entry.firstLocal(); entry.isValid(i);
                ...
                if (grids[level][i][ori] == domain[ori] ||
                    level == crse_level) {
                    ...
                    for (IntVect v = reg.smallEnd(); v <= reg.bigEnd();
                        ...
                        if (msk(v + vin) != RadBndryData::covered) {
                            ...
                            if (d1coefs[level]) {
                                Real fac = delta1 * of2h *
                                           (*d1coefs[level])[idir][i](v);
                                entry(ori, i)(v).push(level, v, fac);
                                entry(ori, i)(v).push(level, v + vin, -fac);
                            }
                            if (d2coefs[level]) {
                                Real fac = delta2 * of2h *
                                           (*d2coefs[level])[idir][i](v + ves);
                                entry(ori, i)(v).push(level, v, fac);
                                entry(ori, i)(v).push(level, v + vin, -fac);
                            }
                            ...
                            if (reg[ori] == domain[ori] &&
                                bd[level]->mixedBndry(ori)) {
                                bct =
                                    (*(bd[level]->bndryTypes(ori)[i]))(v + vin);
                            }
                            if (bct == LO_DIRICHLET) {
                                if (bho == 1) {
                                    evalue(ori, i)(v + vin).push(
                                        level, v,
                                        (bcl * th2) / (h * (bcl + h2)));
                                    evalue(ori, i)(v + vin).push(
                                        level, v - vin,
                                        (-bcl * h2) / (h * (bcl + th2)));
                                    (*ederiv[level])(ori, i)(v + vin).push(
                                        level, v,
                                        (bcl - th2) / (h * (bcl + h2)));
                                    (*ederiv[level])(ori, i)(v + vin).push(
                                        level, v - vin,
                                ...
                                if (ccoefs[level]) {
                                    ...
                                    entry(ori, i)(v).push(
                                        level, v,
                                        fac * (*ccoefs[level])[idir][i](v + ves,
                                                                        i0));
                                    // upwinding from exterior:
                                    entry(ori, i)(v).push(
                                        level, v,
                                        fac * (bcl - h2) / (bcl + h2) *
                                            (*ccoefs[level])[idir][i](v + ves,
                                                                      i1));
                                }
                                if (d1coefs[level]) {
                                    Real fac = 0.5 * delta1 * ofac *
                                               (*d1coefs[level])[idir][i](v);
                                    entry(ori, i)(v).push(
                                ...
                                Real xi =
                                    0.0;  // xi should be passed in through RadBndry?
                                //Real tmp = beta / h; // already done in HypreMultiABec
                                Real tmp = 0.0;
                                Real fac =
                                    0.5 *
                                    (tmp - 0.5 * (delta1 + delta2) * ofac * xi);
                                entry(ori, i)(v).push(&evalue(ori, i)(v + vin),
                                                      cmult * fac);
                            } else {
                                amrex::Error(
                                    "HypreExtMultiABec: unsupported boundary "
                                    "type");
                            }
                        }
                    }
        ...
        if (level > crse_level) {
            ...
            for (OrientationIter oitr; oitr; ++oitr) {
                ...
                ves = (ori.isLow() ? ves : vin);   // edge shift vector
                Real h =
                    geom[level].CellSize(idir);  // normal fine grid spacing
                Real ofac = (ori.isLow() ? -1.0 : 1.0);
                Real of2h = ofac * 0.5 / h;
                for (int i = cintrp[level]->firstLocal();
                     cintrp[level]->isValid(i);
                     i = cintrp[level]->nextLocal(i)) {
                    Box reg = amrex::adjCell(grids[level][i], ori);
                    const Mask& msk = bd[level]->bndryMasks(ori, i);
                    FaceValue(evalue(ori, i), (*cintrp[level])(ori, i), msk,
                              reg, vin, rat[idir], bho, level);
                    reg.shift(-vin);  // fine interior cells
                    for (IntVect v = reg.smallEnd(); v <= reg.bigEnd();
                         reg.next(v)) {
        ...
        for (OrientationIter oitr; oitr; ++oitr) {
            ...
            int idir = ori.coordDir();
            if (a2coefs[level]) {
                c_entry[level]->loadFaceData(ori, (*a2coefs[level])[idir], 0,
                                             pc[0], 1);
            }
            if (ccoefs[level]) {
                // Remove this assertion when upwinding of ccoefs term
                ...
                // not done yet
                c_entry[level]->loadFaceData(ori, (*ccoefs[level])[idir], 0,
                                             pc[1], 1);
            }
            if (d2coefs[level]) {
                c_entry[level]->loadFaceData(ori, (*d2coefs[level])[idir], 0,
                                             pc[2], 1);
            }
            IntVect vin = amrex::BASISV(idir), ves;
            vin = (ori.isLow() ? -vin : vin);  // outward normal unit vector
            ...
            for (int i = c_entry[level]->firstLocal();
                ...
                for (int j = 0; j < (*c_entry[level]).size(ori, i); j++) {
                    ...
                    FaceValue(c_evalue(ori, i, j),
                              (*c_cintrp[level])(ori, i, j), msk, reg, vin,
                              rat[idir], bho, level);
...
void HypreExtMultiABec::loadLevelVectors(int level, MultiFab& dest, int icomp,
...
void HypreExtMultiABec::loadLevelVectorB(int level,
                                         ...
                                         BC_Mode inhom) {
    {
        ...
        if (verbose >= 1 && first && ParallelDescriptor::IOProcessor()) {
            first = 0;
            std::cout << "In HypreExtMultiABec::loadLevelVectorB(), the "
                         "multipliers are:"
                      << std::endl;
            std::cout << "  HypreExtMultiABec::alpha  = " << alpha << std::endl;
            std::cout << "  HypreExtMultiABec::alpha2 = " << alpha2
                      << std::endl;
            std::cout << "  HypreExtMultiABec::beta   = " << beta << std::endl;
            std::cout << "  HypreExtMultiABec::gamma  = " << gamma << std::endl;
            std::cout << "  HypreExtMultiABec::delta1 = " << delta1
                      << std::endl;
            std::cout << "  HypreExtMultiABec::delta2 = " << delta2
                      << std::endl;
        }
                ...
                for (IntVect v = reg.smallEnd(); v <= reg.bigEnd();
                    ...
                    if (msk(v + vin) != RadBndryData::covered) {
                        ...
                        if (bct == LO_DIRICHLET) {
                            ...
                            if (bcoefs[level]) {
                                Real tmp =
                                    ((dfac / h) * fs(v + vin, bdcomp) * beta *
                                     (*bcoefs[level])[idir][mfi](v + ves));
                                HYPRE_SStructVectorAddToValues(
                                    b, part, getV1(v), 0, &tmp);
                            }
                            if (a2coefs[level]) {
                                Real tmp =
                                    ((-0.5 * vfac) * fs(v + vin, bdcomp) *
                                     alpha2 *
                                     (*a2coefs[level])[idir][mfi](v + ves));
                                HYPRE_SStructVectorAddToValues(
                                    b, part, getV1(v), 0, &tmp);
                            }
                            if (ccoefs[level]) {
                            ...
                            if (d1coefs[level]) {
                                Real tmp =
                                    ((-0.5 * dfac) * fs(v + vin, bdcomp) *
                                     delta1 * ofac *
                                     (*d1coefs[level])[idir][mfi](v));
                                HYPRE_SStructVectorAddToValues(
                                    b, part, getV1(v), 0, &tmp);
                            }
                            if (d2coefs[level]) {
                            ...
                            HYPRE_SStructVectorAddToValues(b, part, getV1(v), 0,
                                                           &tmp);
                        } else if (bct == LO_MARSHAK ||
                                   bct == LO_SANCHEZ_POMRANING) {
                            // cmult should be c for photons, 1 for neutrinos
...
void HypreExtMultiABec::boundaryDterm(int level, MultiFab* Dterm,
                                      MultiFab& Soln, int icomp) {
        ...
        for (OrientationIter oitr; oitr; oitr++) {
            ...
            if (reg[oitr()] == domain[oitr()]) {
                ...
                hdterm3(BL_TO_FORTRAN(Dterm[idim][mfi]),
                        ...
                        ARLIM(reg.hiVect()), cdir, bctype, tfp, bcl,
                        BL_TO_FORTRAN_N(bcv, bdcomp), BL_TO_FORTRAN(msk),
                        BL_TO_FORTRAN((*d2coefs[level])[idim][mfi]),
                        geom[level].CellSize());
            } else {
                hdterm(BL_TO_FORTRAN(Dterm[idim][mfi]),
                       BL_TO_FORTRAN_N(Soln[mfi], icomp), ARLIM(reg.loVect()),
                       ARLIM(reg.hiVect()), cdir, bct, bcl,
                       BL_TO_FORTRAN_N(bcv, bdcomp), BL_TO_FORTRAN(msk),
                       BL_TO_FORTRAN((*d2coefs[level])[idim][mfi]),
                       geom[level].CellSize());
            }
