...
HypreABec::HypreABec(const BoxArray &grids, const DistributionMapping &dmap,
                     const Geometry &_geom, int _solver_flag)
    ...
    if (verbose >= 1 && first && ParallelDescriptor::IOProcessor()) {
        ...
        if (solver_flag == 1 || solver_flag == 3 || solver_flag == 5) {
            std::cout << "habec.pfmg_relax_type           = " << pfmg_relax_type
                      << std::endl;
        }
        std::cout << "habec.verbose                   = " << verbose
                  << std::endl;
        std::cout << "habec.verbose_threshold         = " << verbose_threshold
                  << std::endl;
    }
...
*/
    ...
    int offsets[2][2] = {{-1, 0}, {0, 0}};
#elif (BL_SPACEDIM == 2)
    int offsets[3][2] = {{-1, 0}, {0, -1}, {0, 0}};
#elif (BL_SPACEDIM == 3)
    int offsets[4][3] = {{-1, 0, 0}, {0, -1, 0}, {0, 0, -1}, {0, 0, 0}};
#endif
            ...
            for (OrientationIter oitr; oitr; oitr++) {
                ...
                if (reg[oitr()] == domain[oitr()]) {
                    ...
                    hbflx3(BL_TO_FORTRAN(Flux[idim][si]),
                           ...
                           ARLIM(reg.loVect()), ARLIM(reg.hiVect()), cdir,
                           bctype, tfp, bho, bcl, BL_TO_FORTRAN_N(fs, bdcomp),
                           BL_TO_FORTRAN(msk),
                           BL_TO_FORTRAN((*bcoefs[idim])[si]), beta, dx,
                           flux_factor, r.dataPtr(), inhom, pSPa,
                           ARLIM(SPabox.loVect()), ARLIM(SPabox.hiVect()));
                } else {
                    hbflx(BL_TO_FORTRAN(Flux[idim][si]),
                          BL_TO_FORTRAN_N(Soln[si], icomp), ARLIM(reg.loVect()),
                          ARLIM(reg.hiVect()), cdir, bct, bho, bcl,
                          BL_TO_FORTRAN_N(fs, bdcomp), BL_TO_FORTRAN(msk),
                          BL_TO_FORTRAN((*bcoefs[idim])[si]), beta, dx, inhom);
                }
            }
        }
    }
...
void HypreABec::getFaceMetric(Vector<Real> &r, const Box &reg,
                              const Orientation &ori, const Geometry &geom) {
    if (ori.coordDir() == 0) {
        if (geom.IsCartesian()) {
            r.resize(1, 1.0);
...
#pragma gpu box(reg) sync
        hacoef(AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
               BL_TO_FORTRAN_ANYD(matfab), BL_TO_FORTRAN_ANYD((*acoefs)[ai]),
               alpha);
...
#pragma gpu box(reg) sync
            hbcoef(AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                   ...
                   BL_TO_FORTRAN_ANYD((*bcoefs[idim])[ai]), beta,
        ...
        for (OrientationIter oitr; oitr; oitr++) {
            ...
            if (reg[oitr()] == domain[oitr()]) {
                ...
                if (SPa != 0) {
                    ...
                    pSPa = &foo;
                    SPabox =
                        Box(IntVect::TheZeroVector(), IntVect::TheZeroVector());
                }

#pragma gpu box(reg) sync
                hbmat3(
                    AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                    reg.loVect()[0], reg.hiVect()[0], oitr().isLow(), idim + 1,
                    BL_TO_FORTRAN_ANYD(matfab), cdir, bctype, tfp,
                    AMREX_INT_ANYD(fsb.loVect()), AMREX_INT_ANYD(fsb.hiVect()),
                    bcl, msk.dataPtr(), AMREX_INT_ANYD(msk.loVect()),
                    AMREX_INT_ANYD(msk.hiVect()),
                    BL_TO_FORTRAN_ANYD((*bcoefs[idim])[ai]), beta,
                    AMREX_REAL_ANYD(dx), flux_factor, pSPa,
...
#pragma gpu box(reg) sync
                hbmat(
                    AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
                    BL_TO_FORTRAN_ANYD(matfab), cdir, bct, bcl, msk.dataPtr(),
                    AMREX_INT_ANYD(msk.loVect()), AMREX_INT_ANYD(msk.hiVect()),
                    BL_TO_FORTRAN_ANYD((*bcoefs[idim])[ai]), beta,
                    AMREX_REAL_ANYD(dx));
            }
        }
        ...
        HYPRE_StructMatrixSetBoxValues(A, loV(reg), hiV(reg), size,
                                       stencil_indices, mat);
        Gpu::synchronize();
    }
    ...
    if (solver_flag == 0) {
        ...
        if (solver_flag == 5) {
            ...
            HYPRE_StructPFMGSetSkipRelax(precond, 0);
            HYPRE_StructPFMGSetLogging(precond, 0);
            HYPRE_StructHybridSetPrecond(solver, HYPRE_StructPFMGSolve,
                                         HYPRE_StructPFMGSetup, precond);
            ...
            HYPRE_StructSMGSetNumPostRelax(precond, 1);
            HYPRE_StructSMGSetLogging(precond, 0);
            HYPRE_StructHybridSetPrecond(solver, HYPRE_StructSMGSolve,
                                         HYPRE_StructSMGSetup, precond);
        }
...
void HypreABec::solve(MultiFab &dest, int icomp, MultiFab &rhs, BC_Mode inhom) {
    ...
    for (MFIter di(dest); di.isValid(); ++di) {
        ...
        if (dest.nGrow() == 0) {  // need a temporary if dest is the wrong size
            ...
            AMREX_PARALLEL_FOR_3D(reg, i, j, k, {
                f_arr(i, j, k, fcomp) = d_arr(i, j, k, icomp);
            });
        }
        ...
        AMREX_PARALLEL_FOR_3D(reg, i, j, k,
                              { f_arr(i, j, k, fcomp) = r_arr(i, j, k, 0); });
...
#pragma gpu box(reg)
                    hbvec3(AMREX_INT_ANYD(reg.loVect()),
                           ...
                           AMREX_INT_ANYD(msk.hiVect()),
                           BL_TO_FORTRAN_ANYD((*bcoefs[idim])[di]), beta,
                           AMREX_REAL_ANYD(dx));
                } else {
#pragma gpu box(reg)
...
Real HypreABec::getAbsoluteResidual() {
    ...
    Real bnorm;
    bnorm =
        hypre_StructInnerProd((hypre_StructVector *)b, (hypre_StructVector *)b);
    bnorm = sqrt(bnorm);
