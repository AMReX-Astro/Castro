...
MGRadBndry::MGRadBndry(const BoxArray& _grids, const DistributionMapping& _dmap,
                       const int _ngroups, const Geometry& _geom)
    : NGBndry(_grids, _dmap, _ngroups, _geom) {
    if (first) init(_ngroups);
    first = 0;
...
void MGRadBndry::setBndryConds(const BCRec& bc, const Geometry& geom,
                               IntVect& ratio) {
...
*/
                if (p_bc == LO_DIRICHLET || p_bc == LO_NEUMANN ||
                    ...
                    std::cerr
                        << "MGRadBndry---Not a recognized boundary condition"
                        << std::endl;
                    exit(1);
                }
            } else {
...
void MGRadBndry::setBndryFluxConds(const BCRec& bc,
            ...
            if (domain[face] == grd[face] && !geom.isPeriodic(dir)) {
                if (bcflag[face] <= 1) {
                    if (p_bc == LO_MARSHAK || p_bc == LO_SANCHEZ_POMRANING ||
                        ...
                        if (p_bcflag == 0) {
                            for (int igroup = 0; igroup < ngroups; igroup++) {
                                bndry[face][bi].setVal<RunOn::Host>(
                                    value_nu[igroup], igroup);
                            }
                        } else {
                            ...
                            FORT_RADBNDRY(BL_TO_FORTRAN(bnd_fab),
                                          ARLIM(domain.loVect()),
                                          ARLIM(domain.hiVect()), dx, xlo, time,
                                          dir, iface);
                        }
                    }
                } else {
                    if (ParallelDescriptor::IOProcessor()) {
                        std::cout
                            << "MGRadBndry ERROR 2: feature not implemented"
                            << std::endl;
                    }
                    exit(2);
