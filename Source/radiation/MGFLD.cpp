...
void Radiation::check_convergence_er(Real& relative, Real& absolute,
                                     Real& err_er, const MultiFab& Er_new,
                                     const MultiFab& Er_pi,
                                     const MultiFab& kappa_p,
                                     const MultiFab& etaTz,
                                     const MultiFab& temp_new, Real delta_t) {
...
#pragma gpu box(bx)
        ca_check_conv_er(
            ...
            BL_TO_FORTRAN_ANYD(Er_new[mfi]), BL_TO_FORTRAN_ANYD(Er_pi[mfi]),
            BL_TO_FORTRAN_ANYD(kappa_p[mfi]), BL_TO_FORTRAN_ANYD(etaTz[mfi]),
            BL_TO_FORTRAN_ANYD(temp_new[mfi]),
            AMREX_MFITER_REDUCE_MAX(&relative),
            AMREX_MFITER_REDUCE_MAX(&absolute),
            AMREX_MFITER_REDUCE_MAX(&err_er), delta_t);
    }
...
void Radiation::check_convergence_matt(
    const MultiFab& rhoe_new, const MultiFab& rhoe_star,
    const MultiFab& rhoe_step, const MultiFab& Er_new, const MultiFab& temp_new,
    const MultiFab& temp_star, const MultiFab& rho, const MultiFab& kappa_p,
    const MultiFab& jg, const MultiFab& dedT, Real& rel_rhoe, Real& abs_rhoe,
    Real& rel_FT, Real& abs_FT, Real& rel_T, Real& abs_T, Real delta_t) {
    rel_rhoe = 0.0;
    rel_FT = 0.0;
    rel_T = 0.0;
    abs_rhoe = 0.0;
...
#pragma omp parallel reduction(max                                          \
                               : rel_rhoe, abs_rhoe, rel_FT, abs_FT, rel_T, \
                                 abs_T)
#endif
...
#pragma gpu box(bx)
        ca_check_conv(
            ...
            BL_TO_FORTRAN_ANYD(rhoe_step[mfi]), BL_TO_FORTRAN_ANYD(Er_new[mfi]),
            BL_TO_FORTRAN_ANYD(temp_new[mfi]),
            BL_TO_FORTRAN_ANYD(temp_star[mfi]), BL_TO_FORTRAN_ANYD(rho[mfi]),
            BL_TO_FORTRAN_ANYD(kappa_p[mfi]), BL_TO_FORTRAN_ANYD(jg[mfi]),
            BL_TO_FORTRAN_ANYD(dedT[mfi]), AMREX_MFITER_REDUCE_MAX(&rel_rhoe),
            AMREX_MFITER_REDUCE_MAX(&abs_rhoe),
            AMREX_MFITER_REDUCE_MAX(&rel_FT), AMREX_MFITER_REDUCE_MAX(&abs_FT),
            AMREX_MFITER_REDUCE_MAX(&rel_T), AMREX_MFITER_REDUCE_MAX(&abs_T),
            delta_t);
    }
...
void Radiation::compute_coupling(MultiFab& coupT, const MultiFab& kpp,
                                 const MultiFab& Eg, const MultiFab& jg) {
#ifdef _OPENMP
#pragma omp parallel
#endif
...
#pragma gpu box(bx)
        ca_compute_coupt(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(coupT[mfi]), BL_TO_FORTRAN_ANYD(kpp[mfi]),
            BL_TO_FORTRAN_ANYD(Eg[mfi]), BL_TO_FORTRAN_ANYD(jg[mfi]));
    }
}

void Radiation::compute_etat(MultiFab& etaT, MultiFab& etaTz, MultiFab& eta1,
                             MultiFab& djdT, const MultiFab& dkdT,
                             const MultiFab& dedT, const MultiFab& Er_star,
                             const MultiFab& rho, Real delta_t, Real ptc_tau) {
#ifdef _OPENMP
#pragma omp parallel
#endif
...
#pragma gpu box(bx)
        ca_compute_etat(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(etaT[mfi]), BL_TO_FORTRAN_ANYD(etaTz[mfi]),
            BL_TO_FORTRAN_ANYD(eta1[mfi]), BL_TO_FORTRAN_ANYD(djdT[mfi]),
            BL_TO_FORTRAN_ANYD(dkdT[mfi]), BL_TO_FORTRAN_ANYD(dedT[mfi]),
            BL_TO_FORTRAN_ANYD(Er_star[mfi]), BL_TO_FORTRAN_ANYD(rho[mfi]),
            delta_t, ptc_tau);
    }
}

void Radiation::eos_opacity_emissivity(
    const MultiFab& S_new, const MultiFab& temp_new, const MultiFab& temp_star,
    MultiFab& kappa_p, MultiFab& kappa_r, MultiFab& jg, MultiFab& djdT,
    MultiFab& dkdT, MultiFab& dedT, int level, int it, int ngrow) {
...
#pragma gpu box(box)
        ca_compute_c_v(
            ...
            BL_TO_FORTRAN_ANYD(dedT[mfi]), BL_TO_FORTRAN_ANYD(temp_new[mfi]),
...
#pragma gpu box(bx)
        ca_opacs(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(S_new[mfi]), BL_TO_FORTRAN_ANYD(temp_new[mfi]),
            BL_TO_FORTRAN_ANYD(temp_star[mfi]),
            BL_TO_FORTRAN_ANYD(kappa_p[mfi]), BL_TO_FORTRAN_ANYD(kappa_r[mfi]),
            BL_TO_FORTRAN_ANYD(dkdT[mfi]), use_dkdT, star_is_valid, lag_opac);
...
#pragma gpu box(reg)
        ca_compute_emissivity(
            ...
            BL_TO_FORTRAN_ANYD(jg[mfi]), BL_TO_FORTRAN_ANYD(djdT[mfi]),
            BL_TO_FORTRAN_ANYD(temp_new[mfi]), BL_TO_FORTRAN_ANYD(kappa_p[mfi]),
...
void Radiation::gray_accel(MultiFab& Er_new, MultiFab& Er_pi, MultiFab& kappa_p,
                           MultiFab& kappa_r, MultiFab& etaT, MultiFab& eta1,
                           MultiFab& mugT, Array<MultiFab, BL_SPACEDIM>& lambda,
...
#pragma gpu box(bx)
        ca_accel_spec(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                      BL_TO_FORTRAN_ANYD(kappa_p[mfi]),
                      BL_TO_FORTRAN_ANYD(mugT[mfi]),
                      BL_TO_FORTRAN_ANYD(spec[mfi]), delta_t, ptc_tau);
    }
...
#pragma gpu box(bx)
        ca_accel_acoe(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                      BL_TO_FORTRAN_ANYD(eta1[mfi]),
                      BL_TO_FORTRAN_ANYD(spec[mfi]),
                      BL_TO_FORTRAN_ANYD(kappa_p[mfi]),
                      BL_TO_FORTRAN_ANYD(acoefs[mfi]), delta_t, ptc_tau);
    }
...
#pragma gpu box(bx)
                lbcoefna(AMREX_INT_ANYD(bx.loVect()),
                         AMREX_INT_ANYD(bx.hiVect()),
                         BL_TO_FORTRAN_ANYD(bcoefs[idim][mfi]),
                         BL_TO_FORTRAN_ANYD(bcgrp[idim][mfi]),
                         BL_TO_FORTRAN_N_ANYD(spec[mfi], igroup), idim);

                if (nGroups > 1) {
#pragma gpu box(bx)
                    ca_accel_ccoe(AMREX_INT_ANYD(bx.loVect()),
                                  AMREX_INT_ANYD(bx.hiVect()),
                                  BL_TO_FORTRAN_ANYD(bcgrp[idim][mfi]),
                                  BL_TO_FORTRAN_ANYD(spec[mfi]),
...
#pragma gpu box(bx)
        ca_accel_rhs(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(Er_new[mfi]), BL_TO_FORTRAN_ANYD(Er_pi[mfi]),
            BL_TO_FORTRAN_ANYD(kappa_p[mfi]), BL_TO_FORTRAN_ANYD(etaT[mfi]),
            BL_TO_FORTRAN_ANYD(rhs[mfi]), delta_t);
    }
...
#pragma gpu box(reg)
        ljupna(AMREX_INT_ANYD(reg.loVect()), AMREX_INT_ANYD(reg.hiVect()),
               BL_TO_FORTRAN_ANYD(Er_new[mfi]), BL_TO_FORTRAN_ANYD(spec[mfi]),
               BL_TO_FORTRAN_ANYD(accel[mfi]), nGroups);
    }
...
void Radiation::local_accel(MultiFab& Er_new, const MultiFab& Er_pi,
                            const MultiFab& kappa_p, const MultiFab& etaT,
                            const MultiFab& mugT, Real delta_t, Real ptc_tau) {
#ifdef _OPENMP
#pragma omp parallel
#endif
    for (MFIter mfi(Er_new, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
...
#pragma gpu box(bx)
        ca_local_accel(
            ...
            BL_TO_FORTRAN_ANYD(Er_new[mfi]), BL_TO_FORTRAN_ANYD(Er_pi[mfi]),
            BL_TO_FORTRAN_ANYD(kappa_p[mfi]), BL_TO_FORTRAN_ANYD(etaT[mfi]),
            BL_TO_FORTRAN_ANYD(mugT[mfi]), delta_t, ptc_tau);
    }
}
        ...
        for (MFIter mfi(msk); mfi.isValid(); ++mfi) {
            ...
            if (level < parent->finestLevel()) {
                std::vector<std::pair<int, Box> > isects =
                    baf.intersections(mfi.validbox());
...
#pragma omp parallel
    ...
    for (MFIter mfi(state, true); mfi.isValid(); ++mfi) {
        ...
        (reg.loVect(), reg.hiVect(), BL_TO_FORTRAN(state[mfi]),
         BL_TO_FORTRAN(rhoe[mfi]), BL_TO_FORTRAN(temp[mfi]),
         BL_TO_FORTRAN(msk[mfi]), &derat, &dT);
    }
...
void Radiation::update_matter(MultiFab& rhoe_new, MultiFab& temp_new,
                              ...
                              const MultiFab& rhoe_step, const MultiFab& etaT,
                              const MultiFab& etaTz, const MultiFab& eta1,
                              const MultiFab& coupT, const MultiFab& kappa_p,
                              const MultiFab& jg, const MultiFab& mugT,
                              const MultiFab& S_new, int level, Real delta_t,
...
#pragma gpu box(bx) sync
            ca_update_matter(
                ...
                BL_TO_FORTRAN_ANYD(eta1[mfi]), BL_TO_FORTRAN_ANYD(coupT[mfi]),
                BL_TO_FORTRAN_ANYD(kappa_p[mfi]), delta_t, ptc_tau);

            temp_new[mfi].copy<RunOn::Device>(rhoe_new[mfi], bx);
            Gpu::synchronize();

#pragma gpu box(bx) sync
            ca_compute_temp_given_rhoe(AMREX_INT_ANYD(bx.loVect()),
                                       AMREX_INT_ANYD(bx.hiVect()),
                                       BL_TO_FORTRAN_ANYD(temp_new[mfi]),
                                       BL_TO_FORTRAN_ANYD(S_new[mfi]), 0);
        } else {
            BL_FORT_PROC_CALL(CA_NCUPDATE_MATTER, ca_ncupdate_matter)
            (bx.loVect(), bx.hiVect(), BL_TO_FORTRAN(temp_new[mfi]),
             BL_TO_FORTRAN(Er_new[mfi]), BL_TO_FORTRAN(rhoe_star[mfi]),
             BL_TO_FORTRAN(rhoe_step[mfi]), BL_TO_FORTRAN(etaTz[mfi]),
             BL_TO_FORTRAN(kappa_p[mfi]), BL_TO_FORTRAN(jg[mfi]), &delta_t);

#pragma gpu box(bx) sync
            ca_get_rhoe(AMREX_INT_ANYD(bx.loVect()),
                        AMREX_INT_ANYD(bx.hiVect()),
                        BL_TO_FORTRAN_ANYD(rhoe_new[mfi]),
                        BL_TO_FORTRAN_ANYD(temp_new[mfi]),
                        BL_TO_FORTRAN_ANYD(S_new[mfi]));
...
void Radiation::compute_limiter(
    int level, const BoxArray& grids, const MultiFab& Sborder,
    const MultiFab& Erborder,
    MultiFab& lamborder) {  // it works for both single- and multi-group
...
#pragma omp parallel
        ...
        for (MFIter mfi(Er_wide, false); mfi.isValid(); ++mfi) {
            ...
            (BL_TO_FORTRAN(Er_wide[mfi]), BL_TO_FORTRAN(kpr[mfi]),
             BL_TO_FORTRAN(lamborder[mfi]), dx, &ngrow, &limiter,
             &filter_lambda_T, &filter_lambda_S);
        }
...
void Radiation::estimate_gamrPr(const FArrayBox& state, const FArrayBox& Er,
                                FArrayBox& gPr, const Real* dx,
                                const Box& box) {
    if (limiter == 0) {
        BL_FORT_PROC_CALL(CA_EST_GPR0, ca_est_gpr0)
        (BL_TO_FORTRAN(Er), BL_TO_FORTRAN(gPr));
    } else {
        ...
        (BL_TO_FORTRAN(kappa_r), BL_TO_FORTRAN(Er), BL_TO_FORTRAN(gPr),
         box.loVect(), box.hiVect(), dx, &limiter, &comoving);
    }
}

void Radiation::MGFLD_compute_rosseland(FArrayBox& kappa_r,
...
#pragma gpu box(kbox) sync
    ca_compute_rosseland(AMREX_INT_ANYD(kbox.loVect()),
                         ...
                         BL_TO_FORTRAN_ANYD(kappa_r), BL_TO_FORTRAN_ANYD(state),
                         0, nGroups - 1, nGroups);
}

void Radiation::MGFLD_compute_rosseland(MultiFab& kappa_r,
...
#pragma gpu box(bx)
        ca_compute_rosseland(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(kappa_r[mfi]), BL_TO_FORTRAN_ANYD(state[mfi]), 0,
            nGroups - 1, nGroups);
    }
}

void Radiation::MGFLD_compute_scattering(FArrayBox& kappa_s,
                                         const FArrayBox& state) {
...
void Radiation::bisect_matter(MultiFab& rhoe_new, MultiFab& temp_new,
                              const MultiFab& rhoe_star,
                              const MultiFab& temp_star, const MultiFab& S_new,
...
#pragma gpu box(bx)
        ca_get_rhoe(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                    BL_TO_FORTRAN_ANYD(rhoe_new[mfi]),
                    BL_TO_FORTRAN_ANYD(temp_new[mfi]),
                    BL_TO_FORTRAN_ANYD(S_new[mfi]));
    }
...
#pragma gpu box(bx)
        ca_rhstoer(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                   BL_TO_FORTRAN_ANYD(rhs[ri]), AMREX_REAL_ANYD(dx), dt);
    }
}
...
#endif
        {
            ...
            for (MFIter mfi(S_new, true); mfi.isValid(); ++mfi) {
                ...
                ca_inelastic_sct(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                                 ...
                                 BL_TO_FORTRAN_ANYD(kps), dt);
            }
        }

        castro->computeTemp(S_new, castro->state[State_Type].curTime(),
                            S_new.nGrow());
    }
}
