...
void RadBndry::setBndryConds(const BCRec& bc, const Geometry& geom,
                             IntVect& ratio) {
...
*/
                if (p_bc == LO_DIRICHLET || p_bc == LO_NEUMANN ||
                    ...
                    std::cerr
                        << "RadBndry---Not a recognized boundary condition"
                        << std::endl;
                    exit(1);
                }
            } else {
            ...
            if (domain[face] == grd[face] && !geom.isPeriodic(dir)) {
                if (bcflag[face] <= 1) {
                    ...
                    if (ParallelDescriptor::IOProcessor()) {
                        std::cout << "RadBndry ERROR 2: feature not implemented"
                                  << std::endl;
                    }
                    exit(2);
