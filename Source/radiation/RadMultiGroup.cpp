...
void Radiation::get_groups(int verbose) {
    ...
    dlognugroup.resize(nGroups,
                       0.0);  // Log of the width of the frequency group
    ...
    if (SolverType == MGFLDSolver) {
        ...
        (nugroup.dataPtr(), dnugroup.dataPtr(), dlognugroup.dataPtr(),
         xnu.dataPtr(), nGroups, nG0, nG1);
    } else if (xnu.size() > 0) {
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        if (dlognugroup.size() > 0) {
            ...
            for (int i = 0; i < dlognugroup.size(); i++) {
                groupfile << "group(" << i << ") = " << dlognugroup[i]
                          << std::endl;
            }
        }
...
void Radiation::write_groups(ostream& os) {
    ...
    if (xnu.size() > 0) {
        ...
        for (int i = 0; i < xnu.size(); i++) {
            os << "group(" << i << ") = " << xnu[i] * group_print_factor
               << std::endl;
        }
    }
}
