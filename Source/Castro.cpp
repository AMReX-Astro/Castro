...
Castro::reflux ()
{
	...
	MultiFab& fine_state = fine_lev.get_new_data(State_Type);
...
#ifdef GRAVITY
	if (do_grav && gravity->get_gravity_type() == "PoissonGrav" && gravity->NoSync() == 0)
	    MultiFab::Add(drho[lev-1], crse_state, Density, 0, 1, 0);
#endif
	...
	// changed in regions underlying the fine level.

	crse_lev.avgDown();

	// Clean up any aberrant state data generated by the reflux.

	crse_lev.clean_state(crse_state);
