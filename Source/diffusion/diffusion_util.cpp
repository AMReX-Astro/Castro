...
void fill_temp_cond(const Box& bx, Array4<Real const> const& U_arr,
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
        ...
        if (eos_state.rho > castro::diffuse_cutoff_density) {
            ...
            if (eos_state.rho < castro::diffuse_cutoff_density_hi) {
                Real multiplier =
                    (eos_state.rho - castro::diffuse_cutoff_density) /
        ...
        coeff_arr(i, j, k) =
            castro::diffuse_cond_scale_fac * eos_state.conductivity;
    });
}

void fill_temp_diff_coeff(const Box& bx, Array4<Real const> const& U_arr,
                          Array4<Real> const& coeff_arr) {
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
        ...
        coeff_arr(i, j, k) = castro::diffuse_cond_scale_fac *
                             eos_state.conductivity * rhoinv / eos_state.cv;
    });
}
