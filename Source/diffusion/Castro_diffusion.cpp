...
Castro::construct_old_diff_source(Real time, Real dt)
{
    ...
    add_temp_diffusion_to_source(*old_sources[0], TempDiffTerm, time, 1);

#if (BL_SPACEDIM == 1)
    add_spec_diffusion_to_source(*old_sources[0], SpecDiffTerm, time, 1);
    add_viscous_term_to_source(*old_sources[0], ViscousTermforMomentum, ViscousTermforEnergy, time);
#endif
}
...
Castro::construct_new_diff_source(Real time, Real dt)
{
    ...
    add_temp_diffusion_to_source(*new_sources[0], TempDiffTerm, time, 0, mult_factor);

#if (BL_SPACEDIM == 1)
    add_spec_diffusion_to_source(*new_sources[0], SpecDiffTerm, time, 0, mult_factor);
    add_viscous_term_to_source(*new_sources[0], ViscousTermforMomentum, ViscousTermforEnergy, time, mult_factor);
#endif
    ...
    add_temp_diffusion_to_source(*new_sources[0], TempDiffTerm, old_time, 1, mult_factor);

#if (BL_SPACEDIM == 1)
    add_spec_diffusion_to_source(*new_sources[0], SpecDiffTerm, old_time, 1, mult_factor);
    add_viscous_term_to_source(*new_sources[0], ViscousTermforMomentum, ViscousTermforEnergy, old_time, mult_factor);
#endif
