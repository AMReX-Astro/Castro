...
void Castro::construct_old_diff_source(MultiFab& source, MultiFab& state_in,
                                       Real time, Real dt) {
    ...
    if (verbose > 1) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Castro::construct_old_diff_source() time = "
                          << run_time << "\n"
                          << "\n";
#ifdef BL_LAZY
        });
...
void Castro::construct_new_diff_source(MultiFab& source, MultiFab& state_old,
    ...
    add_temp_diffusion_to_source(source, state_new, TempDiffTerm, time,
                                 mult_factor);
    ...
    add_temp_diffusion_to_source(source, state_old, TempDiffTerm, old_time,
                                 mult_factor);
    ...
    if (verbose > 1) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor())
                std::cout << "Castro::construct_new_diff_source() time = "
                          << run_time << "\n"
                          << "\n";
#ifdef BL_LAZY
        });
...
void Castro::add_temp_diffusion_to_source(MultiFab& ext_src, MultiFab& state_in,
                                          MultiFab& DiffTerm, Real t,
                                          Real mult_factor) {
...
void Castro::getTempDiffusionTerm(Real time, MultiFab& state_in,
                                  MultiFab& TempDiffTerm) {
    ...
    {
        FillPatchIterator fpi(*this, state_in, 1, time, State_Type, 0,
                              NUM_STATE);
...
#pragma omp parallel
#endif
        {
            ...
            for (MFIter mfi(grown_state, TilingIfNotGPU()); mfi.isValid();
                 ++mfi) {
                ...
                for (int idir = 0; idir < AMREX_SPACEDIM; ++idir) {
                    ...
                    Array4<Real> const edge_coeff_arr =
                        (*coeffs[idir]).array(mfi);

                    AMREX_PARALLEL_FOR_3D(nbx, i, j, k, {
                        if (idir == 0) {
                            edge_coeff_arr(i, j, k) =
                                0.5_rt *
                                (coeff_arr(i, j, k) + coeff_arr(i - 1, j, k));
                        } else if (idir == 1) {
                            edge_coeff_arr(i, j, k) =
                                0.5_rt *
                                (coeff_arr(i, j, k) + coeff_arr(i, j - 1, k));
                        } else {
                            edge_coeff_arr(i, j, k) =
                                0.5_rt *
                                (coeff_arr(i, j, k) + coeff_arr(i, j, k - 1));
                        }
                    });
                }
            }
    ...
    if (level > 0) {
        ...
        const DistributionMapping& crse_dmap =
            getLevel(level - 1).DistributionMap();
