...
void Diffusion::output_job_info_params(std::ostream& jobInfoFile) {
#include "diffusion_job_info_tests.H"
}

void Diffusion::install_level(int level, AmrLevel* level_data,
...
void Diffusion::applyop(int level, MultiFab& Temperature, MultiFab& CrseTemp,
                        MultiFab& DiffTerm,
...
#if (BL_SPACEDIM < 3)
void Diffusion::weight_cc(int level, MultiFab& cc) {
...
void Diffusion::unweight_cc(int level, MultiFab& cc) {
    const Real* dx = parent->Geom(level).CellSize();
    const int coord_type = parent->Geom(level).Coord();
#ifdef _OPENMP
#pragma omp parallel
...
void Diffusion::applyop_mlmg(
    int level, MultiFab& Temperature, MultiFab& CrseTemp, MultiFab& DiffTerm,
    Vector<std::unique_ptr<MultiFab> >& temp_cond_coef) {
    ...
    mlabec.setBCoeffs(0, Array<MultiFab const*, AMREX_SPACEDIM>{AMREX_D_DECL(
                             temp_cond_coef[0].get(), temp_cond_coef[1].get(),
