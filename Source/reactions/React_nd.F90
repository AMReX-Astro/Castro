...
contains
  ...
  ! Calculate the instantaneous RHS due to reactions.

  subroutine ca_react_rhs(lo, hi, &
                          state, s_lo, s_hi, &
                          react_rhs, r_lo, r_hi) bind(C, name="ca_react_rhs")
    ...
    real(rt), intent(inout) :: react_rhs(r_lo(1):r_hi(1),r_lo(2):r_hi(2),r_lo(3):r_hi(3),NVAR)
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             ! Initialize to zero in case we're not in a valid burning zone.
             react_rhs(i,j,k,:) = ZERO
             ...
             burn_state % xn  = state(i,j,k,UFS:UFS+nspec-1) * rhoInv
#if naux > 0
             burn_state % aux = state(i,j,k,UFX:UFX+naux-1) * rhoInv
#endif

             if (.not. ok_to_burn(burn_state)) cycle

             burn_state % dx = dx_min
             ...
             react_rhs(i,j,k,UEINT) = burn_state % ydot(net_ienuc) * state(i,j,k,URHO)
             react_rhs(i,j,k,UEDEN) = react_rhs(i,j,k,UEINT)
             react_rhs(i,j,k,UFS:UFS+nspec-1) = burn_state % ydot(1:nspec) * state(i,j,k,URHO)

          enddo
       enddo
    enddo

  end subroutine ca_react_rhs
