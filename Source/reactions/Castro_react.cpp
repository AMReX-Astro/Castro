...
bool Castro::react_state(MultiFab& s, MultiFab& r, Real time, Real dt) {
    ...
    if (time_integration_method != CornerTransportUpwind) {
        amrex::Error(
            "Strang reactions are only supported for the CTU advance.");
    }
        ...
        if (level <= castro::reactions_max_solve_level) {
            ...
            reduce_op.eval(
                ...
                // Don't burn on zones inside shock regions, if the relevant option is set.
...
#ifdef SHOCK_VAR
                    if (U(i, j, k, USHK) > 0.0_rt &&
                        disable_shock_burning == 1) {
                        do_burn = false;
                    }
#endif
                    ...
                    if (burn_state.T < lreact_T_min ||
                        burn_state.T > lreact_T_max ||
                        burn_state.rho < lreact_rho_min ||
                        burn_state.rho > lreact_rho_max) {
                        do_burn = false;
                    }
                    ...
                    if (do_burn) {
                        ...
                        if (reactions.contains(i, j, k)) {
                            for (int n = 0; n < NumSpec; ++n) {
                                reactions(i, j, k, n) =
                                    (burn_state.xn[n] -
                                     U(i, j, k, UFS + n) * rhoInv) /
                                    dt;
                            }
                            reactions(i, j, k, NumSpec) = delta_e / dt;
                            reactions(i, j, k, NumSpec + 1) = delta_rho_e / dt;
                            reactions(i, j, k, NumSpec + 2) = amrex::max(
                                1.0_rt,
                                static_cast<Real>(burn_state.n_rhs +
                                                  2 * burn_state.n_jac));
                        }
                        ...
                        for (int n = 0; n < NumSpec; ++n) {
                            U(i, j, k, UFS + n) =
                                U(i, j, k, URHO) * burn_state.xn[n];
                        }

#if naux > 0
                        for (int n = 0; n < NumSpec; ++n) {
                            U(i, j, k, UFX + n) =
                                U(i, j, k, URHO) * burn_state.aux[n];
                        }
#endif
...
#pragma gpu box(bx)
            ca_react_state(
                ...
                BL_TO_FORTRAN_ANYD(s[mfi]), BL_TO_FORTRAN_ANYD(r[mfi]), time,
                dt, AMREX_MFITER_REDUCE_SUM(&burn_failed));
            ...
            amrex::ParallelFor(
                bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
                    if (U.contains(i, j, k) && reactions.contains(i, j, k)) {
                        for (int n = 0; n < NumSpec; ++n) {
                            U(i, j, k, UFS + n) +=
                                reactions(i, j, k, n) * U(i, j, k, URHO) * dt;
                        }
                        U(i, j, k, UEINT) +=
                            reactions(i, j, k, NumSpec) * U(i, j, k, URHO) * dt;
                        U(i, j, k, UEDEN) +=
                            reactions(i, j, k, NumSpec) * U(i, j, k, URHO) * dt;
                    }
                });
    ...
    if (print_update_diagnostics) {
        ...
        if (e_added != 0.0) {
            amrex::Print() << "... (rho e) added from burning: " << e_added
                           << std::endl
                           << std::endl;
        }
    }
...
bool Castro::react_state(Real time, Real dt) {
    BL_PROFILE("Castro::react_state()");
    ...
    if (time_integration_method != SimplifiedSpectralDeferredCorrections) {
        amrex::Error(
            "This react_state interface is only supported for simplified SDC.");
    }
...
#pragma gpu box(bx)
        ca_react_state_simplified_sdc(
            AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
            BL_TO_FORTRAN_ANYD(uold), BL_TO_FORTRAN_ANYD(unew),
            BL_TO_FORTRAN_ANYD(a), BL_TO_FORTRAN_ANYD(r), BL_TO_FORTRAN_ANYD(m),
            time, dt, sdc_iteration, AMREX_MFITER_REDUCE_SUM(&burn_failed));
    }
...
bool Castro::valid_zones_to_burn(MultiFab& State) {
    ...
    if (small_size > 0) {
        amrex::ParallelDescriptor::ReduceRealMin(small_limiters.dataPtr(),
                                                 small_size);
    ...
    if (large_size > 0) {
        amrex::ParallelDescriptor::ReduceRealMax(large_limiters.dataPtr(),
                                                 large_size);
    ...
    if (verbose > 1) {
        amrex::Print() << "  No valid zones to burn, skipping react_state()."
                       << std::endl;
    }
