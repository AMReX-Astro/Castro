...
Castro::react_state(MultiFab& s, MultiFab& r, const iMultiFab& m, Real time, Real dt_react, int strang_half, int ngrow)
{
    ...
    // be that we always burn on the GPU except for very hard cases
    // where we expect the GPU to be worse because a few zones require
    // many integrator steps while everything else does not. 

    // The MultiFab which stores this result should be allocated in
    // CPU pinned memory, since it will be simultaneously accessed
    // later by both the CPU and GPU, and we want to avoid thrashing.
...
#pragma omp parallel
    ...
    for (MFIter mfi(s, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        // Maintain two separate counters for the CPU and GPU failures.
        // We'll add these together at the end. If we're in a pure
        // CPU build, we'll only use the first one.
        ...
        // Define the per-zone burn lambda function.
        ...
        auto f = [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k) noexcept
        {
            ...
            // When compiling for GPUs, we have the option to run some of the
            // burns on the CPU. The burnOnCpu MultiFab, as set above, determines
            // which zones are burned on the CPU. This function, which is a host
            // device lambda, will be compiled for both CPU and GPU and run simultaneously
            // on both (see below, under the lambda definition). We create here
            // a bool do_burn which tracks whether we should do the burn. We
            // assume we should do the burn on the GPU except for zones marked
            // for the CPU, whereas for the CPU compile we assume we should not
            // do the burn except for zones marked for the CPU. do_burn may still
            // be turned off below in either case (for example, if the temperature
            // is not not enough).

#if AMREX_DEVICE_COMPILE
            bool burn_on_gpu = true;
            if (burnOnCpu_arr(i,j,k) == 1.0_rt) {
                burn_on_gpu = false;
            }
            bool do_burn = burn_on_gpu;
#else
            bool burn_on_cpu = false;
            if (burnOnCpu_arr(i,j,k) == 1.0_rt) {
                burn_on_cpu = true;
            }
            bool do_burn = burn_on_cpu;
#endif

#else

            const bool burn_on_cpu = true;
            bool do_burn = burn_on_cpu;

#endif

            // Initialize the burn state.
            ...
            // For this check and later checks, we'll be testing the value
            // of do_burn before doing the check. Aside from saving extra
            // work, the idea is that in the limit where none of the burns
            // happen on the CPU, we don't want the CPU to ever read from
            // the state data to avoid thrashing.
            ...
            if (do_burn) {
                ...
                // Now load the state data into the burn type.

                rhoInv = 1.0_rt / U(i,j,k,URHO);
            ...
            // If we still want to burn at this point, do it.
            ...
            // We are atomically updating separate counters in the
            // host and device implementations since we do not have
            // an atomic implementation that is safe for simultaneous
            // CPU and GPU accesses. We'll add up the failure counts
            // from both later.
            ...
            if (do_burn) {
                ...
                // If we didn't burn, ensure that we have valid data
                // in the Reactions_Type StateData. The processor that
                // would have been responsible for this burn will do
                // the write.

#if AMREX_DEVICE_COMPILE
                bool update_reactions = burn_on_gpu;
#else
                bool update_reactions = burn_on_cpu;
#endif
