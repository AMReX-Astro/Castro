...
contains
  ...
  subroutine ambient_fill(lo, hi, adv, adv_lo, adv_hi, domlo, domhi, numcomp, bc) bind(C, name="ambient_fill")
    ...
    real(rt), intent(inout) :: adv(adv_lo(1):adv_hi(1),adv_lo(2):adv_hi(2),adv_lo(3):adv_hi(3),numcomp)
          ...
          do j = lo(2), hi(2)
             do i = lo(1), hi(1)
                if ((i < domlo(1) .and. (bc(1,1,1) == amrex_bc_foextrap .or. bc(1,1,1) == amrex_bc_hoextrap))  &
                   ...
                   adv(i,j,k,:) = ambient_state(URHO:URHO+numcomp-1)
                end if
             end do
          end do
       end do
