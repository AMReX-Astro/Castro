...
void ca_statefill(Box const& bx, FArrayBox& data, const int dcomp,
    ...
    if (Gpu::inLaunchRegion()) {
        GpuBndryFuncFab<CastroGenericFill> gpu_bndry_func(
            castro_generic_fill_func);
...
#pragma gpu box(bx)
        denfill(AMREX_INT_ANYD(bx.loVect()), AMREX_INT_ANYD(bx.hiVect()),
                ...
                AMREX_INT_ANYD(geom.Domain().loVect()),
                AMREX_INT_ANYD(geom.Domain().hiVect()),
                AMREX_REAL_ANYD(geom.CellSize()),
                AMREX_REAL_ANYD(geom.ProbLo()), time, bc_f);
...
#pragma gpu box(bx)
    ...
    if ((bcr[URHO].lo(0) == EXT_DIR && bcr[URHO].lo(1) == EXT_DIR) ||
        ...
        amrex::Error(
            "Error: external boundaries meeting at a corner not supported");
    }
#endif
...
void ca_face_fillx(Real* var, const int* var_lo, const int* var_hi,
                   const int* domlo, const int* domhi, const Real* dx,
                   const Real* xlo, const Real* time, const int* bc) {
    int lo[3] = {0};
    int hi[3] = {0};
    ...
    face_fillx(AMREX_INT_ANYD(lo), AMREX_INT_ANYD(hi), var,
               AMREX_INT_ANYD(var_lo), AMREX_INT_ANYD(var_hi),
...
void ca_face_filly(Real* var, const int* var_lo, const int* var_hi,
    ...
    face_filly(AMREX_INT_ANYD(lo), AMREX_INT_ANYD(hi), var,
               AMREX_INT_ANYD(var_lo), AMREX_INT_ANYD(var_hi),
               AMREX_INT_ANYD(domlo), AMREX_INT_ANYD(domhi),
               AMREX_REAL_ANYD(dx), AMREX_REAL_ANYD(xlo), *time, bc_f);
}
...
void ca_face_fillz(Real* var, const int* var_lo, const int* var_hi,
    ...
    face_fillz(AMREX_INT_ANYD(lo), AMREX_INT_ANYD(hi), var,
               AMREX_INT_ANYD(var_lo), AMREX_INT_ANYD(var_hi),
               AMREX_INT_ANYD(domlo), AMREX_INT_ANYD(domhi),
               AMREX_REAL_ANYD(dx), AMREX_REAL_ANYD(xlo), *time, bc_f);
}
