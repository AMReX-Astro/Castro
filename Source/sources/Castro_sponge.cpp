...
void Castro::construct_old_sponge_source(MultiFab& source, MultiFab& state_in,
                                         Real time, Real dt) {
...
#pragma omp parallel
    ...
    for (MFIter mfi(state_in, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        apply_sponge(bx, state_in.array(mfi), source.array(mfi), dt,
                     mult_factor);
    }
    ...
    if (verbose > 1) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor()) {
                std::cout << "Castro::construct_old_thermo_source() time = "
                          << run_time << "\n"
                          << "\n";
            }
#ifdef BL_LAZY
...
void Castro::construct_new_sponge_source(MultiFab& source, MultiFab& state_old,
...
#pragma omp parallel
    ...
    for (MFIter mfi(state_old, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        apply_sponge(bx, state_old.array(mfi), source.array(mfi), dt,
                     mult_factor_old);
    }
...
#pragma omp parallel
    ...
    if (verbose > 1) {
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (ParallelDescriptor::IOProcessor()) {
                std::cout << "Castro::construct_new_thermo_source() time = "
                          << run_time << "\n"
                          << "\n";
            }
#ifdef BL_LAZY
...
void Castro::sponge_init() { ca_allocate_sponge_params(); }

void Castro::sponge_finalize() { ca_deallocate_sponge_params(); }

void Castro::apply_sponge(const Box& bx, Array4<Real const> const state,
                          Array4<Real> const source, Real dt,
                          Real mult_factor) {
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j,
        ...
        if (lsponge_lower_radius >= 0.0_rt &&
            lsponge_upper_radius > lsponge_lower_radius) {
            Real rad = std::sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);

            if (rad < lsponge_lower_radius) {
                sponge_factor = lsponge_lower_factor;

            } else if (rad >= lsponge_lower_radius &&
                       rad <= lsponge_upper_radius) {
                sponge_factor =
                    lsponge_lower_factor +
                    0.5_rt * (lsponge_upper_factor - lsponge_lower_factor) *
                        (1.0_rt - std::cos(M_PI * (rad - lsponge_lower_radius) /
                                           delta_r));
        ...
        if (lsponge_upper_density > 0.0_rt && lsponge_lower_density > 0.0_rt) {
            ...
            } else if (rho <= lsponge_upper_density &&
                       rho >= lsponge_lower_density) {
                sponge_factor =
                    lsponge_lower_factor +
                    0.5_rt * (lsponge_upper_factor - lsponge_lower_factor) *
                        (1.0_rt -
                         std::cos(M_PI * (rho - lsponge_upper_density) /
                                  delta_rho));
        ...
        if (lsponge_upper_pressure > 0.0_rt &&
            lsponge_lower_pressure >= 0.0_rt) {
            ...
            } else if (p <= lsponge_upper_pressure &&
                       p >= lsponge_lower_pressure) {
                sponge_factor =
                    lsponge_lower_factor +
                    0.5_rt * (lsponge_upper_factor - lsponge_lower_factor) *
                        (1.0_rt - std::cos(M_PI * (p - lsponge_upper_pressure) /
                                           delta_p));
        ...
        for (int n = 0; n < 3; n++) {
            Sr[n] =
                (state(i, j, k, UMX + n) - rho * lsponge_target_velocity[n]) *
                fac * mult_factor / dt;
            src[UMX + n] = Sr[n];
        }
