...
void Castro::construct_old_thermo_source(MultiFab& source, MultiFab& state_in,
                                         Real time, Real dt) {
...
void Castro::construct_new_thermo_source(MultiFab& source, MultiFab& state_old,
...
void Castro::fill_thermo_source(Real time, Real dt, MultiFab& state,
                                MultiFab& thermo_src) {
        ...
        amrex::ParallelFor(
            bx, [=] AMREX_GPU_HOST_DEVICE(int i, int j, int k) noexcept {
                ...
                if (coord == 0) {
                    ...
                    src(i, j, k, UEINT) =
                        -0.5_rt *
                        (rp * U(i + 1, j, k, UMX) / U(i + 1, j, k, URHO) -
                         rm * U(i - 1, j, k, UMX) / U(i - 1, j, k, URHO)) /
                        (r * dx[0]);
                    ...
                    src(i, j, k, UEINT) =
                        -0.5_rt *
                        (rp * rp * U(i + 1, j, k, UMX) / U(i + 1, j, k, URHO) -
                         rm * rm * U(i - 1, j, k, UMX) / U(i - 1, j, k, URHO)) /
                        (r * r * dx[0]);
                }
