...
void Castro::apply_source_to_state(MultiFab& target_state, MultiFab& source,
                                   Real dt, int ng) {
...
void Castro::time_center_source_terms(MultiFab& S_new, MultiFab& src_old,
                                      MultiFab& src_new, Real dt) {
...
bool Castro::source_flag(int src) {
    switch (src) {
...
void Castro::do_old_sources(
#ifdef MHD
    MultiFab& Bx, MultiFab& By, MultiFab& Bz,
#endif
    MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time,
    Real dt, bool apply_to_state, int amr_iteration, int amr_ncycle) {
    ...
    for (int n = 0; n < num_src; ++n) {
        construct_old_source(n, source, state_old, time, dt, amr_iteration,
                             amr_ncycle);
...
void Castro::do_new_sources(
    ...
    for (int n = 0; n < num_src; ++n) {
        ...
        if (apply_sources_consecutively && apply_to_state) {
            ...
            AmrLevel::FillPatch(*this, source, NUM_GROW, time, Source_Type, 0,
                                source.nComp());
...
void Castro::construct_old_source(int src, MultiFab& source, MultiFab& state_in,
                                  Real time, Real dt, int amr_iteration,
                                  int amr_ncycle) {
...
void Castro::construct_new_source(int src, MultiFab& source,
...
#ifdef GRAVITY
        case grav_src:
            construct_new_gravity_source(source, state_old, state_new, time,
                                         dt);
            break;
#endif
...
#ifdef ROTATION
        case rot_src:
            construct_new_rotation_source(source, state_old, state_new, time,
                                          dt);
            break;
#endif
...
bool Castro::apply_sources() {

    for (int n = 0; n < num_src; ++n) {
        if (source_flag(n)) return true;
    }
...
Vector<Real> Castro::evaluate_source_change(MultiFab& source, Real dt,
                                            bool local) {
...
void Castro::print_source_change(Vector<Real> update) {
...
void Castro::print_all_source_changes(Real dt, bool is_new) {
    ...
    MultiFab& source =
        is_new ? get_new_data(Source_Type) : get_old_data(Source_Type);
    ...
    Lazy::QueueReduction([=]() mutable {
        ...
        ParallelDescriptor::ReduceRealSum(
            summed_updates.dataPtr(), source.nComp(),
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << std::endl
                      << "  Contributions to the state from the " << time
                      << "-time sources:" << std::endl;
...
void Castro::sum_of_sources(MultiFab& source) {
...
#ifdef REACTIONS
void Castro::get_react_source_prim(MultiFab& react_src, Real time, Real dt) {
    ...
    AmrLevel::FillPatch(*this, react_src, react_src.nGrow(), cur_time,
                        Simplified_SDC_React_Type, 0, react_src.nComp());
}
#endif
#endif
