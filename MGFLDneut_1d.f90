...
subroutine ca_compute_coupty(  &
  ...
  double precision, intent(out):: cpy(cpy_l1:cpy_h1)
...
subroutine ca_compute_dedx(  &
  ...
  double precision, intent(out) :: dedT(dedT_l1:dedT_h1)
  double precision, intent(out) :: dedY(dedY_l1:dedY_h1)
  ...
  do i=dedT_l1, dedT_h1
     ...
     rho = S(i,URHO)
     xn(1:nspec) = S(i,UFS:UFS+nspec-1) / rho
     xn(nspec+1:nspec+naux)  = ye(i)
     ...
     call eos_given_RTX(e1, dummy_pres, rho, T(i), xn)
     xn(nspec+1:nspec+naux)  = Ye2
     call eos_given_RTX(e2, dummy_pres, rho, T(i), xn)
...
subroutine ca_compute_eta_the( lo, hi, &
  ...
  double precision, intent(out)  :: etaY(etaY_l1:etaY_h1)
  double precision, intent(out)  :: etYz(etYz_l1:etYz_h1)
  double precision, intent(out)  :: eta1(eta1_l1:eta1_h1)
  double precision, intent(out)  :: theT(theT_l1:theT_h1)
  double precision, intent(out)  :: thTz(thTz_l1:thTz_h1)
  double precision, intent(out)  :: theY(theY_l1:theY_h1)
  double precision, intent(out)  :: thYz(thYz_l1:thYz_h1)
  double precision, intent(out)  :: the1(the1_l1:the1_h1)
  double precision, intent(inout):: djdT(djdT_l1:djdT_h1,0:ngroups-1)
  double precision, intent(inout):: djdY(djdY_l1:djdY_h1,0:ngroups-1)
...
subroutine ca_opac_emis_neut( lo, hi,  &
  ...
  double precision, intent(out  ) :: jg  (   j_l1:   j_h1,0:ngroups-1)
  double precision, intent(out  ) :: djdT(djdT_l1:djdT_h1,0:ngroups-1)
  double precision, intent(out  ) :: djdY(djdY_l1:djdY_h1,0:ngroups-1)
  double precision, intent(inout) :: dkdT(dkdT_l1:dkdT_h1,0:ngroups-1)
  double precision, intent(inout) :: dkdY(dkdY_l1:dkdY_h1,0:ngroups-1)
...
subroutine ca_update_matter_neut( lo, hi,  &
  ...
  double precision,intent(out)::rY_n(rY_n_l1:rY_n_h1)
  double precision,intent(out)::Ye_n(Ye_n_l1:Ye_n_h1)
...
subroutine ca_ncupdate_matter_neut( lo, hi,  &
  ...
  double precision,intent(inout)::Ye_n(Ye_n_l1:Ye_n_h1)
  double precision,intent(in   )::Er_n(Er_n_l1:Er_n_h1,0:ngroups-1)
  double precision,intent(in   )::re_s(re_s_l1:re_s_h1)
  double precision,intent(in   )::rY_s(rY_s_l1:rY_s_h1)
  double precision,intent(in   )::re_2(re_2_l1:re_2_h1)
