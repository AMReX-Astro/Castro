...
int main(int argc, char* argv[]) {
    ...
    // Input arguments
    if (argc < 2) Abort("ERROR: Missing plotfiles");

    Print()
        << "\nUsing a density threshhold of half the maximum analytic density"
        << std::endl;
    ...
    for (auto f = farg; f < argc; f++) {
        ...
        // radial coordinate
        Vector<Real> r(nbins);
        for (auto i = 0; i < nbins; i++) r[i] = (i + 0.5) * dx_fine + rmin;

        // find variable indices
        auto dens_comp = data.StateNumber("density");

        if (dens_comp < 0) Abort("ERROR: density variable not found");
...
#pragma omp parallel
            ...
            for (MFIter mfi(lev_data_mf, true); mfi.isValid(); ++mfi) {
                ...
                fdustcollapse1d(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                                ...
                                dens.dataPtr(), imask.dataPtr(), mask_size, r1,
                                dens_comp, &cnt);
#elif (AMREX_SPACEDIM == 2)

                fdustcollapse2d(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                                BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), nbins,
                                dens.dataPtr(), volcount.dataPtr(),
                                imask.dataPtr(), mask_size, r1, ZFILL(level_dx),
                                dx_fine, yctr, dens_comp);
#else
                fdustcollapse3d(
                    bx.loVect(), bx.hiVect(),
                    BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), nbins, dens.dataPtr(),
                    volcount.dataPtr(), imask.dataPtr(), mask_size, r1,
        ...
        for (int i = 0; i < nbins; i++)
            if (volcount[i] != 0.) dens[i] /= volcount[i];
#endif
        ...
        if (fabs(data.Time()) <= 1.e-8)
            ...
            Print() << "Dont know the maximum density at this time: "
                    << data.Time() << std::endl;
            Abort();
        }
        ...
        // output
        Print() << "\ntime = " << data.Time()
                << ", r_interface = " << std::setprecision(16) << r_interface
                << std::endl
                << std::endl;
        ...
        if (profile) {
            ...
            // write the header
            outfile << std::setw(w) << "r" << std::setw(w) << "density"
                    << std::endl;

            for (auto i = 0; i < nbins; i++)
                outfile << std::setw(w) << r[i] << std::setw(w) << dens[i]
                        << std::endl;
