...
// Prototypes
//
void GetInputArgs(const int argc, char** argv, string& pltfile);
...
int main(int argc, char* argv[]) {
    ...
    int dens_comp, xmom_comp, ymom_comp, zmom_comp, pres_comp, rhoe_comp,
        spec_comp, temp_comp;
    ...
    if (first_spec_name == "") Abort("ERROR: no species were found");
    ...
    for (int lev = 0; lev <= data.FinestLevel(); lev++) {
        ...
        for (MFIter mfi(lev_data_mf, true); mfi.isValid(); ++mfi) {
            ...
            for (auto n = 0; n < data.NComp(); n++) {
                data.FillVar(&tmp_fab, bx, lev, varNames[n],
            ...
            find_timestep_limiter(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                                  ...
                                  pres_comp, rhoe_comp, spec_comp,
                                  time_integration_method, level_dx.dataPtr(),
                                  &dt, dt_loc.dataPtr());

            find_timestep_limiter_burning(
                ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), dens_comp, temp_comp,
                rhoe_comp, spec_comp, level_dx.dataPtr(), &burning_dt,
                burning_dt_loc.dataPtr());
#ifdef DIFFUSION
            find_timestep_limiter_diffusion(
                ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), dens_comp, temp_comp,
                rhoe_comp, spec_comp, level_dx.dataPtr(), &diffusion_dt,
                diffusion_dt_loc.dataPtr());
#endif
        }
    }
    Print() << std::endl;
...
void GetInputArgs(const int argc, char** argv, string& pltfile) {
    ...
    Print() << "Finding limiting timestep in plotfile  = \"" << pltfile << "\""
            << std::endl;
}
