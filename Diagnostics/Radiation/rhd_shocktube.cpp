...
int main(int argc, char* argv[]) {
    ...
    for (auto i = 0; i < nbins; i++) x[i] = (i + 0.5) * dx_coarse;

    // find variable indices
    Vector<std::string> compVarNames = {"density", "x_velocity", "pressure",
                                        "rad0"};
...
#pragma omp parallel
    ...
    for (MFIter mfi(lev_data_mf, true); mfi.isValid(); ++mfi) {
        ...
        frhdshocktube(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                      ...
                      rad_bin.dataPtr(), dens_comp, velx_comp, pres_comp,
                      rad0_comp, ngroups);
    }

    // write slicefile
    Vector<Vector<Real> > vars = {dens_bin, vel_bin, pres_bin, rad_bin};
    Vector<std::string> slcvarNames = {"density", "velocity", "pressure",
                                       "rad"};
