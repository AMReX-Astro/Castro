...
int main(int argc, char* argv[]) {
    ...
    // check that idir <= DIM
    if (idir > AMREX_SPACEDIM) Abort("ERROR: idir must be <= DIM");
    ...
    for (auto i = 0; i < nvars; i++) fill_comps[i] = i;
...
#pragma omp parallel
    ...
    // find variable indices
#if (AMREX_SPACEDIM == 1)
    compVarNames = {"density", "x_velocity", "pressure", "eint_E",
                    "Temp",    "rad",        "rad"};
#elif (AMREX_SPACEDIM == 2)
    compVarNames = {"density", "x_velocity", "y_velocity", "pressure",
                    "eint_E",  "Temp",       "rad",        "rad"};
#else
    compVarNames = {"density",    "x_velocity", "y_velocity",
                    "z_velocity", "pressure",   "eint_E",
                    "Temp",       "rad",        "rad"};
#endif
    Vector<int> comps = GetComponents(data, compVarNames);

#if (AMREX_SPACEDIM == 1)
    Vector<std::string> slcvarNames = {
        "density",     "x-velocity", "pressure", "int. energy",
        "temperature", "rad energy", "rad temp"};
#elif (AMREX_SPACEDIM == 2)
    Vector<std::string> slcvarNames = {
        "density",     "x-velocity",  "y-velocity", "pressure",
        "int. energy", "temperature", "rad energy", "rad temp"};
#else
    Vector<std::string> slcvarNames = {
    ...
    for (auto i = 0; i < cnt; i++) {
        ...
        auto it = comps.end() - 1;
        slicefile << std::setw(w)
                  << pow(vars_bin[isv[i] + ((*it) + 1) * nbins] / arad, 0.25);
