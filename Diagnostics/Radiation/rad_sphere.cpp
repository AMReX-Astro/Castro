...
void Print_Help() {
    Print()
        << "\nPrint out the radiation quantities at a specified distance from"
        << "\nthe origin.  This is written for the 1-d radiating sphere "
           "problem."
        << "\n"
        << "\n./fradsphere -p plotfile -r radius -g groupfile -v variable"
        << "\n"
        << "\nHere groupfile is the file containing the group structure "
           "information"
        << "\nas output by Castro (usually group_structure.dat)."
        << "\nvariable should be either rad or rad_analytic_ (to obtain the "
           "numerical"
...
int main(int argc, char* argv[]) {
    ...
    for (int l = finestLevel; l >= 0; l--) {
        ...
        for (MFIter mfi(lev_data_mf, true); mfi.isValid(); ++mfi) {
            ...
            fradsphere(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                       ...
                       vars_bin.dataPtr(), imask.dataPtr(), mask_size, r1,
                       ZFILL(dx), &cnt);
        }
    ...
    for (auto i = 0; i < cnt; i++) coords[i] = vars_bin[i];

    auto isv = sort_indexes(coords);

    Print() << "coords_min = " << coords[0]
            << " coords_max = " << coords[cnt - 1] << std::endl;
    ...
    slicefile << std::setw(15) << "# group name" << std::setw(w)
              << "group center energy" << std::setw(w)
              << "E_rad(nu)*dnu (erg/cm^3)" << std::setw(w)
              << "E_rad(nu) (erg/cm^3/Hz)" << std::endl;

    for (auto i = 0; i < ngroups; i++) {
        slicefile << std::setw(15) << varNames[rad_comp + i] << std::setw(w)
                  << nu_groups[i] << std::setw(w)
                  << vars_bin[isv[idx_obs] + (rad_comp + i + 1) * nbins]
                  << std::setw(w)
                  << vars_bin[isv[idx_obs] + (rad_comp + i + 1) * nbins] /
                         dnu_groups[i]
                  << std::endl;
