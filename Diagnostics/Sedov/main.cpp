...
// Prototypes
//
void GetInputArgs(const int argc, char** argv, string& pltfile, string& slcfile,
                  bool& sphr);
...
int main(int argc, char* argv[]) {
    ...
    for (auto i = 0; i < nbins; i++) r[i] = (i + 0.5) * dx_fine;
...
#if (AMREX_SPACEDIM == 3)
    if (ymom_comp < 0 || zmom_comp < 0) Abort("ERROR: variable(s) not found");
#endif
    ...
    for (auto i = 0; i < data.NComp(); i++) fill_comps[i] = i;
...
#pragma omp parallel
        ...
        for (MFIter mfi(lev_data_mf, true); mfi.isValid(); ++mfi) {
            ...
            fextract1d(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                       ...
                       pres_bin.dataPtr(), e_bin.dataPtr(), imask.dataPtr(),
                       mask_size, r1, dens_comp, xmom_comp, pres_comp,
                       rhoe_comp);
#elif (AMREX_SPACEDIM == 2)
            if (sphr) {
                ...
                fextract2d_sph(
                    ...
                    BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), nbins,
                    dens_bin.dataPtr(), vel_bin.dataPtr(), pres_bin.dataPtr(),
                    e_bin.dataPtr(), volcount.dataPtr(), imask.dataPtr(),
                    mask_size, r1, dens_comp, xmom_comp, ymom_comp, pres_comp,
                    rhoe_comp, dx_fine, level_dx.dataPtr(), xctr, yctr);
                ...
                fextract2d_cyl(
                    ...
                    BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), nbins,
                    dens_bin.dataPtr(), vel_bin.dataPtr(), pres_bin.dataPtr(),
                    e_bin.dataPtr(), ncount.dataPtr(), imask.dataPtr(),
                    mask_size, r1, dens_comp, xmom_comp, ymom_comp, pres_comp,
                    rhoe_comp, dx_fine, level_dx.dataPtr(), xctr, yctr);
            }
#else
            if (sphr) {
                fextract3d_sph(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                               BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), nbins,
                               dens_bin.dataPtr(), vel_bin.dataPtr(),
                               pres_bin.dataPtr(), e_bin.dataPtr(),
                               ncount.dataPtr(), imask.dataPtr(), mask_size, r1,
                               dens_comp, xmom_comp, ymom_comp, zmom_comp,
                               pres_comp, rhoe_comp, dx_fine,
                               level_dx.dataPtr(), xctr, yctr, zctr);

            } else {
                fextract3d_cyl(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
                               BL_TO_FORTRAN_FAB(lev_data_mf[mfi]), nbins,
                               dens_bin.dataPtr(), vel_bin.dataPtr(),
                               pres_bin.dataPtr(), ncount.dataPtr(),
                               imask.dataPtr(), mask_size, r1, dens_comp,
                               xmom_comp, ymom_comp, zmom_comp, pres_comp,
    ...
    // write the header
    slicefile << "# " << std::setw(w) << "x" << std::setw(w) << "density"
              << std::setw(w) << "velocity" << std::setw(w) << "pressure"
              << std::setw(w) << "int. energy" << std::endl;
    ...
    for (auto i = 0; i < nbins; i++) {
        ...
        slicefile << std::setw(w) << r[i] << std::setw(w) << dens_bin[i]
                  << std::setw(w) << vel_bin[i] << std::setw(w) << pres_bin[i]
                  << std::setw(w) << e_bin[i] << std::endl;
    }
...
#elif (AMREX_SPACEDIM >= 2)
    if (sphr) {
        Print() << "Extracting slice from " << AMREX_SPACEDIM
                << "d spherical problem" << std::endl;
    } else {
        Print() << "Extracting slice from " << AMREX_SPACEDIM
                << "d cylindrical problem" << std::endl;
    }
#endif
...
Vector<Real> GetCenter(const string pltfile) {
    ...
    std::string s;
    while (std::getline(iss, s, ',')) center.push_back(stod(s));
