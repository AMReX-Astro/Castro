...
int main(int argc, char* argv[])
{
...

	{
		...
		int dens_comp, xmom_comp, ymom_comp, pres_comp, rhoe_comp;

		GetComponents(data, dens_comp, xmom_comp, ymom_comp, pres_comp, rhoe_comp);
		...
		for (int l = finestLevel; l >= 0; l--) {
			...
			for (MFIter mfi(lev_data_mf, true); mfi.isValid(); ++mfi) {
				...
				fgaussian_pulse(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
				           ...
				           nbins, rad_bin.dataPtr(), imask.dataPtr(), mask_size, r1,
