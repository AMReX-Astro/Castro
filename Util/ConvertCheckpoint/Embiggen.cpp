...
#define VSHOWVAL(verbose, val)                              \
    {                                                       \
        if (verbose && ParallelDescriptor::IOProcessor()) { \
            cout << #val << " = " << val << endl;           \
        }                                                   \
    }
LevelBld *getLevelBld() { return 0; }
...
static void ScanArguments() {
    ...
    if (grown_factor <= 1) amrex::Abort("must have grown_factor > 1");
...
static void ReadCheckpointFile(const std::string &fileName) {
    ...
    if (ParallelDescriptor::IOProcessor())
        std::cout << "previous finest_lev is " << fakeAmr.finest_level
                  << std::endl;
    ...
    if (ParallelDescriptor::IOProcessor()) {
        std::cout << "     new finest_lev is " << fakeAmr.finest_level
                  << std::endl;
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        for (i = 1; i <= mx_lev; i++) {
            ...
            std::cout << " ... domain is       " << fakeAmr.geom[i].Domain()
                      << std::endl;
            std::cout << " ...     dx is       "
                      << fakeAmr.geom[i].CellSize()[0] << std::endl;
            std::cout << "  " << std::endl;
        }
    }
    ...
    if ((fakeAmr.level_steps[0] * ref_ratio) != fakeAmr.level_steps[1])
        amrex::Abort(
            "Number of steps in original checkpoint must be divisible by "
            "ref_ratio");
    ...
    if (fakeAmr.level_count[1] == fakeAmr.level_steps[1]) {
        ...
        fakeAmr.level_count[0] =
            std::min(fakeAmr.level_count[1], fakeAmr.level_steps[0]);
        ;
    }
    ...
    for (int lev(n - 1); lev >= 0; lev--) {
        ...
        for (int i = 0; i < ndesc_save; i++) {
            ...
            falRef.state[i].new_time.start =
                falRef_orig.state[i].new_time.start;
            falRef.state[i].new_time.stop = falRef_orig.state[i].new_time.stop;
            falRef.state[i].old_time.start =
                falRef.state[i].new_time.start - fakeAmr.dt_level[lev];
            falRef.state[i].old_time.stop =
                falRef.state[i].new_time.stop - fakeAmr.dt_level[lev];
            ...
            if (nsets_save[i] >= 1) {
                ...
                falRef.state[i].new_data =
                    new MultiFab(falRef.grids, dmap, ncomp, ngrow);
                falRef.state[i].new_data->setVal(0.);

                if (nsets_save[i] == 2) {
                    falRef.state[i].old_data =
                        new MultiFab(falRef.grids, dmap, ncomp, ngrow);
...
// ---------------------------------------------------------------
static void WriteCheckpointFile(const std::string &inFileName,
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        if (oldCastroHeaderFile.good()) {
            ...
            newCastroHeaderFile.open(newCastroHeaderName.c_str(),
                                     std::ios::binary);
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        for (i = 0; i < max_level; i++)
            HeaderFile << fakeAmr.ref_ratio[i] << ' ';
        HeaderFile << '\n';
        for (i = 0; i <= max_level; i++)
            HeaderFile << fakeAmr.dt_level[i] << ' ';
        HeaderFile << '\n';
        for (i = 0; i <= max_level; i++) HeaderFile << fakeAmr.dt_min[i] << ' ';
        HeaderFile << '\n';
        for (i = 0; i <= max_level; i++)
            HeaderFile << fakeAmr.n_cycle[i] << ' ';
        HeaderFile << '\n';
        for (i = 0; i <= max_level; i++)
            HeaderFile << fakeAmr.level_steps[i] << ' ';
        HeaderFile << '\n';
        for (i = 0; i <= max_level; i++)
            HeaderFile << fakeAmr.level_count[i] << ' ';
        HeaderFile << '\n';
    }
    ...
    for (int lev(0); lev <= fakeAmr.finest_level; ++lev) {
        ...
        if (ParallelDescriptor::IOProcessor()) {
            ...
            std::cout << " ... domain is       " << fakeAmr.geom[lev].Domain()
                      << std::endl;
            std::cout << " ...     dx is       "
                      << fakeAmr.geom[lev].CellSize()[0] << std::endl;
            std::cout << "  " << std::endl;
        }
    }
...
static void ConvertData() {
    ...
    for (int n = 0; n < nstatetypes; n++) falRef0.state[n].grids = newgrids;
