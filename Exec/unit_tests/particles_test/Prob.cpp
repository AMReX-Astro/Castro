...
void Castro::problem_post_simulation(
    ...
    if (!ifs.good()) amrex::FileOpenFailed(file);
    ...
    for (int i = 0; i < num_particles; i++) {
        AMREX_D_TERM(ifs >> p.m_rdata.pos[0];, ifs >> p.m_rdata.pos[1];
                     , ifs >> p.m_rdata.pos[2];);
    ...
    for (auto& kv : pmap) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (; !match && it != nparticles.end(); ++it) {
                if (it->m_idata.id == p.m_idata.id) match = true;
            }
            ...
            if (delta == delta) total_err += delta;
