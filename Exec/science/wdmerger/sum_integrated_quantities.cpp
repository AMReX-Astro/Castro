...
void Castro::sum_integrated_quantities() {
    if (level > 0) return;
    ...
    int dataprecision =
        16;  // Number of digits after the decimal point, for float data
    ...
    for (int lev = 0; lev <= finest_level; lev++) {
        ...
        momentum[0] +=
            ca_lev.volWgtSum("inertial_momentum_x", time, local_flag);
        momentum[1] +=
            ca_lev.volWgtSum("inertial_momentum_y", time, local_flag);
        momentum[2] +=
            ca_lev.volWgtSum("inertial_momentum_z", time, local_flag);

        angular_momentum[0] +=
            ca_lev.volWgtSum("inertial_angular_momentum_x", time, local_flag);
        angular_momentum[1] +=
            ca_lev.volWgtSum("inertial_angular_momentum_y", time, local_flag);
        angular_momentum[2] +=
            ca_lev.volWgtSum("inertial_angular_momentum_z", time, local_flag);
        ...
        if (do_grav)
            rho_phi +=
                ca_lev.volProductSum("density", "phiGrav", time, local_flag);
#endif

#ifdef ROTATION
        if (do_rotation)
            rho_phirot +=
                ca_lev.volProductSum("density", "phiRot", time, local_flag);
#endif

#ifdef GRAVITY
#if (BL_SPACEDIM > 1)
        // Gravitational wave signal. This is designed to add to these quantities so we can send them directly.
        ca_lev.gwstrain(time, h_plus_1, h_cross_1, h_plus_2, h_cross_2,
                        h_plus_3, h_cross_3, local_flag);
#endif
#endif
        ...
        for (int i = 0; i < NumSpec; i++)
            species_mass[i] +=
                ca_lev.volWgtSum("rho_" + species_names[i], time, local_flag) /
                M_solar;
    }
    ...
    com_p_mag += std::pow(
        std::pow(com_p[0], 2) + std::pow(com_p[1], 2) + std::pow(com_p[2], 2),
        0.5);
    com_s_mag += std::pow(
        std::pow(com_s[0], 2) + std::pow(com_s[1], 2) + std::pow(com_s[2], 2),
        0.5);
    vel_p_mag += std::pow(
        std::pow(vel_p[0], 2) + std::pow(vel_p[1], 2) + std::pow(vel_p[2], 2),
        0.5);
    vel_s_mag += std::pow(
        std::pow(vel_s[0], 2) + std::pow(vel_s[1], 2) + std::pow(vel_s[2], 2),
        0.5);

#if (BL_SPACEDIM == 3)
    if (mass_p > 0.0) {
        vel_p_rad = (com_p[axis_1 - 1] / com_p_mag) * vel_p[axis_1 - 1] +
                    (com_p[axis_2 - 1] / com_p_mag) * vel_p[axis_2 - 1];
        vel_p_phi = (com_p[axis_1 - 1] / com_p_mag) * vel_p[axis_2 - 1] -
                    (com_p[axis_2 - 1] / com_p_mag) * vel_p[axis_1 - 1];
    }

    if (mass_s > 0.0) {
        vel_s_rad = (com_s[axis_1 - 1] / com_s_mag) * vel_s[axis_1 - 1] +
                    (com_s[axis_2 - 1] / com_s_mag) * vel_s[axis_2 - 1];
        vel_s_phi = (com_s[axis_1 - 1] / com_s_mag) * vel_s[axis_2 - 1] -
                    (com_s[axis_2 - 1] / com_s_mag) * vel_s[axis_1 - 1];
    }
#else
    if (mass_p > 0.0) {
    ...
    if (mass_p > 0.0 && mass_s > 0.0) {
        ...
        for (int i = 0; i < 3; i++) wd_dist[i] = com_s[i] - com_p[i];
        ...
        angle = atan2(wd_dist[axis_2 - 1] - wd_dist_init[axis_2 - 1],
                      wd_dist[axis_1 - 1] - wd_dist_init[axis_1 - 1]) *
                180.0 / M_PI;
    ...
    if (amrex::ParallelDescriptor::IOProcessor()) {
        ...
        if (parent->NumDataLogs() > 0) {
            ...
            if (log.good()) {
                ...
                if (time == 0.0) {
                    ...
                    ++n;
                    header << std::setw(fixwidth)
                           << "                     TIME";
                    ++n;
                    header << std::setw(datwidth)
                           << "             TOTAL ENERGY";
                    ++n;
                    header << std::setw(datwidth)
                           << "             TOTAL E GRID";
                ...
                log << std::setw(fixwidth) << std::setprecision(dataprecision)
                    << time;

                log << std::scientific;

                log << std::setw(datwidth) << std::setprecision(dataprecision)
                    << total_energy;
                log << std::setw(datwidth) << std::setprecision(dataprecision)
                    << total_E_grid;
                log << std::setw(datwidth) << std::setprecision(dataprecision)
            ...
            if (log.good()) {
                ...
                if (time == 0.0) {
                    ...
                    for (int i = 0; i < NumSpec; i++) {
                        ...
                        while (outString.length() + specString.length() +
                                   massString.length() <
                               datwidth)
                            outString += " ";
                        outString += massString;
                        outString += specString;
                        header << std::setw(datwidth) << outString;
                        ++n;
                    }
                ...
                for (int i = 0; i < NumSpec; i++)
                    log << std::setw(datwidth)
                        << std::setprecision(dataprecision) << species_mass[i];
            ...
            if (log.good()) {
                ...
                if (time == 0.0) {
                    ...
                    header << std::setw(fixwidth)
                           << "                       DT";
                    ++n;
                    header << std::setw(intwidth) << "  FINEST LEV";
                    ++n;
                    header << std::setw(fixwidth)
                    ...
                    for (int i = 3; i < 4; ++i) log << std::setw(datwidth) << i;

                    log << std::setw(intwidth)
                        << 4;  // Handle the finest lev column
        ...
        if (parent->NumDataLogs() > 5) {
            ...
            if (log.good()) {
                ...
                for (int i = 0; i <= 6; ++i)
                    log << std::setw(datwidth)
                        << std::setprecision(dataprecision) << rad_p[i];
        ...
        if (parent->NumDataLogs() > 6) {
            ...
            if (log.good()) {
...
#endif
                    header << std::setw(datwidth)
                           << "     SECONDARY T_FREEFALL";
                    ++n;
                    for (int i = 0; i <= 6; ++i) {
                        header << "     SECONDARY 1E" << i << " RADIUS";
                        ++n;
                    }
                ...
                for (int i = 0; i <= 6; ++i)
                    log << std::setw(datwidth)
                        << std::setprecision(dataprecision) << rad_s[i];
