subroutine amrex_probinit(init, name, namlen, problo, probhi) bind(C, name="amrex_probinit")
  ...
  ! ! Read namelists
  ! open(newunit=untin, file=probin(1:namlen), form='formatted',s tatus='old')
  ! read(untin,fortin)
  ! close(unit=untin)
  ...
  do k = lo(3), hi(3)
     do j = lo(2), hi(2)
        do i = lo(1), hi(1)
           ...
           state(i,j,k,URHO) = 1.0
           state(i,j,k,UTEMP) = 0.0
           state(i,j,k,UMX:UMZ) = 0.0

           ! set the composition to be all in the first species
           state(i,j,k,UFS:UFS-1+nspec) = 0.e0_rt
           state(i,j,k,UFS  ) = state(i,j,k,URHO)

           state(i,j,k,UEINT) = 0.0
           state(i,j,k,UEDEN) = 0.0
