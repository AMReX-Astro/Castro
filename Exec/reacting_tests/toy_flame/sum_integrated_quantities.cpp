...
void Castro::sum_integrated_quantities() {
    ...
    for (int lev = 0; lev <= finest_level; lev++) {
        ...
        prod_sgs +=
            0.5 * ca_lev.volWgtSum("prod_sgs", prev_time, local_flag) * dt_crse;
        prod_sgs +=
            0.5 * ca_lev.volWgtSum("prod_sgs", cur_time, local_flag) * dt_crse;
        diss_sgs +=
            0.5 * ca_lev.volWgtSum("diss_sgs", prev_time, local_flag) * dt_crse;
        diss_sgs +=
            0.5 * ca_lev.volWgtSum("diss_sgs", cur_time, local_flag) * dt_crse;
        turb_src +=
            0.5 * ca_lev.volWgtSum("turb_src", prev_time, local_flag) * dt_crse;
        turb_src +=
            0.5 * ca_lev.volWgtSum("turb_src", cur_time, local_flag) * dt_crse;
        ...
        if (lev == 0) ca_lev.flame_speed_properties(time, rho_fuel_dot);
    }

    if (verbose > 0) {
#ifdef SGS
        const int nfoo = 14;
        Real foo[nfoo] = {mass,     mom[0],   mom[1],   mom[2],  rho_e,
                          rho_K,    rho_E,    Etot,     delta_E, delta_K,
        ...
        Lazy::QueueReduction([=]() mutable {
            ...
            if (show_center_of_mass)
                ParallelDescriptor::ReduceRealSum(
                    com, 3, ParallelDescriptor::IOProcessorNumber());
            ...
            if (ParallelDescriptor::IOProcessor()) {
                ...
                std::cout << "TIME= " << time << " TURB_SRC    = " << turb_src
                          << '\n';
                std::cout << "TIME= " << time << " DE+DK-TURB_SRC = "
                          << delta_E + delta_K - turb_src << '\n';
#endif
                std::cout << "TIME= " << time
                          << " FLAME WIDTH = " << flame_width << '\n';
                std::cout << "TIME= " << time
                ...
                if (parent->NumDataLogs() > 0) {
                    ...
                    if (data_log1.good()) {
                        ...
                        if (time == 0.0) {
                            ...
                            data_log1 << std::setw(14) << "  flame speed "
                                      << std::endl;
                        }

                        // Write the quantities at this time
                        data_log1 << std::setw(14) << time;
                        data_log1 << std::setw(14) << std::setprecision(6)
                                  << mass;
                        data_log1 << std::setw(14) << std::setprecision(6)
                                  << mom[0];
                        data_log1 << std::setw(14) << std::setprecision(6)
                ...
                if (parent->NumDataLogs() > 1) {
                    ...
                    if (data_log2.good()) {
                        ...
                        if (time == 0.0) {
                            ...
                            data_log2 << std::setw(14) << "      turb_src"
                                      << std::endl;
                        }

                        data_log2 << std::setw(14) << std::setprecision(6)
                                  << time;
                        data_log2 << std::setw(14) << std::setprecision(6)
                                  << Etot;
                        data_log2 << std::setw(14) << std::setprecision(6)
                        ...
                        data_log2 << std::setw(14) << std::setprecision(6)
                                  << delta_K;
                        data_log2 << std::setw(16) << std::setprecision(10)
                                  << Etot - sum_turb_src;
                        data_log2 << std::setw(14) << std::setprecision(6)
                                  << rms_mach;
                        data_log2 << std::setw(14) << std::setprecision(6)
                ...
                if (show_center_of_mass) {
                    ...
                    std::cout << "TIME= " << time
                              << " CENTER OF MASS X-LOC = " << com[0] << '\n';
                    std::cout << "TIME= " << time
                              << " CENTER OF MASS X-VEL = " << com_vel[0]
                              << '\n';

                    std::cout << "TIME= " << time
                              << " CENTER OF MASS Y-LOC = " << com[1] << '\n';
                    std::cout << "TIME= " << time
                              << " CENTER OF MASS Y-VEL = " << com_vel[1]
                              << '\n';

                    std::cout << "TIME= " << time
                              << " CENTER OF MASS Z-LOC = " << com[2] << '\n';
                    std::cout << "TIME= " << time
                              << " CENTER OF MASS Z-VEL = " << com_vel[2]
                              << '\n';
                }
            }
#ifdef BL_LAZY
