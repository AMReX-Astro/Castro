...
\item {\tt gravity.tol} : if {\tt gravity.gravity\_type =
  PoissonGrav}, this is the absolute tolerance for the Poisson solve. You
  specify a single value for this tolerance, and then the absolute tolerance
  used by the multigrid solve is $\text{tol} \times 4\pi G\, \rho_{\text{max}}$
  where $\rho_{\text{max}}$ is the maximum value of the density on the domain.
  No relative tolerance is used in any solve.
