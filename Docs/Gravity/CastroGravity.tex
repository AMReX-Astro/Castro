...
\item {\tt gravity.abs\_tol} : if {\tt gravity.gravity\_type =
  ...
  is the maximum value of the density on the domain. (No relative tolerance is used in any solve.)
  ...
  For a gravity sync solve, we continue to use as an absolute tolerance the final
  residual norm obtained in the main Poisson solve. The old parameters
  {\tt gravity.ml\_tol}, {\tt gravity.sl\_tol}, and {\tt gravity.delta\_tol} no
  longer have any effect, and the user is warned about this if they try to set these.
