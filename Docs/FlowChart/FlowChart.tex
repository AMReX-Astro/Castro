...
\item {\em Construct time-level $n$ sources and apply}
  ...
  \item {[\ifdef{DIFFUSION}]} diffusion : thermal diffusion can be
    ...
    Chapter~\ref{ch:diffusion} for more details.  \MarginPar{this may
      not be time-centered correctly---see issue \# 22}
...
\item {\em Clean State}
  ...
  \begin{enumerate}
  \item enforces that the density is above \runparam{small\_dens}
