...
\Ub^{n+1,(a)} &= \Ub^\star + \Delta t \Sb(\Ub^n) \\
\Ub^{n+1,(b)} &= \Ub^{n+1,(a)} - \Delta t \nabla \cdot\Fb^\nph(\Ub^\star) \\
\Ub^{n+1,(c)} &= \Ub^{n+1,(b)} + \frac{\dt}{2} [S(\Ub^{n+1,(b)}) - S(\Ub^n)] \label{eq:source_correct}\\
\Ub^{n+1}     &= \Ub^{n+1,(c)} + \frac{\dt}{2} \Sb_{\rm react}(\Ub^{n+1,(c)})
\end{align}\MarginPar{does the first addition of the source terms work off of $\Ub$ or $\Ub^\star$?}
...
approximate Riemann solver construct fluxes.  

At the beginning of the time step, we assume that $\Ub$ and $\phi$ are
defined consistently, i.e., $\rho^n$ and $\phi^n$ satisfy equation
(\ref{eq:Self Gravity}).  Also note that in Eq.~\ref{eq:source_correct}, 
we actually can do the new time-level source updates in sequence, and some
(like rotation) implicitly, making that update more akin to:
\begin{equation}
