...
{[\ifdef{DIFFUSION}]} If diffusion is enabled, the timestep will also
be limited by:
\begin{equation}
...
{[\ifdef{REACTIONS}]} If reactions are enabled, the timestep will also
...
\runparam{castro.dtnuc\_e} and \runparam{castro.dtnuc\_X}. These limiters
...
an additional option \runparam{castro.dtnuc\_X\_threshold} which is the
