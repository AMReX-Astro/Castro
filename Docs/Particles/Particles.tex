%I erazed lines related to mass, but I left all others assuming they were implemented in the code.
%It is possible that there are wrong explanations, so please correct them before actually merging.
%I added a few examples including figures, but please feel free to remove them as you prefer. 
%When I compile with the figures, it does not compile by complaining plot files are not found. 
%Also, if the examples with the figures are to be included in the manual, depending on the configuration of the figures, {left} {right} \textit{bottom} panels should be altered accordingly. 
%I also provide the parameter tables in a separate tex file (particle_parameters.txt). But I do know know how to compile with that file, so it may cause some errors.
