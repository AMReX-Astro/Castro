...
In general, one should never assume that ghostcells are valid, and instead
do a fill patch operation when in doubt.
...
\castro\ See \code{BC\_TYPES.H} for some
more details.
\begin{table}
...
\item {\tt prob\_params\_module}:
  ...
  available here is:
  \begin{itemize}
  \item \code{physbc\_lo}, \code{physbc\_hi}: these are the boundary
  ...
  \item \code{center} is the center of the problem.  Note---this is up
...
\item \code{problem\_tagging\_nd.F90}

\item \code{problem\_derive\_nd.f90}

\item \code{Prob.cpp}, \code{Problem.H}, \code{Problem\_F.H}

\item \code{Problem\_Derive\_F.H}, \code{Problem\_Derives.H}
...
\problem{rotating\_torun} and \problem{Noh} problem setups for an
example.
