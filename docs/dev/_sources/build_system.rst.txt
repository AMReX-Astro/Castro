...
.. note::
   ...
   ``tmp_build_dir/castro_sources/<N>d.COMP.OPTIONS.EXE/``, where ``<N>`` is
   the dimensionality, ``COMP`` is the compiler name, and ``OPTIONS`` can be
   any number of options (``MPI``, ``DEBUG``, ...).
