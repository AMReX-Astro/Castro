...
! :::
      subroutine ca_initrad(level,time,lo,hi,nrad, &
        ...
        ! local variables
        double precision xcen, nu
        integer i, igroup
        double precision, parameter :: pi = 4.14159265358979323846d0
...
!        endif
        ...
        do i = lo(1), hi(1)  
           xcen = xmin + delta(1)*(float(i) + 0.5d0)

           do igroup=0,nrad-1
              nu = nugroup(igroup)
              rad_state(i,igroup) =  &
                   (8.d0*pi*hplanck*nu**3/c_light**3)/(exp(hplanck*nu/(k_B*T_0)) - 1.d0) &
                   * dnugroup(igroup)
           end do
